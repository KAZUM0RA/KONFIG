...
Процедура ВыбранПриИзменении(Элемент)
	...
	ТекущийЭлемент = ТекущиеДанные.ПолучитьРодителя();
	Пока ТекущийЭлемент <> Неопределено Цикл
		...
		Если Не Есть1 И Не Есть2 Тогда
			ТекущийЭлемент.Выбран = 0;
			
		ИначеЕсли Не Есть2 И Не Есть0 Тогда
			ТекущийЭлемент.Выбран = 1;
		Иначе
			ТекущийЭлемент.Выбран = 2;
		КонецЕсли;
		
		ТекущийЭлемент = ТекущийЭлемент.ПолучитьРодителя();
	КонецЦикла;
...
Функция ПроверитьМожноЗакрытьФорму()
	...
	Если Модифицированность И Не БылаНажатаКлавишаЗакрытия Тогда
		...
		Ответ = Вопрос(НСтр("ru='Отказаться от изменений?';uk='Відмовитися від змін?'"), РежимДиалогаВопрос.ДаНет);
