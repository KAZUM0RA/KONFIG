...
"Продолжить запись?';uk='Із зазначеним найменуванням уже був уведений раніше доп.реквізит/відомість. Продовжити запис?'");
...
Процедура ОбновитьНадписьОНаборах()
	...
	Если Количество = 0 Тогда
		ИнформационнаяНадпись = НСтр("ru='Не входит ни в один набор';uk='Не входить ні в один набір'");
		
	ИначеЕсли Количество = 1 Тогда
		ИнформационнаяНадпись = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Входит в набор: %1';uk='Входить у набір: %1'"),
	...
	Иначе
		...
		ИнформационнаяНадпись = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Входит в %1 %2';uk='Входить в %1 %2'"),
			Количество,
			СтрокаНаборы);
	КонецЕсли;
...
Функция ПолучитьЗаголовок(Знач Ссылка, Знач ЭтоДополнительноеСведение)
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Если ЭтоДополнительноеСведение Тогда
			Заголовок = Заголовок + " " + НСтр("ru='(Дополнительное сведение)';uk='(Додаткова відомість)'");
		Иначе
			Заголовок = Заголовок + " " + НСтр("ru='(Дополнительный реквизит)';uk='(Додатковий реквізит)'");
		КонецЕсли;
	Иначе
		Если ЭтоДополнительноеСведение Тогда
...
Процедура УстановитьЗаголовокКнопкиФормата(Знач КнопкаФормы, Знач ФорматНеУстановлен)
	...
	Если ФорматНеУстановлен Тогда
		КнопкаФормы.Заголовок = НСтр("ru='Формат по умолчанию';uk='Формат за замовчуванням'");
	Иначе
		КнопкаФормы.Заголовок = НСтр("ru='Формат установлен';uk='Формат установлений'");
	КонецЕсли;
...
"Редактирование списка наборов не производится.';uk='Властивість відкрита з форми набору. Редагування списку наборів не проводиться.'"));
		Возврат;
	КонецЕсли;
