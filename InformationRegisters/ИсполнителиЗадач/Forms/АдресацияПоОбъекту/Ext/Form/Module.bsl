...
Процедура НазначитьИсполнителей(Команда)
	...
	Если Назначение = Неопределено Тогда
		Предупреждение(НСтр("ru='Необходимо выбрать роль в списке.';uk=' Необхідно вибрати роль у списку.'"));
		Возврат;
	КонецЕсли;
	
	Если ИспользоватьВнешниеЗадачиИБизнесПроцессы Тогда
		Если Назначение.ВнешняяРоль = Истина Тогда
			Предупреждение(НСтр("ru='Исполнители внешней роли должны быть определены в другой информационной базе.';uk='Виконавці зовнішньої ролі повинні бути визначені в іншій інформаційній базі.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;	
...
Процедура ОбновитьДанныеЭлементов()
	...
	Для каждого СтрокаСписка Из СписокОбъект Цикл
		Если СтрокаСписка.Исполнители = 0 Тогда
			СтрокаСписка.ИсполнителиСтрока = ?(СтрокаСписка.ВнешняяРоль, НСтр("ru='заданы в другой базе';uk='задані в іншій базі'"), НСтр("ru='не заданы';uk='не задані'"));
		...
		Иначе
			СтрокаСписка.ИсполнителиСтрока =
			  ...
			  НСтр("ru='%1 чел';uk='%1 чіл'"), Строка(СтрокаСписка.Исполнители) );
