...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ИначеЕсли ТекущийОбъект.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета Тогда
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru='Время в реквизите предмета не определено';uk='Час в реквізиті предмета не визначений'");
...
Процедура СпособУстановкиВремениНапоминанияПриИзменении(Элемент)
	...
	Иначе
		Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Интервал времени не определён.';uk='Інтервал часу не визначений.'");
...
Процедура Прекратить(Команда)
	...
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Прекратить';uk='Припинити'"));
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Не прекращать';uk='Не припиняти'"));
	
	Ответ = Вопрос(НСтр("ru='Прекратить напоминание?';uk='Припинити нагадування?'"), КнопкиДиалога);
...
Функция ПолучитьПредопределенныеСпособыОповещения()
	...
	Результат.Вставить(НСтр("ru='в указанное время';uk='в указаний час'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ВУказанноеВремя"));
...
Процедура ОткрытьДиалогНастройкиРасписания()
	...
	Если ДиалогРасписания.ОткрытьМодально() Тогда
		...
		Если Не РасписаниеСоответствуетТребованиям(Расписание) Тогда 
			Предупреждение(НСтр("ru='Периодичность в течение дня не поддерживается, соответствующие настройки очищены.';uk='Періодичність впродовж дня не підтримується, відповідні налаштування очищені.'"));
		КонецЕсли;
