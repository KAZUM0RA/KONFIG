	...
	Если ТипЗнч(Параметры.СравниваемыеВерсии) = Тип("Массив") Тогда
		...
		Если СравниваемыеВерсии.Количество() > 1 Тогда
			...
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			                 НСтр("ru='Отчет по изменениям версий объекта %1 №№ %2';uk=""Звіт по змінах версій об'єкта %1 №№ %2"""),
			                 СсылкаНаОбъект,
			                 СтрокаНомераВерсий);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			                 НСтр("ru='Версия объекта %1 № %2';uk=""Версія об'єкта %1 № %2"""),
			                 СсылкаНаОбъект,
			...
			ИначеЕсли УровеньЧтения = 2 Тогда // указатель на втором уровне - это реквизит или имя табличной части
				...
				Если Не ЗначениеЗаполнено(НовоеЗР.Тип) Тогда
					...
					Если ОписаниеРеквизита = Неопределено Тогда
						...
						ИначеЕсли ОбщегоНазначения.ЭтоСтандартныйРеквизит(Метаданные.ОбщиеРеквизиты, НаименованиеРеквизита) Тогда
							
							ОписаниеРеквизита = Метаданные.ОбщиеРеквизиты[НаименованиеРеквизита];
...
Функция ПолучитьПредставлениеРеквизитаНаЯзыке(знач ИмяРеквизита)
	...
	Если      ИмяРеквизита = "Number" Тогда
		Возврат НСтр("en='Number';ru='Номер';uk='Номер'");
	ИначеЕсли ИмяРеквизита = "Name" Тогда
		Возврат НСтр("en='Name';ru='Наименование';uk='Найменування'");
	ИначеЕсли ИмяРеквизита = "Code" Тогда
		Возврат НСтр("en='Code';ru='Код';uk='Код'");
	ИначеЕсли ИмяРеквизита = "IsFolder" Тогда
		Возврат НСтр("en='Is folder';ru='ЭтоГруппа';uk='ЭтоГруппа'");
	ИначеЕсли ИмяРеквизита = "Description" Тогда
		Возврат НСтр("en='Description';ru='Наименование';uk='Найменування'");
	ИначеЕсли ИмяРеквизита = "Date" Тогда
		Возврат НСтр("en='Date';ru='Дата';uk='Дата'");
	ИначеЕсли ИмяРеквизита = "Posted" Тогда
		Возврат НСтр("en='Posted';ru='Проведен';uk='Проведений'");
	ИначеЕсли ИмяРеквизита = "DeletionMark" Тогда
		Возврат НСтр("en='Deletion mark';ru='ПометкаУдаления';uk='ПометкаУдаления'");
	ИначеЕсли ИмяРеквизита = "Ref" Тогда
		Возврат НСтр("en='Ref';ru='Ссылка';uk='Посилання'");
	ИначеЕсли ИмяРеквизита = "Parent" Тогда
		Возврат НСтр("en='Parent';ru='Родитель';uk='Батько'");
	ИначеЕсли ИмяРеквизита = "Owner" Тогда
		Возврат НСтр("en='Owner';ru='Владелец';uk='Власник'");
	Иначе
...
Функция ПолучитьОбъектИзXML(ДвоичныеДанные)
	...
	Если ЧтениеXML.Прочитать() Тогда
		...
		Иначе
			ЧтениеXML.Закрыть();
			ВызватьИсключение НСтр("ru='Ошибка при восстановлении объекта';uk=""Помилка при відновленні об'єкта""");
		КонецЕсли;
	Иначе
		ЧтениеXML.Закрыть();
		ВызватьИсключение НСтр("ru='Ошибка чтения данных';uk='Помилка читання даних'");
	КонецЕсли;
