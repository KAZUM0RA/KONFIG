...
Процедура ОчиститьВыполнить()
	...
	Если КоличествоВыделенныхАдресныхОбъектов() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Необходимо выбрать как минимум один адресный объект.';uk="" Необхідно вибрати як мінімум один адресний об'єкт."""), ,
					"АдресныеОбъекты");
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru='Адресные сведения по выбранным объектам будут удалены. Удалить адресные сведения?';uk=""Адресні відомості по обраних об'єктах будуть вилучені. Вилучити адресні відомості?""");
	КодВозврата = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
	
	Если КодВозврата = КодВозвратаДиалога.ОК Тогда
		ОчиститьАдресныеСведения();
		Предупреждение(НСтр("ru='Адресные сведения успешно удалены.';uk='Адресні відомості успішно вилучені.'"),,
		               НСтр("ru='Удаление адресных сведений.';uk='Видалення адресних відомостей.'"));
