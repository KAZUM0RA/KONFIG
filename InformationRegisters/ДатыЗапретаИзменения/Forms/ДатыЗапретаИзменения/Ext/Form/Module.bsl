...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Если НЕ СвойстваРазделов.ИспользоватьДатыЗапретаЗагрузкиДанных Тогда
			ВызватьИсключение(НСтр("ru='Даты запрета загрузки данных не используются.';uk='Дати заборони завантаження даних не використовуються.'"));
		КонецЕсли;
		//
		...
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ДляВсехПользователей").Представление =
			НСтр("ru='Для всех узлов планов обмена';uk=' Для всіх вузлів планів обміну'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ПоПользователям").Представление =
			НСтр("ru='По узлам планов обмена';uk=' По вузлах планів обміну'");
		//
		Элементы.Отчеты.Подсказка                                       = НСтр("ru='Отчеты по датам запрета загрузки данных,"
"установленным ""По узлам планов обмена"".';uk='Звіти по датах заборони завантаження даних, установленим "" По вузлах планів обміну"".'");
		Команды.ДатыЗапретаПоПользователям.Заголовок                    = НСтр("ru='Даты запрета по узлам планов обмена';uk='Дати заборони по вузлах планів обміну'");
		Команды.ДатыЗапретаПоПользователям.Подсказка                    = НСтр("ru='Даты запрета загрузки данных"
"по узлам планов обмена и планам обмена.';uk='Дати заборони завантаження даних  по вузлах планів обміну й планам обміну.'");
		Команды.ДатыЗапретаПоРазделамОбъектамДляПользователей.Заголовок = НСтр("ru='Даты запрета по разделам и объектам для узлов планов обмена';uk=""Дати заборони по розділах і об'єктам для вузлів планів обміну""");
		Команды.ДатыЗапретаПоРазделамОбъектамДляПользователей.Подсказка = НСтр("ru='Даты запрета загрузки данных"
"по разделам и объектам для узлов планов обмена и планов обмена.';uk=""Дати заборони завантаження даних  по розділах і об'єктам для вузлів планів обміну й планів обміну.""");
		//
		Элементы.ПользователиПолноеПредставление.Заголовок = НСтр("ru='План обмена: узел';uk='План обміну: вузол'");
		//
	...
	Иначе
		...
		СписокТиповПользователей.Добавить(Тип("СправочникСсылка.Пользователи"),        НСтр("ru='Пользователь';uk='Користувач'"));
		СписокТиповПользователей.Добавить(Тип("СправочникСсылка.ВнешниеПользователи"), НСтр("ru='Внешний пользователь';uk='Зовнішній користувач'"));
	КонецЕсли;
...
Процедура ОбщаяДатаЗапретаСОписаниемПриИзменении(Знач Контекст, РассчитатьДатуЗапрета = Истина);
	...
	Иначе
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Если Контекст.ТекущаяДатаНаСервере > СрокРазрешения Тогда
				ТекстНадписи = Символы.ПС
				             + НСтр("ru='Срок возможности изменения данных с %3 по %4 истек %2';uk='Строк можливості зміни даних з %3 по %4 минув %2'");
			Иначе
				Если РассчитатьДатуЗапрета Тогда
					Контекст.ДатаЗапрета = РасчетныеДатыЗапрета.Предыдущая;
				КонецЕсли;
				ТекстНадписи = Символы.ПС
				             + НСтр("ru='По %2 возможно изменение данных с %3 по %4';uk='По %2 можливо зміну даних з %3 по %4'")
				             + Символы.ПС
				             + НСтр("ru='После %2 будет запрещено изменение данных по %4';uk='Після %2 буде заборонена зміна даних по %4'")
				             + Символы.ПС;
			КонецЕсли;
		Иначе
		...
		Контекст.Элементы.ПояснениеНепроизвольнойДаты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			  НСтр("ru='Запрещено изменение данных по %1';uk='Заборонена зміна даних по %1'")
			+ ТекстНадписи,
...
Функция СписокОписанийДатыЗапрета()
	...
	Список.Добавить("",                "(" + НСтр("ru='По умолчанию';uk=' За замовчуванням'") + ")");
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Для каждого ЗначениеСписка Из Контекст.СписокТиповПользователей Цикл
			Если ТипЗнч(ЗначениеСписка.Значение) = ТипЗнч(Пользователь) Тогда
				...
				Иначе
					Возврат ЗначениеСписка.Представление + ": " + НСтр("ru='<Все узлы>';uk='<Усі вузли>'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстКомментарияДляВсехПользователей()
	...
	Возврат "(" + НСтр("ru='По умолчанию';uk=' За замовчуванням'") + ")";
...
Функция ТекстЗаголовкаКомандыОтчетПоОбъектам()
	...
	Возврат НСтр("ru='Отчет по объектам';uk=""Звіт по об'єктах""");
...
Функция ТекстЗаголовкаКомандыОтчетПоРазделам()
	...
	Возврат НСтр("ru='Отчет по разделам';uk='Звіт по розділах'");
...
Функция ТекстВопросаПересчитатьДатыЗапрета()
	...
	Возврат НСтр("ru='Очистить незаполненые строки и пересчитать относительные даты запрета?';uk='Очистити пустi рядки й перерахувати відносні дати заборони?'");
...
Функция ТекстВопросаОбновитьДанные()
	...
	Возврат НСтр("ru='Обновить данные?';uk='Обновити дані?'");
...
Функция ТекстВопросаУдалитьВсеДатыЗапретаКромеДатДляВсехПользователей()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удалить все даты запрета, кроме установленных %1?';uk='Вилучити всі дати заборони, крім установлених %1?'"), ЗначениеДляВсехПользователей);
...
Функция ТекстВопросаУдалитьВсеДатыЗапрета()
	...
	Возврат НСтр("ru='Удалить все даты запрета?';uk='Вилучити всі дати заборони?'");
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение %1 не изменяется.';uk='Значення %1 не змінюється.'"), ТекстПредставленияДляВсехПользователей(ЭтаФорма));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Комментарий %1 не изменяется.';uk='Коментар %1 не змінюється.'"), ТекстПредставленияДляВсехПользователей(ЭтаФорма));
...
"поэтому сохранен способ указания ""%2""';uk='Спосіб вказівки ""%1"" не використаний, тому збережений спосіб вказівки ""%2""'"),
...
Функция ТекстСообщенияДляУдаленияВыделитеОднуСтроку()
	...
	Возврат НСтр("ru='Для удаления выделите одну строку.';uk=' Для видалення виділите один рядок.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВсехПользователей()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удалить даты запрета %1?';uk='Вилучити дати заборони %1?'"), НРег(ЗначениеДляВсехПользователей));
...
Функция ТекстВопросаУдалитьДатыЗапретаДляПользователя()
	...
	Возврат НСтр("ru='Удалить даты запрета для пользователя?';uk='Вилучити дати заборони для користувача?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыПользователей()
	...
	Возврат НСтр("ru='Удалить даты запрета для группы пользователей?';uk='Вилучити дати заборони для групи користувачів?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВнешнегоПользователя()
	...
	Возврат НСтр("ru='Удалить даты запрета для внешнего пользователя?';uk='Вилучити дати заборони для зовнішнього користувача?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыВнешнихПользователей()
	...
	Возврат НСтр("ru='Удалить даты запрета для группы внешних пользователей?';uk='Вилучити дати заборони для групи зовнішніх користувачів?'");
...
Функция ТекстВопросаУдалитьДатыЗапрета()
	...
	Возврат НСтр("ru='Удалить даты запрета?';uk='Вилучити дати заборони?'");
...
Функция ТекстСообщенияДляВсехПользователейДатыЗапретаНеУстановлены()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 даты запрета не установлены.';uk='%1 дати заборони не встановлені.'"), ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияОбновитьДанныеФормыF5()
	...
	Возврат НСтр("ru='Обновите данные формы (F5).';uk='Обновите дані форми (F5).'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделовИОбъектов()
	...
	Возврат НСтр("ru='Удалить даты запрета для разделов и объектов?';uk=""Вилучити дати заборони для розділів і об'єктів?""");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляОбъектов()
	...
	Возврат НСтр("ru='Удалить даты запрета для объектов?';uk=""Вилучити дати заборони для об'єктів?""");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделов()
	...
	Возврат НСтр("ru='Удалить даты запрета для разделов?';uk='Вилучити дати заборони для розділів?'");
...
"для разделов.';uk='Розділи вже заповнені, можна встановити дати заборони  для розділів.'");
...
Функция ТекстСообщенияРазделыУжеЗаполненыМожноУстановитьДатыЗапретаДляРазделовИОбъектов()
	...
	Возврат НСтр("ru='Разделы уже заполнены,"
"можно установить даты запрета"
"для разделов и объектов.';uk=""Розділи вже заповнені, можна встановити дати заборони  для розділів і об'єктів.""");
...
"можно установить дату запрета.';uk=""Об'єкт уже обраний, можна встановити дату заборони.""");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделаИОбъектов()
	...
	Возврат НСтр("ru='Удалить даты запрета для раздела и объектов?';uk=""Вилучити дати заборони для розділу й об'єктів?""");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРаздела()
	...
	Возврат НСтр("ru='Удалить дату запрета для раздела?';uk='Вилучити дату заборони для розділу?'");
...
Функция ТекстВопросаУдалитьОбщуюДатуЗапрета()
	...
	Возврат НСтр("ru='Удалить общую дату запрета?';uk='Вилучити загальну дату заборони?'");
...
Функция ТекстВопросаУдалитьДатуЗапретаОбъекта()
	...
	Возврат НСтр("ru='Удалить дату запрета для объекта?';uk=""Вилучити дату заборони для об'єкта?""");
...
Функция ТекстСообщенияДляВсехПользователейРазделыВсегдаПоказываются()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 разделы всегда показываются.';uk='%1 розділи завжди показуються.'"), ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияПриУдаленииДатаЗапретаОчищается()
	...
	Возврат НСтр("ru='При удалении дата запрета очищается.';uk=' При видаленні дата заборони очищається.'");
...
Функция ТекстСообщенияДляВсехПользователейОбщаяДатаВсегдаПоказывается()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 <Общая дата> всегда показывается.';uk='%1 <Загальна дата> завжди показується.'"), ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияНедостаточноПравДляВыбораВнешнихПользователей()
	...
	Возврат НСтр("ru='Недостаточно прав для выбора внешних пользователей.';uk='Недостатньо правий для вибору зовнішніх користувачів.'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиПользователямиНеСохранены(Контекст)
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Возврат НСтр("ru='Настройки с невыбранными узлами планов обмена не сохранены.';uk='Настроювання з невибраними вузлами планів обміну не збережені.'");
	КонецЕсли;
	
	Возврат НСтр("ru='Настройки с невыбранными пользователями не сохранены.';uk='Настроювання з невибраними користувачами не збережені.'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиОбъектамиНеСохранены()
	...
	Возврат НСтр("ru='Настройки с невыбранными объектами не сохранены.';uk=""Настроювання з невибраними об'єктами не збережені.""");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляОбъектовНеСохранены()
	...
	Возврат НСтр("ru='Настройки с незаполненными датами запрета для объектов не сохранены.';uk=""Настроювання з незаповненими датами заборони для об'єктів не збережені.""");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляРазделовНеСохранены()
	...
	Возврат НСтр("ru='Настройки с незаполненными датами запрета для разделов не сохранены.';uk='Настроювання з незаповненими датами заборони для розділів не збережені.'");
...
Функция ТекстСообщенияОбщаяДатаУжеПоказана()
	...
	Возврат НСтр("ru='<Общая дата> уже показана.';uk='<Загальна дата> уже показана.'");
...
Функция ТекстСообщенияВсеРазделыУжеПоказаны()
	...
	Возврат НСтр("ru='Все разделы уже показаны.';uk='Усі розділи вже показані.'");
...
Функция ТекстСообщенияСнятоВыделениеСНезаполненныхСтрок()
	...
	Возврат НСтр("ru='Снято выделение с незаполненных строк.';uk='Зняте виділення з незаповнених рядків.'");
