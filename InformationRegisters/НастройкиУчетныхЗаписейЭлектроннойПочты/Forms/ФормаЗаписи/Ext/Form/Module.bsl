...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Запись.ПерсональнаяУчетнаяЗапись Тогда
		Если ПользовательУчетнойЗаписи.Пустая() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан пользователь учетной записи';uk='Не вказаний користувач облікового запису'"),,
			                                                       "ПользовательУчетнойЗаписи");
		КонецЕсли;
	Иначе
		
		Если Запись.ОтветственныйЗаВедениеПапок.Пустая() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан владелец ответственный за ведение папок';uk='Не вказаний власник відповідальний за ведення тек'"),,
			                                                       "ОтветственныйЗаВедениеПапок",
			                                                       "Запись",Истина);
		КонецЕсли;
		
		Если Запись.ОтветственныйЗаОбработкуПисем.Пустая() Тогда
			 ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан владелец ответственный за обработку писем';uk='Не вказаний власник відповідальний за обробку листів'"),,
