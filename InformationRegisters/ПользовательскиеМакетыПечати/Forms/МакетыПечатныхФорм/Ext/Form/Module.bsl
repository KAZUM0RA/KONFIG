...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ЗапроситьПодтверждениеЗакрытияФормы(
		Отказ, 
		ЕстьРедактируемые, , 
		НСтр("ru='В списке остались макеты отмеченные как редактируемые. Закрыть форму?';uk='У списку залишилися макети відзначені, що як редагуються. Закрити форму?'")); 
			...
			Исключение
				...
				СообщениеОбОшибке = СообщениеОбОшибке + Символы.ПС + 
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Файл %1 открыт другим приложением. Действие не выполнено.';uk='Файл %1 відкритий іншим додатком. Дія не виконана.'"),
...
#Если ВебКлиент Тогда
	Попытка
		Если НЕ ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь) Тогда
			Предупреждение(НСтр("ru='Не удалось поместить файл макета в информационную базу.';uk='Не вдалося помістити файл макета в інформаційну базу.'"));
			Возврат;
		КонецЕсли;
	Исключение
		СообщениеОбОшибке = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при помещении файла макета в информационную базу: %1. Убедитесь, что приложение работы с файлом закрыто.';uk='Помилка при приміщенні файлу макета в інформаційну базу: %1. Переконаєтеся, що додаток роботи з файлом закрите.'"), СообщениеОбОшибке));
		Возврат;
	КонецПопытки;
#КонецЕсли
...
#КонецЕсли
	...
	Если КоличествоМакетов = 1 Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Макет изменен';uk='Макет змінений'"),, 
			Элементы.СписокМакетов.ДанныеСтроки(ВыделенныеСтроки[0]).Представление);
	ИначеЕсли КоличествоМакетов > 1 Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Макеты изменены';uk='Макети змінені'"),,
			НСтр("ru='Количество макетов:';uk='Кількість макетів:'") + " " + КоличествоМакетов);
	КонецЕсли;
...
Процедура УдалитьИЗИБ(Команда)
    ...
    КоличествоМакетов = ВыделенныеСтроки.Количество();
	Если КоличествоМакетов = 1 Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Удален пользовательский макет';uk='Вилучений користувацький макет'"),,
			Элементы.СписокМакетов.ДанныеСтроки(ВыделенныеСтроки[0]).Представление);
	ИначеЕсли КоличествоМакетов > 1 Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Удалены пользовательские макеты';uk='Вилучені користувацькі макети'"),,
			НСтр("ru='Количество макетов:';uk='Кількість макетів:'") + " " + КоличествоМакетов);
	КонецЕсли;
...
Функция ПолучитьТекстПоИспользованию(знач Использование)
	...
	Если Использование Тогда
		Возврат НСтр("ru='Пользовательский';uk='Користувацький'");
	Иначе
		Возврат НСтр("ru='Поставляемый';uk='Що поставляється'");
	КонецЕсли;
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		...
		Если ЭлементДерева.ТипМакета = "MXL" И Редактировать Тогда
			Если ПустаяСтрока(ТекстПредупреждения) Тогда
				ТекстПредупреждения = НСтр("ru='Файлы макетов табличных документов сохранены на диск.';uk='Файли макетів табличних документів збережені на диск.'");
			КонецЕсли;
...
"""%2"".';uk='Макет ""%1"" збережений на диск під іменем ""%2"".'"),
						ЭлементДерева.ИмяМакета,
						ПутьКФайлуМакета);
		Иначе
