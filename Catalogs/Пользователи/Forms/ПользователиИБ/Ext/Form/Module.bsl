...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для открытия списка пользователей информационной базы.';uk='Недостатньо правий для відкриття списку користувачів інформаційної бази.'");
	КонецЕсли;
...
Процедура ПользователиИнфБазыПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ПользователиИнфБазы.ТекущиеДанные.КодПроблемы = 0 Тогда
		Ответ = Вопрос(НСтр("ru='Удалить пользователя информационной базы?';uk='Вилучити користувача інформаційної бази?'"), РежимДиалогаВопрос.ДаНет);
...
"""Доступ к информационной базе разрешен"" в соответствующей форме элемента.';uk='Данному пользователю информационной базы не сопоставлен элемент справочника."
""
"Для создания пользователя информационной базы,"
"связанного с пользователем или внешним пользователем применяйте флажок "
