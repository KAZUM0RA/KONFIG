...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не НомерПорядкаУникален(ПорядокЗаполнения, Ссылка) Тогда
		ТекстОшибки = НСтр("ru='Порядок заполнения не уникален - в системе уже есть том с таким порядком';uk='Порядок заповнення не унікальний - у системі уже є тому з таким порядком'");
	...
	Если МаксимальныйРазмер <> 0 Тогда
		...
		Если МаксимальныйРазмер < ТекущийРазмер Тогда
			ТекстОшибки = НСтр("ru='Максимальный размер тома меньше, чем текущий размер';uk='Максимальний розмір тому менше, чим поточний розмір'");
	...
	Если Не ПустаяСтрока(ПолныйПутьWindows) И (Лев(ПолныйПутьWindows, 2) <> "\\" ИЛИ Найти(ПолныйПутьWindows, ":") <> 0) Тогда
		ТекстОшибки = НСтр("ru='Путь к тому должен быть в формате UNC (\\servername\resource) ';uk='Шлях до того повинен бути у форматі UNC (\\servername\resource) '");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ПолныйПутьWindows", "Объект", Отказ);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Путь к тому некорректен. Возможно учетная запись, от лица которой работает сервер 1С:Предприятия, не имеет прав доступа к каталогу тома: ';uk='Шлях до того некоректний. Можливо обліковий запис, від імені якої працює сервер 1С:Підприємства, не має прав доступу до каталогу тому: '");
