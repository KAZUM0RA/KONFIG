...
Процедура ПометитьНаУдалениеСнятьПометку()
	...
	Если Пометка Тогда 
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Cнять с ""%1"" пометку на удаление?';uk='Cнять із ""%1"" позначку на видалення?'"),
			Строка(ТекущиеДанные.Ссылка));
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пометить ""%1"" на удаление?';uk='Позначити ""%1"" на видалення?'"),
			Строка(ТекущиеДанные.Ссылка));
	КонецЕсли;	
...
Процедура СделатьАктивнойВыполнить()
	...
	Иначе
		Предупреждение(НСтр("ru='Смена активной версии разрешена только для не занятых файлов.';uk='Зміна активної версії дозволена тільки для не зайнятих файлів.'"));
	КонецЕсли;	
...
"   Текстовый документ OpenDocument (.odt)';uk='Порівняння версій підтримується тільки для файлів наступних типів:     Текстовий документ (.txt)    Документ формату RTF (.rtf)     Документ Microsoft Word (.doc, .docx)     Документ HTML (.html .htm)     Текстовий документ Opendocument (.odt)'"));
				Возврат;
			КонецЕсли;
			...
			ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется сравнение версий файла ""%1""...';uk='Виконується порівняння версій файлу ""%1""...'"), ДанныеФайла1.Ссылка);
			...
			Состояние();
		#Иначе
			Предупреждение(НСтр("ru='Сравнение версий в веб-клиенте не поддерживается.';uk='Порівняння версій у веб-клієнтові не підтримується.'"));
		#КонецЕсли
