...
Процедура ПередЗаписью(Отказ)
		...
		Если НЕ ПривилегированныйРежим() И ЗаписьПодписанногоОбъекта <> Истина И НЕ УстановленаПометкаУдаления Тогда
			...
			Если ЗначениеЗаполнено(Ссылка) Тогда
				...
				Если ПодписанЭЦП И СсылкаПодписан Тогда
					ВызватьИсключение НСтр("ru='Подписанную версию нельзя редактировать.';uk='Підписану версію не можна редагувати.'");
				КонецЕсли;
	...
	Если Владелец.ТекущаяВерсия = Ссылка Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Активную версию нельзя удалить.';uk='Активну версію не можна вилучити.'"));
		КонецЕсли;
	ИначеЕсли РодительскаяВерсия.Пустая() Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Первую версию нельзя удалить.';uk='Першу версію не можна вилучити.'"));
		КонецЕсли;
