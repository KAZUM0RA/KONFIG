...
""
"Установить профиль в группе доступа (нет - открыть только для просмотра)?';uk='У групи доступу Адміністратори повинен бути профіль Адміністратор.  Установити профіль у групі доступу (немає - відкрити тільки для перегляду)?'"),
...
""
"Обновить виды доступа в группе доступа (нет - открыть только для просмотра)?';uk='Змінився состав видів доступу профілю цієї групи доступу.  Обновити види доступу в групі доступу (немає - відкрити тільки для перегляду)?'"),
		               РежимДиалогаВопрос.ДаНет,
		               ,
		               КодВозвратаДиалога.Нет);
	...
	Для каждого ТекущаяСтрока Из ДеревоПользователейСтроки Цикл
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Пользователь повторяется.';uk='Користувач повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Пользователь в строке %1 повторяется.';uk='Користувач у рядку %1 повторюється.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Внешний пользователь повторяется.';uk='Зовнішній користувач повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Внешний пользователь в строке %1 повторяется.';uk='Зовнішній користувач у рядку %1 повторюється.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Группа пользователей повторяется.';uk='Група користувачів повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей в строке %1 повторяется.';uk='Група користувачів у рядку %1 повторюється.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ru='Группа внешних пользователей повторяется.';uk='Група зовнішніх користувачів повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа внешних пользователей в строке %1 повторяется.';uk='Група зовнішніх користувачів у рядку %1 повторюється.'");
			КонецЕсли;
		...
		Если Объект.Ссылка = Справочники.ГруппыДоступа.Администраторы
		   И ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Внешний пользователь недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач неприпустимий у визначеній групі доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Внешний пользователь в строке %1 недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач у рядку %1 неприпустимий у визначеній групі доступу Адміністратори.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Группа пользователей недопустима в предопределенной группе доступа Администраторы.';uk='Група користувачів неприпустима у визначеній групі доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група користувачів у рядку %1 неприпустима у визначеній групі доступу Адміністратори.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ru='Группа внешних пользователей недопустима в предопределенной группе доступа Администраторы.';uk='Група зовнішніх користувачів неприпустима у визначеній групі доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа внешних пользователей в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група зовнішніх користувачів у рядку %1 неприпустима у визначеній групі доступу Адміністратори.'");
			КонецЕсли;
...
"которому разрешен доступ к информационной базе.';uk='У групі доступу Адміністратори  повинен бути хоча б один користувач, якому дозволений доступ до інформаційної бази.'"));
		КонецЕсли;
	КонецЕсли;
...
"Их нельзя удалить в этом списке.';uk='Користувачі груп відображаються для відомості, що вони одержують доступ груп користувачів. Їх не можна вилучити в цьому списку.'"));
			//
...
"Их нельзя удалить в этом списке.';uk='Зовнішні користувачі груп відображаються для відомості, що вони одержують доступ груп зовнішніх користувачів. Їх не можна вилучити в цьому списку.'"));
			//
...
"Их нельзя удалить в этом списке.';uk='Керівники відображаються для відомості, що вони одержують доступ підлеглих. Їх не можна вилучити в цьому списку.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура НачальнаяНастройкаПриЧтенииИСоздании(ТекущийОбъект)
	...
	Если ТекущийОбъект.Ссылка <> Справочники.ГруппыДоступа.Администраторы Тогда
		...
		Если ЗначениеЗаполнено(ТекущийОбъект.Пользователь) Тогда
			АвтоЗаголовок = Ложь;
			Заголовок = ТекущийОбъект.Наименование + ": " + ТекущийОбъект.Пользователь + " " + НСтр("ru='(Группа доступа)';uk='(Група доступу)'");
			//
...
Процедура ЗаполнитьСписокТиповПользователей()
	...
	ТипыПользователей.Добавить(Неопределено, НСтр("ru='Произвольные участники';uk='Довільні учасники'"));
	ТипыПользователей.Добавить(Справочники.Пользователи.ПустаяСсылка(), НСтр("ru='Обычные пользователи';uk='Звичайні користувачі'"));
		...
		Для каждого ТипСсылкиОбъектаАвторизации ИЗ Метаданные.Справочники.ВнешниеПользователи.Реквизиты.ОбъектАвторизации.Тип.Типы() Цикл
			...
			ТипыПользователей.Добавить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Внешние пользователи (%1)';uk='Зовнішні користувачі (%1)'"),
	...
	ПредставлениеТипаПользователей =
		?(НайденныйЭлемент = Неопределено,
		  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		  		НСтр("ru='Неизвестный тип ""%1""';uk='Невідомий тип ""%1""'"),
...
Функция ВыбранТипПользователиИлиВнешниеПользователи(ВыборИПодборВнешнихПользователей)
	...
	Если ИспользоватьВнешнихПользователей Тогда
		Элемент = СписокТиповПользователей.ВыбратьЭлемент(НСтр("ru='Выбор типа данных';uk='Вибір типу даних'"), СписокТиповПользователей[0]);
...
Процедура ВыбратьПодобратьПользователей(Подобрать)
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Иначе
			Предупреждение(НСтр("ru='Недостаточно прав для выбора внешних пользователей.';uk='Недостатньо правий для вибору зовнішніх користувачів.'"));
		КонецЕсли;
	Иначе
...
Процедура ОбновитьРуководителейПользователя(Знач ОписаниеПользователя, Знач РуководителиПодчиненных, Знач ИзмененныеСтроки)
	...
	Если СтарыеРуководители.Количество() <> НовыеРуководители.Количество() Тогда
		...
		Для каждого Строка Из НовыеРуководители Цикл
			...
			ОписаниеРуководителя.Пользователь = Строка.ПредставлениеРуководителя + " (" + НСтр("ru='руководитель';uk='керівник'") + ")";
		КонецЦикла;
