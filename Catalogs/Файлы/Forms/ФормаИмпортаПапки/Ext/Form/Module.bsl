...
Процедура ИмпортВыполнить()
	...
	Если ПустаяСтрока(Каталог) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран каталог для импорта.';uk='Не обраний каталог для імпорту.'"), , "Каталог");
		Возврат;
		
	КонецЕсли;
	
	Если ПапкаДляДобавления.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите папку.';uk='Укажіть папку.'"), , "ПапкаДляДобавления");
		Возврат;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("КаталогНаДиске") Тогда 	
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данная обработка вызывается из других процедур конфигурации. Вручную ее вызывать запрещено.';uk='Дана обробка викликається з інших процедур конфігурації. Вручну її викликати заборонене.'")); 
		Отказ = Истина;
		Возврат;
	КонецЕсли;
