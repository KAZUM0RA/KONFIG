	...
	#Если НЕ ВебКлиент Тогда
		...
		Если не КаталогВыгрузки.Существует() Тогда
			...
			Исключение
				...
				Предупреждение(НСтр("ru='Не удалось создать папку выгрузки.';uk='Не вдалося створити папку вивантаження.'") + Символы.ВК + НСтр("ru='Причина:';uk='Причина:'") + ИнфоОшибка.Описание);
				Возврат;
...
"Пожалуйста, подождите.';uk='Виконується експорт папки ""%1""...  Будь ласка, почекайте.'"),
						Строка(ЧтоСохраняем) ));
		...
		Если Успех Тогда
			...
			Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			             НСтр("ru='Экспорт папки ""%1"" в каталог на диске ""%2"" успешно завершен.';uk='Експорт папки ""%1"" у каталог на диску ""%2"" успішно завершений.'"),
...
"Причина: %2';uk='Помилка створення папки ""%1"". Причина: %2'"),
			...
			Пока Истина Цикл
				...
				Если ФайлНаДиске.Существует() и ФайлНаДиске.ЭтоКаталог() Тогда
					ТекстВопроса =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					  НСтр("ru='Вместо файла ""%1""  существует папка с таким же именем. Повторить экспорт этого файла?';uk=' Замість файлу ""%1""  існує папка з таким же іменем. Повторити експорт цього файлу?'"),
					  ПолноеИмяФайла );
...
"Перезаписать существующий файл файлом из системы?';uk='У папці ""%1"" існує файл ""%2"" розмір існуючого файлу = %3 байт, дата зміни %4. розмір файлу, що зберігається, = %5 байт, дата зміни %6. Перезаписати існуючий файл файлом із системи?'"),
						      БазовыйКаталогСохраненияФайла,
			...
			Если Результат = КодВозвратаДиалога.Да Тогда
				Пока Истина Цикл
					Попытка
						...
						НадписьПодробнее =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Сохраняется на диск файл ""%1"" (%2 Мб)...';uk='Зберігається на диск файл ""%1"" (%2 Мб)...'"),
							ФайлНаДиске.Имя,
							ФайловыеФункцииКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб) );
						
						Состояние(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						    	НСтр("ru='Экспорт папки ""%1""';uk='Експорт папки ""%1""'"), ЗаписьФайла.НаименованиеПапки),
							,
							НадписьПодробнее, 
...
"Причина: %2';uk='Помилка запису файлу ""%1"". Причина: %2'"),
						  ПолноеИмяФайла,
						  ?(инфоОшибка.Причина = Неопределено, "Неопределено", инфоОшибка.Причина.Описание) );
...
Процедура ПапкаДляЭкспортаОткрытие(Элемент, СтандартнаяОбработка)
	#Если НЕ ВебКлиент Тогда
		...
		Если Не ПустаяСтрока(ПапкаДляЭкспорта) Тогда
			...
			Иначе
				Предупреждение(НСтр("ru='Невозможно открыть папку выгрузки. Возможно, папка еще не создана.';uk='Неможливо відкрити папку вивантаження. Можливо, папка ще не створена.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		Предупреждение(НСтр("ru='В Веб-клиенте экспорт каталогов не поддерживается.';uk='У Веб-клієнтові експорт каталогів не підтримується.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
