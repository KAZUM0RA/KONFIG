...
" Скопировать в новый файл сведения об ЭЦП?';uk='Файл ""%1"" подписан. "
" Копирование сведений об ЭЦП в новый файл сделает его недоступным для изменения. "
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ФайловыеФункцииКлиентСервер.РасширениеФайлаВСписке(СписокРасширенийТекстовыхФайлов, Объект.ТекущаяВерсияРасширение) Тогда
		...
		Если Не ЗначениеЗаполнено(Кодировка) Тогда
			Кодировка = НСтр("ru='По умолчанию';uk=' За замовчуванням'");
		КонецЕсли;	
...
Процедура ЗаполнитьСписокШифрования()
	...
	ТекстЗаголовка = НСтр("ru='Зашифрован для';uk='Зашифрований для'");
...
Процедура ЗаполнитьСписокПодписей()
	...
	ТекстЗаголовка = НСтр("ru='ЭЦП';uk='ЭЦП'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ФайлРедактировался И СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ТекущийПользователь = Объект.Редактирует Тогда 
		Ответ = Вопрос(НСтр("ru='Файл занят вами для редактирования. Закрыть карточку?';uk='Файл зайнятий вами для редагування. Закрити картку?'"), РежимДиалогаВопрос.ДаНет, ,КодВозвратаДиалога.Нет);
...
"Данные будут записаны.';uk='Дані ще не записані. Виконання дії ""Підписати"" можливо тільки після запису даних. Дані будуть записані.'");
...
"Данные будут записаны.';uk='Дані ще не записані. Виконання дії ""Зашифрувати"" можливо тільки після запису даних. Дані будуть записані.'");
		Результат = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
...
Функция ПолучитьПарольДляРасшифровки(Пароль)
	...
	Если Объект.Зашифрован Тогда
		...
		Пароль = "";
		ЗаголовокФормы = НСтр("ru='Введите пароль для расшифровки';uk='Уведіть пароль для розшифрування'");
...
Процедура Удалить(Команда)
	...
	Ответ = Вопрос(НСтр("ru='Удалить выделенные подписи?';uk='Вилучити виділені підписи?'"), РежимДиалогаВопрос.ДаНет);
	Если Ответ = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;	
