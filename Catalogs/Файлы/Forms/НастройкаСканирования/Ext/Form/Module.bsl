...
Процедура ОбновитьСостояние()
	...
	Иначе
		ВерсияКомпонентыСканирования = НСтр("ru='Компонента сканирования не установлена';uk='Компонента сканування не встановлена'");
		Элементы.ИмяУстройства.Доступность = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИспользоватьImageMagickДляПреобразованияВPDF Тогда
		Элементы.ФорматСканированногоИзображения.Заголовок = НСтр("ru='Формат';uk='Формат'");
	Иначе
		Элементы.ФорматСканированногоИзображения.Заголовок = НСтр("ru='Тип';uk='Тип'");
	КонецЕсли;	
...
Процедура ПрочитатьНастройкиСканера()
	...
	Если Не ПустаяСтрока(ИмяУстройства) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						 НСтр("ru='Идет сбор сведений о сканере ""%1""...';uk='Іде збір відомостей про сканер ""%1""...'"), ИмяУстройства);
...
Процедура ПутьКПрограммеКонвертацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Фильтр = НСтр("ru='Исполняемые файлы(*.exe)|*.exe';uk='Файли, що виконуються(*.exe)|*.exe'");
	ДиалогОткрытияФайла.Фильтр = Фильтр;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("ru='Выберите файл для преобразования в PDF';uk='Виберіть файл для перетворення в PDF'");
