...
"Автор (%1): %2';uk='Помилка заповнення автора розсилки"
"Автор (%1): %2'"),
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ПравоВывода Тогда
		Предупреждение(НСтр("ru='Недостаточно прав на вывод информации. Рассылки отчетов не будут выполнены.';uk='Недостатньо прав на виведення інформації. Розсилки звітів не будуть виконані.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЗначенияИзменились 
		...
		Заголовок = НСтр("ru='Отключить архивацию';uk='Відключити архівацію'");
		
		ТекстВопроса = НСтр("ru='При публикации отчетов в папку рекомендуется отключать архивацию в ZIP.';uk='При публікації звітів в теку рекомендується відключати архівацію в ZIP.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ru='Отключить архивацию в ZIP';uk='Відключити архівацію в ZIP'"));
		Кнопки.Добавить(2, НСтр("ru='Продолжить';uk='Продовжити'"));
		...
		Если ПроверитьПерсонализированныеОтборы Тогда
			...
			Если ОтчетПерсонализирован Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru=""В параметрах отчета %1 задан отбор по получателям рассылки, необходимо отключить отбор по получателям или изменить вид рассылки на 'Свой отчет для каждого получателя'."";uk=""У параметрах звіту %1 заданий відбір по одержувачах розсилки, необхідно відключити відбір по одержувачах або змінити вид розсилки на 'Свій звіт для кожного одержувача'."""),
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Получатели.Количество() > 0 Тогда
		...
		НаборКнопок.Добавить("Очистить", НСтр("ru='Очистить';uk='Очистити'"));
	...
	Если НЕ ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		...
		Предупреждение(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недостаточно прав на изменение файлов папки %1';uk='Недостатньо прав на зміну файлів теки %1'"),
...
Процедура РодительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = Кэш.ГруппаЛичныхРассылок Тогда
		СтандартнаяОбработка = Ложь;
		Предупреждение(НСтр("ru='Выбранная группа используется только для личных рассылок по электронной почте';uk='Вибрана група використовується тільки для особистих розсилок по електронній пошті'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Результат.Заголовок = НСтр("ru='Ошибки добавления отчетов в рассылку';uk='Помилки додавання звітів в розсилку'");
		
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			Результат.Текст = НСтр("ru='Не удалось добавить отчет из-за ошибок';uk='Не вдалося додати звіт із-за помилок'");
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			Результат.Текст = НСтр("ru='Не удалось добавить отчеты из-за ошибок';uk='Не вдалося додати звіти із-за помилок'");
...
"Не добавлено из-за ошибок: %3';uk='Додано звіти: %1 з %2"
"Не додано із-за помилок: %3'"),
...
"%2';uk='Успішно додані звіти:"
"%1"
""
"Наступні звіти не вдалося додати із-за помилок:"
"%2'"),
	...
	ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
		...
		Результат.Текст = НСтр("ru='Все выбранные отчеты уже включены в рассылку';uk='Усі вибрані звіти вже включені в розсилку'");
		
	Иначе
		
		Результат.Оповестить = Истина;
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			Результат.Заголовок = НСтр("ru='Отчет включен в рассылку';uk='Звіт включений в розсилку'");
		Иначе
			Результат.Заголовок = НСтр("ru='Отчеты включены в рассылку';uk='Звіти включені в розсилку'");
		КонецЕсли;
	...
	Если НЕ Отказ И НЕ ОтменаРедактирования Тогда
		...
		Если Найденные.Количество() > 1 Тогда
			...
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Дублирование получателя %1. Ввод дубликата отменен.';uk='Дублювання одержувача %1 Введення дубліката скасоване.'"),
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	Вариант = СписокВариантов.ВыбратьЭлемент(НСтр("ru='Выберите шаблон расписания';uk='Виберіть шаблон розкладу'"));
...
Процедура ДобавитьШаблонПолучателя(Команда)
	...
	Если НЕ Объект.Персонализирована Тогда
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Использование получателя в тексте шаблона возможно только для вида рассылки %1';uk='Використання одержувача в тексті шаблону можливе тільки для виду розсилки %1'"),
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 уже соответствует шаблону по умолчанию.';uk='%1 вже відповідає шаблону за умовчанням.'"),
			?(ОчищаетсяТема, НСтр("ru='Тема письма';uk='Тема листа'"), НСтр("ru='Текст письма';uk='Текст листа'"))
		);
	...
	Иначе
		...
		Предмет = ?(ОчищаетсяТема, НСтр("ru='тему письма';uk='тему листа'"), НСтр("ru='текст письма';uk='текст листа'"));
		
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Заменить %1 на шаблон по умолчанию?';uk='Замінити %1 на шаблон за умовчуваням?'"),
			Предмет
		);
		
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Добавить в %1 шаблон по умолчанию';uk='Додати в %1 шаблон за умовчанням'"),
			Предмет
		);
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ru='Заменить';uk='Замінити'"));
		Кнопки.Добавить(2, НСтр("ru='Добавить';uk='Додати'")); //  без замены
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ВидПочтовогоАдресаПолучателейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если НЕ ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Перед выбором почтового адреса необходимо выбрать получателей';uk='Перед вибором поштової адреси необхідно вибрати одержувачів'"),
			,
	...
	Для Индекс = 1 По Объект.Отчеты.Количество() Цикл
		...
		Если НЕ ПараметрыОтчета.Инициализирован Тогда
			...
			ПричинаУдаленияСтрок = ПричинаУдаленияСтрок + ?(ПричинаУдаленияСтрок = "", "", Символы.ПС) 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отчет %1 был удален из-за ошибки инициализации (см. выше).';uk='Звіт %1  був видалений через помилку ініціалізації(см вищий).'"),
...
Функция ОтчетыПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки, ДобавитьДежурнуюФразу = Истина)
	...
	Если СтрокаОтчеты = Неопределено Тогда
		ИдентификаторТекущейСтрокиТаблицыОтчетов = -1;
		Возврат НСтр("ru='Не выбрана строка отчета';uk='Не вибраний рядок звіту'");
	КонецЕсли;
...
Функция ИнициалиазироватьОтчет(Отчет, НастройкиОтчета)
	...
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru='Рассылка отчетов. Инициализация отчета';uk='Розсилка звітів. Ініціалізація звіту'"));
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Проверка способа доставки';uk='Розсилка звітів. Перевірка способу доставки'"));
	...
	Если Выполнена Тогда
		Результат.Текст = НСтр("ru='Тест способа доставки успешно пройден.';uk='Тест способу доставки успішно пройдений.'");
	Иначе
		Результат.Текст = НСтр("ru='Тест способа доставки не пройден.';uk='Тест способу доставки не пройдений.'");
	КонецЕсли;
...
Функция СформироватьОтчетНаСервере(Знач ПараметрыОтчета, Получатель, ТребуемыеРеквизиты)
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Формирование отчета';uk='Розсилка звітів. Формування звіту'"));
	...
	Если ОтчетИнициализирован Тогда
		Попытка
			...
			Если РезультатФормирования.Пустой И НЕ ПараметрыОтчета.ОтправлятьЕслиПустой Тогда
				Результат.Оповестить = Ложь;
				Результат.Текст = НСтр("ru='Отчет пустой';uk='Звіт порожній'");
			Иначе
				Результат.ТабДок = РезультатФормирования.ТабДок;
				Результат.Оповестить = Истина;
				Результат.Заголовок = НСтр("ru='Отчет сформирован';uk='Звіт сформований'");
				Результат.Текст = Строка(ПараметрыОтчета.Отчет);
			КонецЕсли;
		Исключение
			РассылкаОтчетов.ЗаписьЖурнала(ПараметрыЖурнала, , 
				НСтр("ru='Ошибка формирования отчета %1:';uk='Помилка формування звіту %1:'"), ИнформацияОбОшибке(), 
	...
	Если НЕ ОтчетИнициализирован ИЛИ НЕ ОтчетСформирован Тогда
		Результат.Оповестить = Ложь;
		Результат.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать отчет %1.';uk='Не вдалося сформувати звіт %1.'"),
...
Процедура ВидимостьДоступностьКорректность(ЭтаФорма, Изменения = "")
	...
	Если Изменения = "" ИЛИ Изменения = "ВидРассылки" Тогда
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(ЭтаФорма, "Родитель", , 
				НСтр("ru='Группа установлена в соответствии с видом рассылки';uk='Група встановлена відповідно до виду розсилки'")
			);
	...
	Если Изменения = "" 
		...
		СпособыДоставки = "";
		Если Объект.ИспользоватьПапку Тогда
			СпособыДоставки = НСтр("ru='папка';uk='тека'");
		КонецЕсли;
		Если Объект.ИспользоватьСетевойКаталог Тогда
			СпособыДоставки = НСтр("ru='сетевой каталог';uk='мережевий каталог'");
		КонецЕсли;
		...
		Если Объект.ИспользоватьЭлектроннуюПочту И НЕ Объект.ТолькоУведомить Тогда
			СпособыДоставки = СпособыДоставки + ?(СпособыДоставки = "", НСтр("ru='эл. почта';uk='ел. пошта'"), " "+ НСтр("ru='и эл. почта';uk='та ел. пошта'"));
		КонецЕсли;
		
		Элементы.СтраницаДоставка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Доставка (%1)';uk='Доставка (%1)'"),
			СпособыДоставки
		);
	КонецЕсли;
...
Функция ПредставлениеФорматовПоУмолчанию()
	Возврат НСтр("ru='по умолчанию';uk='за умовчанням'");
КонецФункции
...
Процедура ДобавитьПеретащитьПолучателя(ПолучательИлиНаборПолучателей)
	...
	Если МассивНовыхСтрок.Количество() > 0 Тогда
		Если МассивНовыхСтрок.Количество() = 1 Тогда
			ЗаголовокОповещения = НСтр("ru='Получатель добавлен в рассылку';uk='Одержувач доданий в розсилку'");
		Иначе
...
Функция ВыбратьФормат(СсылкаНаОтчет)
	...
	Если ЗаполняетсяФорматПоУмолчанию Тогда
		ЗаголовокФормыВыбора = НСтр("ru='Выберите форматы по умолчанию';uk='Виберіть формати за умовчанням'");
	Иначе
		ЗаголовокФормыВыбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выберите форматы для отчета %1';uk='Виберіть формати для звіту %1'"), 
...
Процедура НачалоВыбораНастройкиОтчета(Элемент, ДанныеВыбора, СтандартнаяОбработка, СКД = Неопределено)
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""Получатели"" не заполнено';uk='Полі ""Получатели"" не заповнено'"),
			,
			"ТипПолучателейРассылки"
		);
		СтандартнаяОбработка = Ложь;
	...
	СписокТипов.Добавить("[Получатель]", НСтр("ru='Получатель отчета';uk='Одержувач звіту'"));
...
"Нажмите ""Продолжить"", чтобы установить флажок ""Подготовлена"" и записать рассылку.';uk='Перед перевіркою розсилка має бути підготовлена."
"Натисніть ""Продовжити"", щоб встановити прапорець ""Підготовлена"" і записати розсилку.'");
		Иначе
			ТекстВопроса = НСтр("ru='Перед проверкой рассылка должна быть записана."
"Нажмите ""Продолжить"", чтобы записать рассылку.';uk='Перед перевіркою розсилка має бути записана."
"Натисніть ""Продовжити"", щоб записати розсилку.'");
		...
		Ответ = Вопрос(ТекстВопроса, Кнопки, 60, 1, НСтр("ru='Проверка способа доставки';uk='Перевірка способу доставки'"));
		Если Ответ = 1 Тогда
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("ru='Тестовое сообщение 1С:Предприятие';uk='Тестове повідомлення 1С :Предприятие'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("ru='Это сообщение отправлено системой рассылок 1С:Предприятие.';uk='Це повідомлення відправлене системою розсилок 1С :Предприятие.'")
...
Процедура ПроверитьТипПолучателей(Отказ)
	Если НЕ ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
		ТекстСообщения = НСтр("ru='Поле ""Получатели"" не заполнено';uk='Полі ""Получатели"" не заповнено'");
...
Процедура ПроверитьКоличествоПолучателей(Отказ)
	Если Объект.Получатели.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Не введено ни одной строки в список ""Получатели"".';uk='Не введено жодного рядка в список ""Одержувачі"".'");
