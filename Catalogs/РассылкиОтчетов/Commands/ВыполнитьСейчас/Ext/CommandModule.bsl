...
Процедура ОбработкаКоманды(МассивРассылок, Параметры)
	...
	Если ЭтаФорма.ИмяФормы = "Справочник.РассылкиОтчетов.Форма.ФормаЭлемента" Тогда
		ТекстСообщения = НСтр("ru='Выполняется рассылка отчетов';uk='Виконується розсилка звітів'");
		
		Объект = ЭтаФорма.Объект;
		Если НЕ Объект.Подготовлена Тогда
			Предупреждение(НСтр("ru='Рассылка не подготовлена';uk='Розсилка не підготовлена'"));
			Возврат;
		КонецЕсли;
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru='Выполняются рассылки отчетов';uk='Виконуються розсилки звітів'");
	КонецЕсли;
