	...
	Если НЕ Подготовлена И ИспользоватьЭлектроннуюПочту И НЕ Личная Тогда
		...
		Если Получатели.Количество() > 0 Тогда
			...
			Если ВключенныеПолучатели.Количество() = 0 Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Необходимо включить хотя бы одного получателя.';uk='Необхідно включити хоч би одного одержувача.'");
...
Функция ПредставлениеЗаданияПоРассылке(НаименованиеРассылки)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Рассылка отчетов: %1';uk='Розсилка звітів: %1'"), 
