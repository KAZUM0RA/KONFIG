...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Иначе
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ru='Настроить параметры подключения вручную';uk='Настроїти параметри підключення вручну'");
	КонецЕсли;
...
"%1 завершена.';uk='Настройка параметрів облікового запису"
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты()
	...
	Тема = НСтр("ru='Тестовое сообщение 1С:Предприятие';uk='Тестове повідомлення 1С:Підприємство'");
	Тело = НСтр("ru='Это сообщение отправлено подсистемой электронной почты Учет в ОСМД';uk='Це повідомлення відправлене підсистемою електронної пошти Облік в ОСББ'");
...
Процедура СоздатьУчетнуюЗапись()
	Если КонтекстныйРежим И ПустаяСтрока(ОбщегоНазначения.ПолучитьЗначениеРеквизита(Справочники.УчетныеЗаписиЭлектроннойПочты.СистемнаяУчетнаяЗаписьЭлектроннойПочты, "АдресЭлектроннойПочты")) Тогда
...
"Настройте параметры подключения вручную.';uk=""Не вдалося визначити настройки з'єднання. "
