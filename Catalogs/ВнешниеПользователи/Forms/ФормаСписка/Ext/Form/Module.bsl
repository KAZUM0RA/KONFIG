	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Если Параметры.ВыборГруппВнешнихПользователей Тогда
				Заголовок                                              = НСтр("ru='Подбор внешних пользователей и групп';uk='Добір зовнішніх користувачів і груп'");
				Элементы.ВыбратьВнешнегоПользователя.Заголовок         = НСтр("ru='Выбрать внешних пользователей';uk='Вибрати зовнішніх користувачів'");
				Элементы.ВыбратьГруппуВнешнихПользователей.Заголовок   = НСтр("ru='Выбрать группы';uk='Вибрати групи'");
			...
			Иначе
				Заголовок                                              = НСтр("ru='Подбор внешних пользователей';uk='Добір зовнішніх користувачів'");
			КонецЕсли;
		Иначе
			// Режим выбора
			Если Параметры.ВыборГруппВнешнихПользователей Тогда
...
Процедура ОбновитьСодержимоеФормыПриИзмененииГруппы(Знач ТекущиеДанные = Неопределено)
	...
	Если НЕ ИспользоватьГруппы
	 ИЛИ Элементы.ГруппыВнешнихПользователей.ТекущаяСтрока = ГруппаВнешнихПользователейВсеПользователи Тогда
		...
		ОписаниеОтображаемыхВнешнихПользователей = НСтр("ru='Все внешние пользователи';uk='Усі зовнішні користувачі'");
	...
	Иначе
		Если Элементы.ГруппыВнешнихПользователей.ТекущаяСтрока <> Неопределено И
			...
			ОписаниеОтображаемыхВнешнихПользователей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Все %1';uk='Усі %1'"),
							НРег(?(ЭлементПредставленияТипаОбъектаАвторизации = Неопределено, НСтр("ru='<Недопустимый тип>';uk='<Неприпустимий тип>'"), ЭлементПредставленияТипаОбъектаАвторизации.Представление)) );
			//
			Элементы.ГруппаПоказыватьВнешнихПользователейДочернихГрупп.ТекущаяСтраница = Элементы.ГруппаНельзяУстановитьСвойство;
			ОбновитьЗначениеПараметраКомпоновкиДанных(ВнешниеПользователиСписок, "ВыбиратьИерархически", Истина);
		Иначе
