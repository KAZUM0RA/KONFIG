...
""
"Для продолжения необходимо изменить вид Публикации с ""%2"" на ""%3"" или ""%4"".';uk='Ім''я ""%1"", використовуване даним звітом (обробкою), уже зайняте іншим опублікованим додатковим звітом (обробкою).    Для продовження необхідно змінити вид Публікації з ""%2"" на ""%3"" або ""%4"".'"),
					ИмяОбъекта,
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЭтоНовый() Тогда
		Если Вид <> ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "Вид") Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Невозможно сменить вид у существующего дополнительного отчета или обработки.';uk='Неможливо перемінити вид в існуючого додаткового звіту або обробки.'"),,,,
				Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяОбъекта = "ПечатьКвитанцииНаОплату" Тогда
		Назначение.Очистить();	
	КонецЕсли;
