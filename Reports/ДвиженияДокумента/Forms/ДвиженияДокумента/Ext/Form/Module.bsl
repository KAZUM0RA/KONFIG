...
Процедура СформироватьСписокПолей(МетаданныеРесурса, ТаблицаПолей, СписокПолей)
	...
	Для Каждого Ресурс Из МетаданныеРесурса Цикл
		...
		Ресурс_Синоним = Ресурс.Синоним;
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВариантКоттеджныйПоселок") Тогда
			Если Ресурс.Имя = "Здание" Тогда
				Ресурс_Синоним = НСтр("ru='Коттеджный поселок';uk='Котеджне селище'");	
			КонецЕсли;
		КонецЕсли;
		ТаблицаПолей.Колонки.Добавить(Ресурс.Имя, , Ресурс_Синоним);
...
Процедура ОбработатьВыводДанныхПоМассиву(СписокПолей, ТаблицаРесурсов, ТаблицаИзмерений, ТаблицаРеквизитов, ТаблицаВидДвижений = Неопределено, Знач ИмяРегистра, СинонимРегистра)
	...
	Если Отчет.СпособВыводаОтчета = Перечисления.СпособВыводаОтчета.ПоГоризонтали Тогда
		...
		Если ТаблицаВидДвижений <> Неопределено Тогда
			ОбластьЗаголовокЯчейки.Параметры.ЗаголовокКолонки = НСтр("ru='Вид движения';uk='Вид руху'", КодЯзыкаПечать);
...
Процедура СформироватьОтчет()
    ...
    Если НЕ ЗначениеЗаполнено(Отчет.Документ) Тогда
		...
		Сообщение.Текст = НСтр("ru='Не выбран документ!';uk='Не обраний документ!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
