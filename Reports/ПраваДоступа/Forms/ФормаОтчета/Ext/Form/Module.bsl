...
Процедура ВывестиОтчет(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
		Свойства.Вставить("ПредставлениеВидаОбъекта",    НСтр("ru='пользователя';uk='користувача'"));
		Свойства.Вставить("ПредставлениеВидаОбъектаДоп", НСтр("ru='пользователю';uk='користувачеві'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Пользователь: ';uk='Користувач: '") + Ссылка);
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Свойства.Вставить("ПредставлениеВидаОбъекта",    НСтр("ru='внешнего пользователя';uk='зовнішнього користувача'"));
		Свойства.Вставить("ПредставлениеВидаОбъектаДоп", НСтр("ru='внешнему пользователю';uk='зовнішньому користувачеві'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Внешний пользователь: ';uk='Зовнішній користувач: '") + Ссылка);
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
		Свойства.Вставить("ПредставлениеВидаОбъекта",    НСтр("ru='группы пользователей';uk='групи користувачів'"));
		Свойства.Вставить("ПредставлениеВидаОбъектаДоп", НСтр("ru='группе пользователей';uk='групі користувачів'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Группа пользователей: ';uk='Група користувачів: '") + Ссылка);
	Иначе
		Свойства.Вставить("ПредставлениеВидаОбъекта",    НСтр("ru='группы внешних пользователей';uk='групи зовнішніх користувачів'"));
		Свойства.Вставить("ПредставлениеВидаОбъектаДоп", НСтр("ru='группе внешних пользователей';uk='групі зовнішніх користувачів'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Группа внешних пользователей: ';uk='Група зовнішніх користувачів: '") + Ссылка);
	КонецЕсли;
	...
	Если НЕ ВыводПравГруппы Тогда
		...
		Если ПользовательИБПрочитан Тогда
			...
			Если НЕ ЗначениеЗаполнено(СвойстваПользователяИБ.ПользовательИнфБазыПользовательОС) Тогда
				Область.Параметры.ПользовательИнфБазыПользовательОС = НСтр("ru='Не задан';uk='Не заданий'");
			КонецЕсли;
			Документ.Вывести(Область, 2);
		Иначе
	...
	Если ПодробныеСведенияОПравахДоступа Тогда
		...
		Область.Параметры.Заполнить(Свойства);
		Область.Параметры.ПредставлениеДоступа = ?(ОднаПерсональнаяГруппа, НСтр("ru='Ограничения доступа';uk='Обмеження доступу'"), НСтр("ru='Группы доступа';uk='Групи доступу'"));
		Документ.Вывести(Область, 1);
	...
	Для каждого ОписаниеВидаОбъектов Из ПраваОбъектов.Строки Цикл
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
			...
			Область.Параметры.ПредставлениеОбъекта = ОписаниеОбъекта.ПредставлениеОбъекта
				+ "
				  |(" + НСтр("ru='просмотр';uk='перегляд'")
				+ ?(ОписаниеОбъекта.Просмотр,
					"",
					НСтр("ru='*';uk='*'") )
				+ ", "
				+ ?(ОписаниеОбъекта.ЧтениеБезОграничения,
					НСтр("ru='не ограничен';uk='не обмежений'"),
					НСтр("ru='ограничен';uk='обмежений'") )
				+ ")";
							...
							Иначе
								...
								Если ПодробныеСведенияОПравахДоступа Тогда
									...
									Область.Параметры.ПредставлениеПрофиляИлиГруппыДоступа = ОписаниеГруппыДоступа.ПредставлениеГруппыДоступа
										+ "
										  |("
										+ НСтр("ru='профиль';uk='профіль'") + ": " + ОписаниеПрофиля.ПредставлениеПрофиля + ", "
										+ ?(КоличествоРолей > 1, НСтр("ru='роли';uk='ролі'"), НСтр("ru='роль';uk='роль'")) + ":"
										+ "
										  |"
									...
									Если ОписаниеВидаДоступа.Строки.Количество() = 0 Тогда
										Область.Параметры.ПредставлениеВидаДоступа = ОписаниеВидаДоступа.ПредставлениеВидаДоступа
											...
											+ ?(ОписаниеВидаДоступа.ДоступРазрешен,
												   НСтр("ru='без запрещенных';uk=' без заборонених'"),
												   НСтр("ru='без разрешенных';uk='без разрешенных'"))
											+ ?(ОписаниеВидаДоступа.ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.Пользователи,
												НСтр("ru=', текущий пользователь всегда разрешен';uk=' пользователь всегда разрешен'"),
												"")
											+ ?(ОписаниеВидаДоступа.ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.ВнешниеПользователи,
												НСтр("ru=', текущий внешний пользователь всегда разрешен';uk=', поточний зовнішній користувач завжди дозволений'"),
												"")
											+ ")";
									Иначе
										Область.Параметры.ПредставлениеВидаДоступа = ОписаниеВидаДоступа.ПредставлениеВидаДоступа
											...
											+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													?(ОписаниеВидаДоступа.ДоступРазрешен,
													  НСтр("ru='запрешенные';uk='забороненi'"),
													  НСтр("ru='разрешенные';uk='дозволені'")),
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
			...
			Область.Параметры.ПредставлениеОбъекта = ОписаниеОбъекта.ПредставлениеОбъекта
				+ ?(НЕ ДобавлениеИспользуется,
					"",
					Символы.ПС + "("
					+ НСтр("ru='добавление';uk='додавання'")
					+ ?(НЕ ОписаниеОбъекта.Добавление,
						" " + НСтр("ru='не доступно';uk='не доступно'"),
						?(ОписаниеОбъекта.ИнтерактивноеДобавление,
							"",
							НСтр("ru='*';uk='*'") )
						+ ", "
						+ ?(ОписаниеОбъекта.ДобавлениеБезОграничения,
							НСтр("ru='не ограничено';uk='не обмежене'"),
							НСтр("ru='ограничено';uk='обмежене'") ) ) )
				+ Символы.ПС
	...
	Для каждого ОписаниеТипаЗначений Из ПраваПоЗначениям.Строки Цикл
		...
		// Вывод шапки таблицы
		МакетШапки.Область("R1C1").Текст = НСтр("ru='Расп';uk='Расп'");
		МакетШапки.Область("R1C1").ШиринаКолонки = 6;
		МакетШапки.Область("R1C2").Текст = НСтр("ru='Тип прав';uk='Тип прав'");
		...
		Если НЕ ВыводПравГруппы Тогда
			НомерЯчейки = "R1C" + Формат(ТекущийНомерОбласти, "ЧГ=");
			МакетШапки.Область(НомерЯчейки).Текст = НСтр("ru='Настрока прав от';uk='Настрока прав від'");
		...
		Для каждого ОписаниеОбъекта Из ОписаниеТипаЗначений.Строки Цикл
			...
			МакетСтроки.Область("R1C2").Текст = ?(ОписаниеОбъекта.Запрещено, НСтр("ru='Запрещено';uk='Заборонене'"), НСтр("ru='Разрешено';uk='Дозволене'"));
			...
			Если НЕ ВыводПравГруппы Тогда
				Если ОписаниеОбъекта.ГрупповоеУчастие И НЕ ОписаниеОбъекта.ПодчиненноеУчастие Тогда
					МакетСтроки.Область(НомерЯчейки).Текст = ОписаниеПрава.ПользовательНаименование + " (" + НСтр("ru='группа';uk='група'") + ")";
				ИначеЕсли ОписаниеОбъекта.ГрупповоеУчастие И ОписаниеОбъекта.ПодчиненноеУчастие Тогда
					МакетСтроки.Область(НомерЯчейки).Текст = ОписаниеПрава.ПользовательНаименование + " (" + НСтр("ru='группа подчиненного';uk='група подчиненного'") + ")";
				ИначеЕсли НЕ ОписаниеОбъекта.ГрупповоеУчастие И ОписаниеОбъекта.ПодчиненноеУчастие Тогда
					МакетСтроки.Область(НомерЯчейки).Текст = ОписаниеПрава.ПользовательНаименование + " (" + НСтр("ru='подчиненный';uk='підлеглий'") + ")";;
				КонецЕсли;
			КонецЕсли;
...
Функция ПредставлениеРежимаЗапуска(РежимЗапуска)
	...
	ИначеЕсли РежимЗапуска = "УправляемоеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ru='Управляемое приложение';uk='Керований додаток'");
	Иначе
