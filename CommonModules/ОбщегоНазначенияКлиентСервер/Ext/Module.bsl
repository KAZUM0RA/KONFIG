...
//                    например, НСтр("ru='Пользователь не выбран.';uk='Користувач не обраний.'").
//
...
//                    свойством ГруппаОшибок, например, НСтр("ru='Пользователь в строке %1 не выбран.';uk='Користувач у рядку %1 не обраний.'").
//
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		                    НСтр("ru='Неправильный формат строки версии: %1';uk='Неправильний формат рядка версії: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Строка2, ".");
	Если Версия2.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	                         НСтр("ru='Неправильный формат строки версии: %1';uk='Неправильний формат рядка версії: %1'"), СтрокаВерсии2);
	КонецЕсли;
	...
	Для Каждого СтрокаАдреса Из МассивАдресов Цикл
		...
		Если АдресОпределен Тогда 
			Если ЕстьНедопустимыеСимволыВПсевдониме Тогда
				ОписаниеОшибки = НСтр("ru='Представление содержит недопустимые символы';uk='Вистава містить неприпустимі символи'");
			КонецЕсли;
		Иначе
