...
Процедура ДополнитьПолучателей(Объект, Узлы) Экспорт
	...
	Для Каждого Элемент Из Узлы Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru='Для состава плана обмена [ИмяПланаОбмена] не указана регистрация объекта [ПолноеИмя]';uk="" Для состава плану обміну [ИмяПланаОбмена] не зазначена реєстрація об'єкта [ПолноеИмя]""");
...
"Выполните обмен данными два раза и повторите попытку.';uk='Настроювання обміну даними не може бути змінена. Існують зареєстровані зміни даних для обміну. Виконаєте обмін даними два рази й повторите спробу.'"
		);
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ПодробноеПредставлениеОшибки,,,, Отказ);
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными';uk='Обмін даними'"), УровеньЖурналаРегистрации.Ошибка,, Источник.Ссылка, ПодробноеПредставлениеОшибки);
...
Функция ОбъектМодифицированДляПланаОбмена(Источник, ОбъектМетаданных, ИмяПланаОбмена, РежимЗаписи, ЗарегистрироватьОбъектКВыгрузке)
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ru='Ошибка определения модифицированности объекта: %1';uk=""Помилка визначення модифiкованностi об'єкта: %1""");
...
"Описание ошибки: %2';uk=""Помилка виконання правил реєстрації об'єктів для плану обміну %1. Опис помилки: %2""");
		//
...
"[ЗначениеКонстанты]"
"// {Окончание алгоритма}"
"';uk='Ошибка алгоритма вычисления значения константы:"
"План обмена: [ИмяПланаОбмена]"
"Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]"
"Алгоритм:"
...
Процедура ЗаписьЖурналаРегистрацииПРО(Комментарий, ОбъектМетаданных = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Правила регистрации объектов';uk=""Обмін даними.Правила реєстрації об'єктів"""), УровеньЖурналаРегистрации.Ошибка, ОбъектМетаданных, , Комментарий);
...
"Описание ошибки: [Описание]';uk='Помилка при виконанні оброблювача: ""[ИмяОбработчика]""; План обміну: [ИмяПланаОбмена]; Об''єкт метаданих: [ОбъектМетаданныхИмя] Опис помилки: [Опис]'");
			//
