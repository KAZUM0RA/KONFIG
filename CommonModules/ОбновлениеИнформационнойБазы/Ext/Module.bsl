...
Функция ПроверитьНевозможностьОбновленияИнформационнойБазы() Экспорт
	...
	Возврат ТребуетсяОбновлениеОбщихДанных
		ИЛИ НеобходимоОбновлениеИнформационнойБазы() И НЕ ЕстьПраваНаОбновлениеИнформационнойБазы();
...
"Обратитесь к системному администратору.';uk='Не виконана загальна частина відновлення інформаційної бази.  Звернетеся до системного адміністратора.'");
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Изменился номер версии конфигурации: с ""%1"" на ""%2"". Будет выполнено обновление информационной базы.';uk='Змінився номер версії конфігурації: з ""%1"" на ""%2"". Буде виконане відновлення інформаційної бази.'"),
	...
	Если НЕ ЕстьПраваНаОбновлениеИнформационнойБазы() Тогда
		Сообщение = НСтр("ru='Недостаточно прав для выполнения обновления. Обратитесь к системному администратору.';uk='Недостатньо правий для виконання відновлення. Звернетеся до системного адміністратора.'");
		ЗаписатьОшибку(Сообщение);
		ВызватьИсключение Сообщение;
	КонецЕсли;
...
""
"Подробности ошибки:"
"%1';uk='Неможливо виконати відновлення інформаційної бази, тому що до неї підключені інші сеанси.  Звернетеся до системного адміністратора.  Подробиці помилки: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"%2';uk='Відновлення інформаційної бази на версію ""%1"" завершилося з помилками:  %2'"), ВерсияМетаданных, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьОшибку(Сообщение);
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обновление информационной базы на версию ""%1"" выполнено успешно.';uk='Відновлення інформаційної бази на версію ""%1"" виконане успішно.'"), ВерсияМетаданных);
	ЗаписатьИнформацию(Сообщение);
...
Функция ВыполнитьИтерациюОбновления(Знач ИдентификаторБиблиотеки, Знач ВерсияМетаданныхИБ, 
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ru='Выполняются обязательные процедуры обновления информационной базы.';uk=""Виконуються обов'язкові процедури відновлення інформаційної бази.""");
		Иначе
			НоваяВерсияИБ = Версия.Версия;
			
			Если ИдентификаторБиблиотеки = Метаданные.Имя Тогда 
				Сообщение = НСтр("ru='Выполняется обновление информационной базы с версии %1 на версию %2.';uk='Виконується відновлення інформаційної бази з версії %1 на версію %2.'");
			Иначе
				Сообщение = НСтр("ru='Выполняется обновление информационной базы родительской конфигурации %3 с версии %1 на версию %2.';uk='Виконується відновлення інформаційної бази батьківської конфігурації %3 з версії %1 на версію %2.'");
			КонецЕсли;
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ru='Выполнены обязательные процедуры обновления информационной базы.';uk=""Виконані обов'язкові процедури відновлення інформаційної бази.""");
		Иначе
			// Установка номера версии информационной базы
			УстановитьВерсиюИБ(ИдентификаторБиблиотеки, НоваяВерсияИБ);
			
			Если ИдентификаторБиблиотеки = Метаданные.Имя Тогда 
				Сообщение = НСтр("ru='Выполнено обновление информационной базы с версии %1 на версию %2.';uk='Виконане відновлення інформаційної бази з версії %1 на версію %2.'");
			Иначе
				Сообщение = НСтр("ru='Выполнено обновление информационной базы родительской конфигурации %3 с версии %1 на версию %2.';uk='Виконане відновлення інформаційної бази батьківської конфігурації %3 з версії %1 на версію %2.'");
			КонецЕсли;
...
Функция ПервыйЗапуск() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() Тогда
		...
		Если НЕ Константы.ИспользоватьРазделениеПоОбластямДанных.Получить()
			И НЕ Константы.НеИспользоватьРазделениеПоОбластямДанных.Получить() Тогда
...
Процедура СохранитьИнформациюОбОбновленииДляПользователей(Знач ВыполненныеОбработчики)
	...
	Для каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		ОписаниеНастроек.Представление = НСтр("ru='Выполненные обработчики обновления информационной базы';uk='Виконані оброблювачі відновлення інформаційної бази'");
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Обновление информационной базы';uk='Відновлення інформаційної бази'");
