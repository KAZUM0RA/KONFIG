...
Процедура КомандаОбновитьИндексППДВыполнить(ИнформацияОбАктуальностиИндексаППД,ППДВключен,ИндексАктуален) Экспорт
	...
	СтрВопроса = НСтр("ru='Обновление индекса полнотекстового поиска может занять длительное время. Продолжить?';uk='Оновлення індексу повнотекстового пошуку може зайняти тривалий час. Продовжити?'");
...
Функция ПриИзмененииФорматаСообщенияНаОбычныйТекст() Экспорт
	...
	ТекстСообщения = НСтр("ru='При преобразовании этого сообщения в обычный текст будут утеряны все элементы оформления, картинки и прочие вставленные элементы. Продолжить?';uk='При перетворенні цього повідомлення в звичайний текст будуть загублені усі елементи оформлення, картинки і інші вставлені елементи. Продовжити?'");
	Результат = Вопрос(ТекстСообщения, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, НСтр("ru='Изменение формата письма';uk='Зміна формату листа'"));
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование,ТолькоПочта,ДокументыДоступныеДляСоздания,ПараметрыСоздания = Неопределено) Экспорт
	...
	Если Копирование Тогда
		...
		Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Или
			...
			Если Не ТолькоПочта Тогда
				Предупреждение(НСтр("ru='Копирование электронных писем запрещено';uk='Копіювання електронних листів заборонене'"));
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			Отказ = Истина;
			РезультатВыбора = ДокументыДоступныеДляСоздания.ВыбратьЭлемент(НСтр("ru='Выберите тип документа';uk='Виберіть тип документу'"));
