...
"""По возрастанию"" находилось на первой позиции.';uk=' Для зміни порядку елементів необхідно настроїти сортування списку таким чином, щоб поле ""Порядок"" з видом сортування "" По зростанню"" перебувало на першій позиції.'"));
		Возврат Ложь;
	КонецЕсли;
...
"кроме отборов по Владельцу и по Группе.';uk=' Для зміни порядку елементів необхідно відключити всі відбори,  крім відборів по Власникові й по Групі.'"));
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого ЭлементГруппировки Из СписокРеквизит.Группировка.Элементы Цикл
		Если ЭлементГруппировки.Использование Тогда
			Предупреждение(НСтр("ru='Для изменения порядка элементов необходимо отключить все группировки.';uk=' Для зміни порядку елементів необхідно відключити всі угруповання.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
