...
Процедура ОтправитьЗагрузитьПочтуПользователя(ЭлементСписок = Неопределено) Экспорт
	...
	Состояние(НСтр("ru='Идет отправка и получение электронной почты ...';uk='Йде відправка і отримання електронної пошти ...'"));
	УправлениеЭлектроннойПочтой.ОтправитьЗагрузитьПочтуПользователя(ПолученоПисем, ДоступноУчетныхЗаписей, ЕстьОшибки);
	Состояние();
	
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Получено писем: %1 ';uk='Отримано листів: %1 '"), ПолученоПисем);
	Если ДоступноУчетныхЗаписей > 1 Тогда
		ТекстСообщения = ТекстСообщения + " " +
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(учетных записей: %1)';uk='(облікових записів: %1)'"),
	...
	Если ЕстьОшибки Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='При получении почты были ошибки. Подробности см. в журнале регистрации';uk='При отриманні пошти були помилки. Подробиці см в журналі реєстрації'"));
