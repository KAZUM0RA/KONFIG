...
//       ВариантыОтчетов.УстановитьОписаниеВариантаВДопНастройках(Настройки, "ПОСчету50", НСтр("ru='Кассовые операции за текущий месяц';uk='Касові операції за поточний місяць'"));
//   КонецПроцедуры
//
...
//	
//	Процедура НастройкиОтчета(Настройки) Экспорт
//		ВариантыОтчетов.ДобавитьПредопределенныйВариантВДопНастройках(Настройки, "Отчет.ОСВ\ПОСчету50", НСтр("ru='Кассовые операции';uk='Касові операції'"), Новый Структура("Счет", 50));
//	КонецПроцедуры
//
...
Функция ПравоИзмененияВариантов(КлючОбъекта = Неопределено, ЧерезИсключение = Ложь) Экспорт
			...
			Если НЕ ПравоДоступа("Просмотр", ОтчетМД) Тогда
				Если ЧерезИсключение Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Недостаточно прав доступа для работы с отчетом %1';uk='Недостатньо правий доступу для роботи зі звітом %1'"),
	...
	Иначе
		...
		Если ОтчетМД = Неопределено Тогда
			Если ЧерезИсключение Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отчет %1 не найден в системе';uk='Звіт %1 не знайдений у системі'"),
					КлючОбъекта
				);
			Иначе
				Возврат Ложь;
		...
		ИначеЕсли НЕ ПравоДоступа("Использование", ОтчетМД) Тогда
			Если ЧерезИсключение Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Недостаточно прав доступа к отчету %1';uk='Недостатньо правий доступу до звіту %1'"),
					ОтчетМД.Представление()
				);
			Иначе
				Возврат Ложь;
...
Функция ОбработкаПредопределенныхВариантов(ИзмененныеОбъектыДанных = Неопределено) Экспорт
	...
	Исключение	
		...
		ЗаписьЖурналаРегистрации(
			...
			,
			НСТр("ru='В процессе обновления вариантов возникла ошибка:';uk='У процесі відновлення варіантів виникла помилка:'")
			+ Символы.ПС
...
Процедура ДобавитьВДопНастройки(ИмяОтчета, ОписаниеВариантов, ДанныеОПодсистемахВариантов, СписокДопВариантов, ЗначениеНастроекДопВариантов)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			...
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Описания вариантов отчета %1 будут заполнены по умолчанию.';uk='Опису варіантів звіту %1 будуть заповнені за замовчуванням.'"),
...
//
Функция ИмяСобытия()
	Возврат НСтр("ru='Варианты отчетов';uk='Варіанти звітів'");
КонецФункции
