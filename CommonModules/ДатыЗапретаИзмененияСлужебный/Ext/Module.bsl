...
Процедура ПересчитатьТекущиеЗначенияОтносительныхДатЗапрета(ЗаписатьОписаниеРезультатаВЖурналРегистрации = Истина, ОписаниеРезультата = "") Экспорт
	...
	Если НетОтносительныхДат Тогда
		ОписаниеРезультата = НСтр("ru='Относительные даты запрета не заданы.';uk='Відносні дати заборони не задані.'");
		//
...
"При пересчете возникли ошибки:';uk='Перелічені деякі поточні значення відносних дат заборони.  При перерахуванні виникли помилки:'");
		Иначе
			ОписаниеРезультата = НСтр("ru='Не пересчитаны текущие значения дат запрета."
"При пересчете возникли ошибки:';uk='Не перелічені поточні значення дат заборони.  При перерахуванні виникли помилки:'");
		КонецЕсли;
		ОписаниеРезультата = ОписаниеРезультата + ПредставлениеОшибки;
	Иначе
		Если ЕстьОбновленныеДаты Тогда
			ОписаниеРезультата = НСтр("ru='Успешно пересчитаны текущие значения относительных дат запрета.';uk='Успішно перелічені поточні значення відносних дат заборони.'");
		Иначе
			ОписаниеРезультата = НСтр("ru='Сегодня уже пересчитаны текущие значения относительных дат запрета.';uk='Сьогодні вже перелічені поточні значення відносних дат заборони.'");
		КонецЕсли;
	КонецЕсли;
	
	Если ЗаписатьОписаниеРезультатаВЖурналРегистрации Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Даты запрета изменения.Пересчет относительных дат';uk='Дати заборони зміни.Перерахування відносних дат'"),
