...
Процедура ПолучитьПланыОбмена(ПланыОбменаПодсистемы) Экспорт
	...
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменУчетВОСМДБухгалтерия);
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменУчетВОСМДБухгалтерия20);
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ПодомовойУчетЗатрат20);
...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка, Отбор) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ЗакрытиеМесяца.Дата >= &ДатаНач
		|	И ЗакрытиеМесяца.Проведен = ИСТИНА";
		
	Запрос.УстановитьПараметр("ДатаНач", Получатель.ДатаНачалаВыгрузкиДокументов); 		
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		Документы.ЗакрытиеМесяца.СформироватьНачислениеДляБухгалтерии(Выборка.Ссылка);
		Документы.ЗакрытиеМесяца.СформироватьЛьготыСубсидииДляБухгалтерии(Выборка.Ссылка);
	
	КонецЦикла; 
	
	Если ТипЗнч(Получатель) = Тип("ПланОбменаСсылка.ОбменУчетВОСМДБухгалтерия") 
		ИЛИ ТипЗнч(Получатель) = Тип("ПланОбменаСсылка.ОбменУчетВОСМДБухгалтерия20")
		ИЛИ ТипЗнч(Получатель) = Тип("ПланОбменаСсылка.ПодомовойУчетЗатрат20") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ЗначенияРеквизитов = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Получатель, "ИспользоватьОтборПоОрганизациям, ДатаНачалаВыгрузкиДокументов, Организации, ВыгружатьПлатежныеДокументы");
		...
		ОбменДаннымиСобытияОСМД.ЗарегистрироватьДанныеПоДатеНачалаВыгрузкиИОрганизациям(Получатель, ЗначенияРеквизитов, Организации, Отбор);
