...
Функция ТекущийСеансВыполняетРегламентныеЗадания(ЗаданияВыполняютсяНормально = Неопределено,
	...
	Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		ОписаниеОшибки = НСтр("ru='Задания выполняются на сервере.';uk='Завдання виконуються на сервері.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ НайденСеансВыполняющийЗадания Тогда
		ОписаниеОшибки = НСтр("ru='Нет сеанса, выполняющего регламентные задания.';uk='Немає сеансу, що виконує регламентні завдання.'");
...
"    в сеансе номер:        %4.';uk='Выполнение регламентных заданий не начинается уже более 1 часа."
"Возможно, требуется проверка и перезапуск сеанса. Выполнение ожидается"
"    на компьютере:         %1,"
"    в приложении:          %2,"
"    от имени пользователя: %3,"
...
"    в приложении:          %2,"
"    от имени пользователя: %3,"
"    в сеансе номер:        %4.';uk='Выполнение регламентного задания происходит непрерывно более 1 часа."
"Возможно, требуется проверка и перезапуск сеанса. Выполнение происходит"
"    на компьютере:         %1,"
"    в приложении:          %2,"
"    от имени пользователя: %3,"
...
"Возможно, оно удалено другим пользователем.';uk='Завдання не знайдене в списку.  Можливо, воно вилучене іншим користувачем.'") );
	КонецЕсли;
...
Функция ТекстНеОпределено() Экспорт
	...
	Возврат НСтр("ru='<не определено>';uk='<не визначене>'");
...
Функция ПараметрыЗапускаОтдельногоСеансаВыполненияРегламентныхЗаданий(Знач ПоНастройкеАвтоОткрытия = Ложь) Экспорт
	...
	Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Иначе
			...
			Результат.ОписаниеОшибки = НСтр("ru='Регламентные задания выполняются на сервере.';uk='Регламентні завдання виконуються на сервері.'");
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗаданияВыполняютсяНормально Тогда
		...
		Иначе
			Результат.Отказ = Истина;
			Результат.ОписаниеОшибки = НСтр("ru='Сеанс, выполняющий регламентные задания, уже открыт.';uk='Сеанс, що виконує регламентні завдання, уже відкритий.'");
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
...
"в файловой информационной базе.';uk='Фонові завдання не використовуються  у файловій інформаційній базі.'"));
	КонецЕсли;
			...
			Иначе
				Если Свойства.Состояние = СостояниеФоновогоЗадания.Активно Тогда
					...
					Если НЕ НайденСеансВыполняющийЗадания Тогда
						...
						Свойства.ОписаниеИнформацииОбОшибке = НСтр("ru='Не найден сеанс, выполняющий процедуру регламентного задания.';uk='Не знайдений сеанс, що виконує процедуру регламентного завдання.'");
					КонецЕсли;
				КонецЕсли;
...
Функция ВыполнитьРегламентноеЗадание(Знач Задание,
	...
	НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		?(ЗапускВручную,
		  НСтр("ru='Запуск вручную: %1';uk='Запуск вручну: %1'"),
		  НСтр("ru='Автозапуск: %1';uk='Автозапуск: %1'")),
