...
Функция ПроверитьСсылкиНаОбъект(Знач МассивСсылок, Знач СинонимыРеквизитов) Экспорт
	...
	ЗаголовокДиалога = НСтр("ru='Редактирование реквизитов объекта';uk=""Редагування реквізитів об'єкта""");
...
"объекта и оценить последствия изменения этих реквизитов.';uk=""Зміна наступних реквізитів може привести до неузгодженості даних інформаційної бази:  %1.  Настійно рекомендується перевірити всі випадки використання об'єкта й оцінити наслідку зміни цих реквізитів.""");
	Иначе
		ТекстВопроса = НСтр("ru='Изменение реквизита ""%1"""
"может привести к рассогласованию данных информационной базы."
""
"Настоятельно рекомендуется проверить все случаи использования"
"объекта и оценить последствия изменения этого реквизита.';uk='Зміна реквізиту ""%1"" може привести до неузгодженості даних інформаційної бази.  Настійно рекомендується перевірити всі випадки використання об''єкта й оцінити наслідку зміни цього реквізиту.'");
	КонецЕсли;
	
	ТекстВопроса = ТекстВопроса + Символы.ПС + Символы.ПС;
	
	Если МассивСсылок.Количество() = 1 Тогда
		ТекстВопроса = ТекстВопроса + НСтр("ru='Продолжить с проверкой использования объекта?';uk=""Продовжити з перевіркою використання об'єкта?""");
	Иначе
		Если Прав(Строка(МассивСсылок.Количество()), 1) = "1" Тогда
			ШаблонТекстаВопроса = НСтр("ru='Разрешить редактирование реквизитов для %1 объекта?';uk=""Дозволити редагування реквізитів для %1 об'єкта?""");
		Иначе
			ШаблонТекстаВопроса = НСтр("ru='Разрешить редактирование реквизитов для %1 объектов?';uk=""Дозволити редагування реквізитів для %1 об'єктів?""");
		КонецЕсли;
...
""
"Разрешить редактирование реквизитов объекта?';uk="" об'єкт, що редагується, використовується в інформаційній базі.  Дозволити редагування реквізитів об'єкта?""");
...
""
"Редактирование реквизитов объекта разрешено.';uk="" об'єкт, що редагується, не використовується в інформаційній базі.  Редагування реквізитів об'єкта дозволене.""");
...
Процедура ПредупреждениеВсеВидимыеРеквизитыРазблокированы() Экспорт
	...
	Предупреждение(НСтр("ru='Редактирование всех видимых реквизитов объекта уже разрешено.';uk=""Редагування всіх видимих реквізитів об'єкта вже дозволене."""));
