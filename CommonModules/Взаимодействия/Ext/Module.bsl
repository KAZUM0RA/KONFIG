	...
	Для Каждого Элемент Из МассивЗаменить Цикл
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при замене предмета у %1: %2';uk='Помилка при заміні предмета у %1: %2'"),
			                                                            Элемент,
			                                                            ПредставлениеОшибки),
			,
			,
...
Процедура ОбновитьИнформациюОбАктуальностиИндексаППД(ИнформацияОбАктуальностиИндексаППД, ППДВключен, ИндексАктуален) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Запретить Тогда
		ППДВключен = Ложь;
		ИнформацияОбАктуальностиИндексаППД = НСтр("ru='ППД не включен';uk='ППД не включений'");
	Иначе
		ППДВключен = Истина;
		Если ПолнотекстовыйПоиск.ИндексАктуален() Тогда
			ИнформацияОбАктуальностиИндексаППД = НСтр("ru='Индекс актуален';uk='Індекс актуальний'");
			ИндексАктуален = Истина;
		Иначе
			ИнформацияОбАктуальностиИндексаППД = НСтр("ru='Дата актуальности:';uk='Дата актуальності :'")+ 
...
Функция ПолучитьСписокДоступныхПоисков(ППДВключен, Параметры, ЭлементыФормы, ДляАдреснойКниги) Экспорт
	...
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоДомену", НСтр("ru='По доменному имени';uk='По доменному імені'"), ДоменныйАдрес);
...
Функция ПолучитьТекстЗапросаПоискКонтактовПоЦепочкеВзаимодействий(ПомещатьВоВременнуюТаблицу)
	...
	СписокПоиска.Добавить("СообщениеSMS.Адресаты",								"Контакт");
...
Функция ОсуществитьПоискКонтактовПоСтроке(Форма,ДляАдреснойКниги = Ложь) Экспорт
	...
	Возврат ?(КоличествоНайденныхЭлементов < 101, "", НСтр("ru='Уточните параметры поиска. В списке отображены не все найденные контакты!';uk='Уточніть параметри пошуку. У списку відображені не усі знайдені контакти!'"));
...
Процедура СформироватьСписокВыбораТипВзаимодействияТолькоПочта(Элемент) Экспорт
	...
	Элемент.СписокВыбора.Добавить("ВсеПисьма", НСтр("ru='Все письма';uk='Усі листи'"));
	...
	Элемент.СписокВыбора.Добавить("УдаленныеПисьма", НСтр("ru='Удаленные';uk='Видалені'"));
...
Процедура УстановитьРодителяУПапки(Папка, НовыйРодитель, НеЗаписыватьПапку = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("Удаленные", НСтр("ru='Удаленные';uk='Видалені'"));
	...
	Если НЕ НовыйРодитель.Пустая()Тогда
		ЗначенияРеквизитовПапки = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
			...
			Если ЗначенияРеквизитовПапки <> Неопределено И 
				...
				ЗначенияРеквизитовПапки.Наименование = НСтр("ru='Удаленные';uk='Видалені'") Тогда
...
Процедура УстановитьПапкуДляМассиваПисем(МассивПисем,Папка) Экспорт
	...
	Если ЗначенияРеквизитовПапки <> Неопределено 
		...
		И ЗначенияРеквизитовПапки.Наименование = НСтр("ru='Удаленные';uk='Видалені'") Тогда
...
"%ДополнительноеОписание%';uk='При видаленні теки сталася помилка."
"Видалення теки не виконане."
"Додатковий опис:"
...
Функция ОтправкаSMSПоДокументу(Документ) Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выполнены настройки отправки SMS.';uk='Не виконані налаштування відправки SMS.'"),,"Объект");
...
Процедура ОтправкаSMS() Экспорт
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru='Начата регламентная отправка сообщений SMS';uk='Розпочата регламентна відправка повідомлень SMS'"));
		
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
			УровеньЖурналаРегистрации.Ошибка, , ,
			НСтр("ru='Не выполнены настройки отправки SMS.';uk='Не виконані налаштування відправки SMS.'"));
		Возврат;
	КонецЕсли;
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Закончена регламентная отправка сообщений SMS';uk='Закінчена регламентна відправка повідомлень SMS'"));
...
Процедура ОбновлениеСтатусовДоставкиSMS() Экспорт
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Начато регламентное обновление состояний сообщений SMS';uk='Розпочато регламентне оновлення станів повідомлень SMS'"));
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Закончено регламентное обновление состояний сообщений SMS';uk='Закінчено регламентне оновлення станів повідомлень SMS'"));
