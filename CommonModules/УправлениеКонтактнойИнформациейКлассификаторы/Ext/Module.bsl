...
Функция НайтиЗаписиВАКПоИндексу(Индекс) Экспорт
	...
	ИначеЕсли Количество > 1 Тогда
		...
		Пока ВыборкаАдресов.Следующий() Цикл
			...
			НоваяСтрока.Улица    = ?(ВыборкаАдресов.ТипАдресногоЭлемента > 4, ВыборкаАдресов.Улица, НСтр("ru='< Без улицы >';uk='< Без вулиці >'"));
...
Функция ПолучитьИндекс(Знач НазваниеРегиона, Знач НазваниеРайона, Знач НазваниеГорода, Знач НазваниеНаселенногоПункта, 
	...
	ИначеЕсли Улица.Код = 0 И Загружен.Улица И Не ПустаяСтрока(НазваниеУлицы) Тогда
		Возврат "";
	КонецЕсли;
...
Функция АвтоПодборТекстВАдресномЭлементе(Текст, Регион, Район, Город, НаселенныйПункт, УровеньЭлемента,
	...
	Для Каждого СтрокаРезультата Из РезультатЗапроса Цикл
		...
		Если СтруктураРезультата.ПометкаУдаления Тогда
			ТекстПредупреждения = НСтр("ru='""%ПолноеНаименование%"" не актуален.';uk='""%ПолноеНаименование%"" не актуален.'");
...
Функция ПроверитьСоответствиеАдресаКЛАДРу(Знач УказанныйИндекс = "", Знач НазваниеРегиона = "", Знач НазваниеРайона = "", 
	...
	Если Не ПустаяСтрока(НазваниеРегиона) И Регион.Код = 0 Тогда
		ЕстьОшибки = Истина;
		СообщениеОбОшибке = НСтр("ru='Регион ""%НазваниеРегиона%"" не найден в адресном классификаторе.';uk='Регіон ""%НазваниеРегиона%"" не знайдений в адресному класифікаторі.'");
	...
	Если Не ПустаяСтрока(НазваниеРайона) И Район.Код = 0 Тогда
		ЕстьОшибки = Истина;
		СообщениеОбОшибке = НСтр("ru='Район ""%НазваниеРайона%"" не найден в адресном классификаторе.';uk='Район ""%НазваниеРайона%"" не знайдений в адресному класифікаторі.'");
	...
	Если Не ПустаяСтрока(НазваниеГорода) И Город.Код = 0 Тогда
		ЕстьОшибки = Истина;
		СообщениеОбОшибке = НСтр("ru='Город ""%НазваниеГорода%"" не найден в адресном классификаторе.';uk='Місто ""%НазваниеГорода%"" не знайдений в адресному класифікаторі.'");
	...
	Если Не ПустаяСтрока(НазваниеНаселенногоПункта) И НаселенныйПункт.Код = 0 Тогда
		ЕстьОшибки = Истина;
		СообщениеОбОшибке = НСтр("ru='Населенный пункт ""%НазваниеНаселенногоПункта%"" не найден в адресном классификаторе.';uk='Населений пункт ""%НазваниеНаселенногоПункта%"" не знайдений в адресному класифікаторі.'");
	...
	Если Не ПустаяСтрока(НазваниеУлицы) И Улица.Код = 0 Тогда
		ЕстьОшибки = Истина;
		СообщениеОбОшибке = НСтр("ru='Улица ""%НазваниеУлицы%"" не найдена в адресном классификаторе.';uk='Вулиця ""%НазваниеУлицы%"" не знайдена в адресному класифікаторі.'");
	...
	Если Не (ПустаяСтрока(НомерДома) И ПустаяСтрока(НомерКорпуса)) Тогда
		...
		Если КодДома = 0 И ЕстьДома Тогда
			...
			Если ПустаяСтрока(НомерДома) Тогда
				СообщениеОбОшибке = НСтр("ru='Корпус ""%НомерКорпуса%"" не найден в адресном классификаторе.';uk='Корпус ""%НомерКорпуса%"" не знайдений в адресному класифікаторі.'");
			...
			ИначеЕсли ПустаяСтрока(НомерКорпуса) ТОгда
				СообщениеОбОшибке = НСтр("ru='Дом ""%НомерДома%"" не найден в адресном классификаторе.';uk='Будинок ""%НомерДома%"" не знайдений в адресному класифікаторі.'");
	...
	Если Не ПустаяСтрока(УказанныйИндекс) И НомерИндекса = "" Тогда
		...
		Если ИндексНеСуществует Тогда
			СообщениеОбОшибке = НСтр("ru='Индекс ""%УказанныйИндекс%"" не найден в адресном классификаторе.';uk='Індекс ""%УказанныйИндекс%"" не знайдений в адресному класифікаторі.'");
		Иначе
			СообщениеОбОшибке = НСтр("ru='Индекс ""%УказанныйИндекс%"" не соответствует введенному адресу.';uk='Індекс ""%УказанныйИндекс%"" не відповідає введеній адресі.'");
		КонецЕсли;
...
Функция ВходитВИнтервал(Знач Номер, Интервал)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Номер, Ложь) Тогда
		...
		Иначе
			Номер = ОставитьТолькоЦифрыВСтроке(Номер);
		КонецЕсли;
	КонецЕсли;
...
// Функция оставляет в строке только цифры, отбрасывая все буквы и другие символы
//
// Параметры:
