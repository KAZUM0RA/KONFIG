...
Функция РасширениеРаботыСФайламиПодключено(ТекстПредложения = Неопределено, ТекстПредупреждения = Неопределено) Экспорт
	...
	Если Результат = "НеПодключено" Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru='Действие недоступно, так как не подключено расширение работы с файлами в Веб-клиенте.';uk='Дія недоступна, тому що не підключене розширення роботи з файлами в Клієнт^-клієнті-веб-клієнтові.'")
		КонецЕсли;
	ИначеЕсли Результат = "НеподдерживаемыйВебКлиент" Тогда
		ТекстСообщения = НСтр("ru='Действие недоступно в используемом Веб-клиенте, так как для него не может быть подключено расширение работы с файлами.';uk='Дія недоступна у використовуваному Клієнт^-клієнті-веб-клієнтові, тому що для нього не може бути підключене розширення роботи з файлами.'");
	КонецЕсли;
...
Процедура ЗарегистрироватьCOMСоединитель() Экспорт
	...
	Исключение
		...
		ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru='Регистрация компоненты comcntr';uk='Реєстрація компонента comcntr'"), "Ошибка", 
	...
	Если СокрЛП(Стр) <> "0" Тогда
		ТекстСообщения = НСтр("ru='Ошибка при регистрации компоненты comcntr.';uk='Помилка при реєстрації компонента comcntr.'") + Символы.ПС;
		ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru='Регистрация компоненты comcntr';uk='Реєстрація компонента comcntr'"), "Ошибка",ТекстСообщения + 
...
""
"Текст команды: "
"%2';uk='Код помилки regsvr32: %1.  (Код помилки 5 означає, що недостатньо правий доступу. Виконаєте команду від імені користувача із правами адміністратора на локальній машині.)  Текст команди:  %2'"), Стр, ТекстКоманды));
		ОбщегоНазначения.ЗаписатьСобытияВЖурналРегистрации(СообщенияДляЖурналаРегистрации);
...
"Отменить и закрыть?';uk='Дані були змінені, внесені зміни будуть скасовані. Скасувати й закрити?'"),
		ТекстПредупреждения);
...
"Включить сейчас?';uk=' Для продовження рекомендується включити використання Журналу реєстрації.   А якщо ні, то, запис подій у Журнал реєстрації не буде зроблена.  Включити зараз?'");
			Иначе
...
"Включить сейчас?';uk=' Для продовження рекомендується включити реєстрацію подій %1 у Журналі реєстрації.   А якщо ні, то, записі подій у Журнал реєстрації будуть неповними.  Включити зараз?'");
		...
		Заголовок = НСтр("ru='Журнал регистрации';uk='Журнал реєстрації'");
	...
	Если КодВозврата = КодВозвратаДиалога.Да Тогда
		Попытка 
			...
			Текст = НСтр("ru='Настройка изменена';uk='Настроювання змінене'");
			Пояснение = НСтр("ru='Регистрация событий в журнале регистрации обновлена';uk='Реєстрація подій у журналі реєстрації оновлена'");
			ПоказатьОповещениеПользователя(Текст,,Пояснение);
		Исключение

			ИмяСобытия = НСтр("ru='Настройка журнала регистрации';uk='Настроювання журналу реєстрації'");
			ПредставлениеУровня = "Ошибка";
			Комментарий = НСтр("ru='Невозможно установить монопольный доступ. Настройка журнала регистрации не изменена';uk='Неможливо встановити монопольний доступ. Настроювання журналу реєстрації не змінена'");
