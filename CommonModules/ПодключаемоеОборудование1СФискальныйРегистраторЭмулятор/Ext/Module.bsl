...
"подключаемого оборудования в форме ""Подключение и настройка оборудования"".';uk='Не налаштовані параметри пристрою."
...
"обладнання, що підключається у формі ""Підключення та налаштування обладнання"".'"));
	ИначеЕсли ТаблицаСоответствийНалоговыхГрупп = Неопределено Тогда
		Результат = Ложь;
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Не заполнены параметры торгового оборудования(параметры налогообложения).';uk='Не заповнені параметри торгового обладнання (параметри оподаткування).'"));
	КонецЕсли;
...
Функция ПечатьЧека(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Результат = МенеджерОборудованияПереопределяемый.ПечатьЧека(ПодключаемоеОборудование1СФискальныйРегистраторЭмулятор,
...
Функция Инкассация(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТипИнкассации, Сумма, ВыходныеПараметры)
	...
	СообщитьПользователю(?(ТипИнкассации = 1, НСтр("ru='Внесение на сумму';uk='Внесення на суму'"), НСтр("ru='Выемка на сумму';uk='Виїмка на суму'")) + " " + Сумма);
...
Функция НапечататьПериодическийОтчетПоДатам(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	СообщитьПользователю(НСтр("ru='Печать периодического отчета по датам, с даты ';uk='Друк періодичного звіту по датах, з дати'")+Формат(ВходныеПараметры[0], "ДФ=dd.MM.yyyy")+
						 НСтр("ru=' по дату ';uk='по дату'")+Формат(ВходныеПараметры[1], "ДФ=dd.MM.yyyy"));
...
Функция НапечататьПериодическийОтчетПоНомерам(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	СообщитьПользователю(НСтр("ru='Печать периодического отчета по номерам, с номера ';uk='Друк періодичного звіту по номерах, з номера'")+ВходныеПараметры[0]+
						 НСтр("ru=' по номер ';uk=' по номер'")+ВходныеПараметры[1]);
...
Процедура СообщитьПользователю(ТекстСообщения)
	...
	ПоказатьОповещениеПользователя(НСтр("ru='1С: Фискальный регистратор (эмулятор)';uk='1С: Фіскальний реєстратор (емулятор)'"), , ТекстСообщения);
