...
Функция ПодключитьОборудование(ИдентификаторКлиента, ТипыПО = Неопределено,
	...
	Если Не Результат Тогда
		ОписаниеОшибки = НСтр("ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';uk='Заздалегідь необхідно вибрати робоче місце устаткування поточного сеансу, що підключається.'");
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';uk='%Наименование%: Не вдається завантажити драйвер пристрою."
"Перевірте, що драйвер коректно встановлений і зареєстрований в системі.'");
				...
				Результат = МенеджерОборудованияПереопределяемый.ПолучитьОбработчикаДрайвера(НовоеПодключение.ОбработчикДрайвера).ПодключитьУстройство(
					    ОбъектДрайвера,
				...
				Иначе
					...
					ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
					               + НСтр("ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';uk='Не вдалося підключити пристрій ""% Найменування%"":% ОпісаніеОшібкі% (% КодОшібкі%)'");
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , Устройство.Наименование);
...
"Укажите другое устройство.';uk='Вибраний пристрій не може використовуватися для підключення."
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';uk='""%Наименование%"": Чи не вдалося завантажити драйвер пристрою."
"Перевірте, що драйвер коректно встановлений і зареєстрований в системі.'");
		...
		ВыходныеПараметры = Неопределено;
		Результат = МенеджерОборудованияПереопределяемый.ПолучитьОбработчикаДрайвера(ПодключенноеУстройство.ОбработчикДрайвера).ОтключитьУстройство(
			    ОбъектДрайвера,
...
Процедура УстановитьДрайвер(Идентификатор) Экспорт
	...
	Попытка
		...
		ИначеЕсли Строка(ДрайверУстройства) = "СКАНКОД: Терминалы сбора данных" ИЛИ Строка(ДрайверУстройства) = "SCANCODE: Терминалы сбора данных"  Тогда
			#Если ВебКлиент Тогда
		...
		ИначеЕсли Строка(ДрайверУстройства) = "СКАНСИТИ: Терминалы сбора данных" Тогда
			#Если ВебКлиент Тогда
 	   ...
 	   ИначеЕсли Строка(ДрайверУстройства) = "Штрих-М: Электронные весы" Тогда
 			#Если ВебКлиент Тогда
			#Иначе			
		...
		ИначеЕсли Строка(ДрайверУстройства) = "ScaleCAS: Весы c печатью этикеток CL5000J" Тогда
			#Если ВебКлиент Тогда
			#Иначе			
...
"Перед выполнением операции устройство должно быть подключено.';uk='Пристрій не підключено."
"Перед виконанням операції пристрій має бути підключений.'");
		ВыходныеПараметры.Добавить(999);
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';uk='Не вдалося завантажити драйвер пристрою."
"Перевірте, що драйвер коректно встановлений і зареєстрований в системі.'");
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(ТекстСообщения);
		...
		Иначе
			Если ОбъектДрайвераЭтоCOM(ОбъектДрайвера) Тогда
				ОбъектДрайвера = СоздатьОбъектДрайвераДляCOM(ОбъектДрайвера);
			КонецЕсли;
			Результат = МенеджерОборудованияПереопределяемый.ПолучитьОбработчикаДрайвера(ДанныеОборудования.ОбработчикДрайвера).ВыполнитьКоманду(
			    Команда,
			    ВходныеПараметры,
			    ВыходныеПараметры,
			    ОбъектДрайвера,
			...
			Если Не Результат Тогда
				ВыходныеПараметры.Добавить(НСтр("ru='Установлен';uk='Встановлено'"));
			КонецЕсли;
		КонецЕсли;
	Иначе
...
"Перед выполнением операции устройство должно быть отключено.';uk='Пристрій підключений."
"Перед виконанням операції пристрій повинен бути відключене.'");
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстСообщения);
		ВыходныеПараметры.Добавить(НСтр("ru='Установлен';uk='Встановлено'"));
	КонецЕсли;
...
Функция ВыполнитьНастройкуОборудования(Идентификатор, НастройкиИзменены = Ложь,СообщениеОбОшибке = "") Экспорт
	...
	МодульОбработчикаДрайвера = МенеджерОборудованияПереопределяемый.ПолучитьОбработчикаДрайвера(ОбработчикДрайвера);
...
Процедура ВыгрузитьДанныеВТСД(УникальныйИдентификатор, ТаблицаВыгрузкиВТСД) Экспорт
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ИдентификаторУстройства = МенеджерОборудованияКлиент.ВыбратьУстройство("ТерминалСбораДанных",
		    НСтр("ru='Выберите терминал сбора данных';uk='Виберіть термінал збору даних'"), НСтр("ru='Терминал сбора данных не подключен';uk='Термінал збору даних не підключений'"));
...
"%ОписаниеОшибки%"
"Данные в терминал сбора данных не выгружены.';uk='При вивантаженні даних в термінал збору даних сталася помилка."
"%ОписаниеОшибки%"
"Дані в термінал збору даних не вивантажені.'");
...
"%ОписаниеОшибки%"
"Данные в терминал сбора данных не выгружены.';uk='При підключенні пристрою сталася помилка."
"%ОписаниеОшибки%"
"Дані в термінал збору даних не вивантажені.'");
...
"%ОписаниеОшибки%"
"Данные из терминала сбора данных не загружены.';uk='При завантаженні даних з терміналу збору даних сталася помилка."
"%ОписаниеОшибки%"
"Дані з терміналу збору даних не завантажені.'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
			...
			Иначе
				ТекстСообщения = НСтр("ru='При подключении устройства произошла ошибка."
"%ОписаниеОшибки%"
"Данные из терминала сбора данных не загружены.';uk='При підключенні пристрою сталася помилка."
"%ОписаниеОшибки%"
"Дані з терміналу збору даних не завантажені.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ОписаниеОшибки);
...
Функция КодСоответствуетШаблонуМК(ДанныеДорожек, ДанныеШаблона) Экспорт
	...
	Для й = 1 По 3 Цикл
		Если ДанныеШаблона["ДоступностьДорожки"+Строка(й)] Тогда
			...
			Если Прав(текСтрока, СтрДлина(ДанныеШаблона["Суффикс"+Строка(й)])) <> ДанныеШаблона["Суффикс"+Строка(й)] Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дорожка ';uk='Доріжка'")+Строка(й)+". "+НСтр("ru='Суффикс карты не соответствует суффиксу шаблона.';uk='Суфікс карти не відповідає суффиксу шаблону.'"));
				ПроверкаПройдена = Ложь;
			КонецЕсли;
			
			Если Лев(текСтрока, СтрДлина(ДанныеШаблона["Префикс"+Строка(й)])) <> ДанныеШаблона["Префикс"+Строка(й)] Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дорожка ';uk='Доріжка'")+Строка(й)+". "+НСтр("ru='Префикс карты не соответствует префиксу шаблона.';uk='Префікс карти не відповідає префіксу шаблону.'"));
				ПроверкаПройдена = Ложь;
			КонецЕсли;
			
			Если Найти(текСтрока, ДанныеШаблона["РазделительБлоков"+Строка(й)]) = 0 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дорожка ';uk='Доріжка'")+Строка(й)+". "+НСтр("ru='Разделитель блоков карты не соответствует разделителю блоков шаблона.';uk='Роздільник блоків карти не відповідає разделителю блоків шаблону.'"));
				ПроверкаПройдена = Ложь;
			КонецЕсли;
				
			Если СтрДлина(текСтрока) <> ДанныеШаблона["ДлинаКода"+Строка(й)] Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дорожка ';uk='Доріжка'")+Строка(й)+". "+НСтр("ru='Длина кода карты не соответствует длине кода шаблона.';uk='Довжина коду карти не відповідає довжині коду шаблону.'"));
				ПроверкаПройдена = Ложь;
			КонецЕсли;
	...
	Если ЕстьХотьОднадорожка Тогда 
		Возврат Истина;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В шаблоне не указано ни одной доступной дорожки.';uk='У шаблоні не вказано жодної доступною доріжки.'"));
		Возврат Ложь;
	КонецЕсли;
КонецФункции
...
Функция ПолучитьОбъектДрайвера(ДрайверУстройства) Экспорт
	...
	Если ОбъектДрайвера = Неопределено Тогда
		Попытка
			...
			ИначеЕсли Строка(ДрайверУстройства) = "ОРИОН: Фискальный регистратор ФР01К" Тогда
				#Если ВебКлиент Тогда
				#Иначе
			...
			ИначеЕсли Строка(ДрайверУстройства) = "АТОЛ: Рабочее место кассира" Тогда
				ОбъектДрайвера = ""; // Драйвер не требуется
				
			ИначеЕсли Строка(ДрайверУстройства) = "Штрих-М: Кассир" Тогда
				ОбъектДрайвера = ""; // Драйвер не требуется
				
			ИначеЕсли Строка(ДрайверУстройства) = "Версия-Т: Касби-04К" Тогда
				#Если ВебКлиент Тогда
					
				#Иначе
					ProgID = "AddIn.Kasbi04KOff";
					ПодключитьВнешнююКомпоненту(ProgID);
					ОбъектДрайвера = Новый (ProgID);
				#КонецЕсли
			...
			ИначеЕсли Строка(ДрайверУстройства) = "Уэб Текнолоджи: WAB-08RK" Тогда
				#Если ВебКлиент Тогда
					
				#Иначе
					ProgID = "AddIn.Wab08RK";
					ПодключитьВнешнююКомпоненту(ProgID);
					ОбъектДрайвера = Новый (ProgID);
				#КонецЕсли
...
Процедура ПодготовкаКПереустановкеКомпоненты(ПодключаемоеОборудованиеСсылка, ВерсияТекущая, Отказ) Экспорт
	...
	#Если ВебКлиент Тогда
		ВерсияИзКонфигурации = МенеджерОборудованияПереопределяемый.ПолучитьОбработчикаДрайвера(МенеджерОборудованияСервер.ПолучитьОбработчикДрайвераИзСсылки(ПодключаемоеОборудованиеСсылка)).ПолучитьВерсиюВнешнейКомпоненты();
		ТреубетсяПереустановка = МенеджерОборудованияСервер.ПолучитьПризнакПереустановкиИзСсылки(ПодключаемоеОборудованиеСсылка);
		Если НЕ ТреубетсяПереустановка
			И ЗначениеЗаполнено(ВерсияИзКонфигурации)
			И ЗначениеЗаполнено(ВерсияТекущая)
			И ВерсияИзКонфигурации <> ВерсияТекущая Тогда
			Ответ = Вопрос(НСтр("ru='На вашем компьютере установлена компонента версии ';uk=""На вашому комп'ютері встановлена компонента версії""")+ВерсияТекущая+Символы.ПС
				+НСтр("ru='В конфигурации доступна версия компоненты ';uk='У конфігурації доступна версія компоненти'")+ВерсияИзКонфигурации+Символы.ПС+Символы.ПС
				+НСтр("ru='Обновить компоненту из конфигурации? (требуется перезапустить 1С:Предприятие)';uk='Оновити компоненту з конфігурації? (потрібно перезапустити 1С: Підприємство)'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да, НСтр("ru='Обновление компоненты';uk='Оновлення компоненти'"));
