...
Процедура УстановитьБлокировкуРегламентныхЗаданий(Значение) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для выполнения операции';uk='Недостатньо правий для виконання операції'");
	КонецЕсли;	
	УстановитьПривилегированныйРежим(Истина);
...
Процедура ПроверитьБлокировкуСоединенийОбластиДанных(Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущийРежим.Конец) Тогда
		ПериодБлокировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='на период с %1 по %2';uk='на період з %1 по %2'"),
			ТекущийРежим.Начало, ТекущийРежим.Конец);
	Иначе
		ПериодБлокировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='с %1';uk='с %1'"), ТекущийРежим.Начало);
	КонецЕсли;
...
""
"Приложение временно недоступно.';uk='Адміністратором додатка встановлене блокування роботи користувачів %1 %2.  Додаток тимчасово недоступний.'"),
	...
	Если (Пользователи.ЭтоПолноправныйПользователь() И Не ТекущийРежим.Эксклюзивная) 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Администратором приложения установлена блокировка работы пользователей %1 %2."
""
"Войти в заблокированное приложение?';uk='Адміністратором додатка встановлене блокування роботи користувачів %1 %2.  Увійти в заблокований додаток?'"),
			ПериодБлокировки, ПричинаБлокировки);
	КонецЕсли;
...
"http://<веб-адрес сервера>/?C=РазрешитьРаботуПользователей';uk='%1"
"Для дозволу роботи користувачів можна відкрити додаток з параметром РазрешитьРаботуПользователей. Наприклад: http://<веб-адреса сервера>/?C=РазрешитьРаботуПользователей'");
	Иначе
		ТекстСообщения = ТекстСообщения +
		    НСтр("ru='%1"
...
"ENTERPRISE %2 /CРазрешитьРаботуПользователей /UC%3';uk='%1  "
"Для того щоб дозволити роботу користувачів, скористайтеся консоллю кластера серверів або запустите ""1С:Підприємство"" з параметрами: ENTERPRISE %2/CРазрешитьРаботуПользователей /UC%3'");
	КонецЕсли;
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,
		СоединенияИБКлиентСервер.ТекстДляАдминистратора(), СтрокаПутиКИнформационнойБазе, 
		НСтр("ru='<код разрешения>';uk='<код дозволу>'"));
...
Функция ТекстНеУдалосьЗавершитьРаботуПользователей(Знач Сообщение) 
	...
	Если Не ПустаяСтрока(Сообщение) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось завершить работу пользователей (%1):';uk='Не вдалося завершити роботу користувачів (%1):'"),
			Сообщение);
	Иначе		
		ТекстСообщения = НСтр("ru='Не удалось завершить работу пользователей:';uk='Не вдалося завершити роботу користувачів:'");
	КонецЕсли;
	Возврат ТекстСообщения;
