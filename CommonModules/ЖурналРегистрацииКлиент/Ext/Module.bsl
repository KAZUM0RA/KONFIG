...
Процедура ОткрытьДанныеДляПросмотра(ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.Данные = Неопределено Тогда
		Предупреждение(НСтр("ru='Эта запись журнала регистрации не связана с данными (см. колонку ""Данные"")';uk='Цей запис журналу реєстрації не пов''язана з даними (див. колонкові ""Дані"")'"));
		Возврат;
	КонецЕсли;
...
"%1';uk=""Цей запис журналу реєстрації пов'язана з даними, але відобразити їх неможливо. %1""");
		Если ТекущиеДанные.Событие = "_$Data$_.Delete" Тогда 
			// это - событие удаления
			ТекстПредупреждения =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ТекстПредупреждения,
						НСтр("ru='Данные удалены из информационной базы';uk='Дані вилучені з інформаційної бази'"));
		Иначе
			ТекстПредупреждения =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ТекстПредупреждения,
						НСтр("ru='Возможно, данные удалены из информационной базы';uk=' Можливо, дані вилучені з інформаційної бази'"));
		КонецЕсли;
