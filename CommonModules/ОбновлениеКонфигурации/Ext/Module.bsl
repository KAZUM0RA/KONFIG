...
Процедура ЗавершитьОбновление(Знач РезультатОбновления, Знач ЭлектроннаяПочта, Знач ИмяАдминистратораОбновления) Экспорт
	...
	ТекстСообщения = НСтр("ru='Завершение обновления из внешнего скрипта.';uk='Завершення відновлення із зовнішнього скрипту.'");
	...
	Если НЕ ПроверитьДоступДляОбновления() Тогда
		ТекстСообщения = НСтр("ru='Недостаточно прав для завершения обновления конфигурации.';uk='Недостатньо правий для завершення відновлення конфігурації.'");
	...
	Если НЕ ПустаяСтрока(ЭлектроннаяПочта) Тогда
		Попытка
			...
			ТекстСообщения = НСтр("ru='Уведомление об обновлении успешно отправлено на адрес электронной почты: ';uk='Повідомлення про відновлення успішно відправлене на адресу електронної пошти: '") + ЭлектроннаяПочта;
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,,ТекстСообщения);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка при отправке письма электронной почты:';uk='Помилка при відправленні листа електронної пошти:'") + ЭлектроннаяПочта + Символы.ПС + 
...
Процедура ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, Знач АдресНазначения, Знач УспешноеОбновление)
	...
	Тема = ? (УспешноеОбновление, НСтр("ru='Успешное обновление конфигурации ""%1"", версия %2';uk='Успішне відновлення конфігурації ""%1"", версія %2'"), 
		НСтр("ru='Ошибка обновления конфигурации ""%1"", версия %2';uk='Помилка відновлення конфігурації ""%1"", версія %2'"));
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Тема, Метаданные.КраткаяИнформация, Метаданные.Версия);
		
	Подробности = ?(УспешноеОбновление, НСтр("ru='Обновление конфигурации завершено успешно.';uk='Відновлення конфігурації завершене успішно.'"), 
		НСтр("ru='При обновлении конфигурации произошли ошибки. Подробности записаны в журнал регистрации.';uk=' При відновленні конфігурації відбулися помилки. Подробиці записані в журнал реєстрації.'"));
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1"
""
"Конфигурация: %2"
"Версия: %3"
"Строка соединения: %4';uk=""%1 "
""
"Конфігурація: %2 "
"Версія: %3 "
"Рядок з'єднання: %4"""),
