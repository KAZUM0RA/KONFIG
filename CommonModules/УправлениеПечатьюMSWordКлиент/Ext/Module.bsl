...
#КонецЕсли
	...
	Исключение
		...
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
		ВызватьИсключение(НСтр("ru='Ошибка при открытии файла шаблона.';uk='Помилка при відкритті файлу шаблону.'") + Символы.ПС 
...
#Если ВебКлиент Тогда
	ТекстУточнения = НСтр("ru='При работе через веб требуется браузер Internet Explorer под управлением операционной системы Windows. См. также главу документации ""Настройка веб-браузеров для работы в веб-клиенте""';uk=' При роботі через веб потрібен браузер Internet Explorer під керуванням операційної системи Windows. Див. також главу документації ""Настроювання веб-браузерів для роботи у веб-клієнтові""'");
#Иначе		
	ТекстУточнения = "";	
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось сформировать печатную форму: %1. "
"Для вывода печатных форм в формате Microsoft Word требуется, чтобы на компьютере был установлен пакет Microsoft Office. %2';uk=""Не вдалося сформувати друковану форму: %1.   Для виводу друкованих форм у форматі Microsoft Word потрібно, щоб на комп'ютері був установлений пакет Microsoft Office. % 2"""),
...
Функция Заменить(Object, Знач СтрокаПоиска, Знач СтрокаЗамены)
	...
	FindObject.Text = СтрокаПоиска;
	FindObject.Replacement.ClearFormatting();
	FindObject.Replacement.Text = СтрокаЗамены;
	
	Возврат FindObject.Execute(,,,,,,,,,,wdReplaceAll);
