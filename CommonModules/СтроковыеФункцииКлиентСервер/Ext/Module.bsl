...
//
// Пример:
//  ПодставитьПараметрыВСтроку(НСтр("ru='%1 пошел в %2';uk='%1 пішов в %2'"), "Вася", "Зоопарк") = "Вася пошел в Зоопарк".
//
...
//
//  Строка = ПодставитьПараметрыВСтроку(НСтр("ru='%1 пошел в %2';uk='%1 пішов в %2'"), МассивПараметров);
//
...
// Функция создает СМС сообщение согласно переданному шаблону
Функция СформироватьТекстSMSСообщения(МассивЛС, ШаблонСообщения,АдресСайта = Неопределено) Экспорт
	
	МассивТекстов = Новый Массив;
	Индекс = 0;
	Пока Индекс <= МассивЛС.ВГраница() Цикл
		Строка = МассивЛС[Индекс];
		Текст = ШаблонСообщения;
		Если Найти(Текст,"[ЛицевойСчет]") Тогда
			Текст = СтрЗаменить(Текст,"[ЛицевойСчет]",Строка.ЛицевойСчет);
		КонецЕсли;
		Если Найти(Текст,"[Владелец]") Тогда
		    Текст = СтрЗаменить(Текст,"[Владелец]",Строка.ОтветственныйВладелец);
		КонецЕсли;
		Если Найти(Текст,"[ОбъектЛС]") Тогда
			Текст = СтрЗаменить(Текст,"[ОбъектЛС]",Строка.ОбъектЛС);
			Текст = СтрЗаменить(Текст,"№ ","");
		КонецЕсли;
		Если Найти(Текст,"[Здание]") Тогда
			Текст = СтрЗаменить(Текст,"[Здание]",Строка.Здание);
		КонецЕсли;
		Если Найти(Текст,"[Долг]") Тогда
			Текст = СтрЗаменить(Текст,"[Долг]",формат(Строка.Долг,"ЧГ="));
		КонецЕсли;
		Если Найти(Текст,"[Дата]") Тогда
			Текст = СтрЗаменить(Текст,"[Дата]",Формат(Строка.Дата,"ДФ=dd.MM.yyyy"));
		КонецЕсли;
		Если Найти(Текст,"[Логин]") Тогда
			Текст = СтрЗаменить(Текст,"[Логин]",Строка.ОтветственныйВладелец.Логин);
		КонецЕсли;
		Если Найти(Текст,"[Пароль]") Тогда
			Текст = СтрЗаменить(Текст,"[Пароль]",Строка.ОтветственныйВладелец.Пароль);
		КонецЕсли;
		Если Найти(Текст,"[АдресСайта]") Тогда
			Текст = СтрЗаменить(Текст,"[АдресСайта]",АдресСайта);
		КонецЕсли;
