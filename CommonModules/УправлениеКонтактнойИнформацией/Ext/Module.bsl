...
Процедура ПриСозданииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения) Экспорт
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("ТолькоУкраинский", Новый ОписаниеТипов("Булево"),                                      ИмяОписания));
	...
	|	ВидыКонтактнойИнформации.АдресТолькоУкраинский,
	...
	Для Каждого СтрокаКИ Из КонтактнаяИнформация Цикл
		...
		НоваяСтрока.ТолькоУкраинский = СтрокаКИ.АдресТолькоУкраинский;
		НоваяСтрока.Вид              = Вид;
...
Функция РазложитьСтрокуНаПодстроки(ВходящаяСтрока, Разделитель) Экспорт
	...
	Строки = СтрЗаменить(ВходящаяСтрока, Разделитель, Символы.ПС);
	
	Для ИндексСтроки = 1 По СтрЧислоСтрок(Строки) Цикл
		Подстрока = СтрПолучитьСтроку(Строки, ИндексСтроки);
...
//	АдресТолькоУкраинский - Булево - Истина, если адрес только украинский, Ложь - Иначе.
...
Функция ПолучитьЗначениеКонтактнойИнформации(СтрокаЗначенийПолей, ИмяПоля) Экспорт
	...
	Если ПозицияПоля <> 0 Тогда
		ЗначенияПолей = Прав(СтрокаЗначенийПолей, СтрДлина(СтрокаЗначенийПолей) - ПозицияПоля - СтрДлина(ИмяПоля));
		ПозицияПС   = Найти(ЗначенияПолей, Символы.ПС);
		Значение    = Сред(ЗначенияПолей.Значение, 0 , ПозицияПС - 1);
	КонецЕсли;
