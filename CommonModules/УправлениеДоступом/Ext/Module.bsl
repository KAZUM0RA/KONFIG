...
Функция ЕстьПравоПоЗначениюДоступа(Знач Право, Знач ЗначениеДоступа, Знач Пользователь = Неопределено) Экспорт
	...
	Если ОписаниеПрава = Неопределено Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найдено право ""%1"" таблицы ""%2""';uk='Не знайдене право ""%1"" таблиці ""%2""'"), Право, ЗначениеДоступа.Метаданные().ПолноеИмя()));
	КонецЕсли;
...
Функция РазрешеноУправлениеПравамиПоЗначениямДоступа(Знач СсылкаНаОбъект, ОписаниеОшибки, ТолькоПросмотр = Неопределено) Экспорт
	...
	Если НЕ ЕстьПравоПоЗначениюДоступа("УправлениеПравами", СсылкаНаОбъект) Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 ""%2"" недоступна для управления правами.';uk='%1 ""%2"" недоступна для керування правами.'"), СсылкаНаОбъект.Метаданные().ПредставлениеОбъекта, СсылкаНаОбъект);
...
"""Записать зависимые наборы значений доступа"".';uk='Невірні параметри. Тип об''єкта ""%1"" не знайдений у жодній з передплат на події ""Записати набори значень доступу"", ""Записати залежні набори значень доступу"".'"),
				ТипЗначенияОбъект));
	КонецЕсли;
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ВызватьИсключение(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект ""%1"" сформировал пустой набор значений доступа.';uk='Об''єкт ""%1"" сформував порожній набір значень доступу.'"),
				ТипЗначенияОбъект));
	КонецЕсли;
...
"не задан тип ""%1"" для значения ""%2""';uk='У вимірі ""ЗначениеДоступа"" регістру відомостей ""Набори значень доступу"" не заданий тип ""%1"" для значення ""%2""'"),
...
"""Записать наборы значений доступа"".';uk='Невірні параметри. Тип об''єкта ""%1"" не знайдений у передплаті на події ""Записати набори значень доступу"".'"),
				ТипЗначенияОбъект));
	КонецЕсли;
...
"не задан тип %1';uk=""Помилка при записі наборів значень доступу: у регістрі відомостей НаборыЗначенийДоступа у вимірі Об'єкт не заданий тип %1"""),
...
"с другим типом объектов информационной базы ""%3"".';uk='Зовнішній користувач ""%1"" використовується в групі доступу ""%2"" з іншим типом об''єктів інформаційної бази ""%3"".'"),
					Строка(СсылкаНаОбъект),
...
"используется в группе доступа ""%2"""
"с другим типом объектов информационной базы ""%3"".';uk='Група зовнішніх користувачів ""%1"" використовується в групі доступу ""%2"" з іншим типом об''єктів інформаційної бази ""%3"".'"),
					Строка(СсылкаНаОбъект),
					Выборка.СсылкаПредставление,
					ПредставлениеДругогоТипаОбъектаАвторизации);
			КонецЕсли;
		...
		Иначе
			...
			Если ОграничиватьДоступНаУровнеЗаписей() Тогда
				...
				Если НаборЗаписей.Количество() = 0 Тогда
					...
					ВызватьИсключение(НСтр("ru='Попытка записать пустой набор групп доступа или видов доступа.';uk='Спроба записати порожній набір груп доступу або видів доступу.'"));
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбновитьРолиПользователей(Знач МассивПользователей) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ЗаписьЖурналаРегистрации(НСтр("ru='Управление доступом.Роль не найдена в метаданных';uk='Керування доступом.Роль не знайдена в метаданих'"),
		                         УровеньЖурналаРегистрации.Ошибка,
		                         ,
		                         ,
		                         СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		                              НСтр("ru='При обновлении ролей пользователя ""<%1>"" роль ""<%2>"" профиля групп доступа ""<%3>"" не найдена в метаданных.';uk=' При відновленні ролей користувача ""<%1>"" роль ""<%2>"" профілю груп доступу ""<%3>"" не знайдена в метаданих.'"),
...
"Тип объекта ""%1"""
"не найден в подписке на события"
"""Записать зависимые наборы значений доступа"".';uk='Невірні параметри. Тип об''єкта ""%1"" не знайдений у передплаті на події ""Записати залежні набори значень доступу"".'"),
				ТипЗначенияОбъект));
	КонецЕсли;
...
Процедура ЗаполнениеДанныхДляОграниченияДоступа(КоличествоДанных = 0) Экспорт
	...
	Если КоличествоДанных < 1000 Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Управление доступом.Заполнение данных для ограничения доступа';uk='Керування доступом.Заповнення даних для обмеження доступу'"), 
			УровеньЖурналаРегистрации.Информация, , , НСтр("ru='Завершено заполнение данных для ограничения доступа';uk='Завершене заповнення даних для обмеження доступу'"), 
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
	...
	Иначе
		ЗаписьЖурналаРегистрации(НСтр("ru='Управление доступом.Заполнение данных для ограничения доступа';uk='Керування доступом.Заповнення даних для обмеження доступу'"),
			УровеньЖурналаРегистрации.Информация, , , НСтр("ru='Выполнена запись порции данных для ограничения доступа';uk='Виконаний запис порції даних для обмеження доступу'"), 
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
...
Процедура ПриИзмененииОграниченияДоступаНаУровнеЗаписей(ОграничениеДоступаНаУровнеЗаписейВключено) Экспорт
	...
	Если ОграничениеДоступаНаУровнеЗаписейВключено Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Управление доступом.Заполнение данных для ограничения доступа';uk='Керування доступом.Заповнення даних для обмеження доступу'"), 
			УровеньЖурналаРегистрации.Информация, , , НСтр("ru='Начато заполнение данных для ограничения доступа';uk='Почате заповнення даних для обмеження доступу'"), 
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
...
// например, НСтр("ru='Права доступа: %1 (%2)';uk='Права доступу: %1 (%2)'" -> "Права доступа: Иванов (Пользователь)".
//
// Параметры:
...
"""%1"" возникла ошибка:"
""
"%2';uk=' При відновленні залежного набору значень доступу об''єкта ""%1"" виникла помилка:  %2'"),
