...
"установлен для нескольких внешних пользователей.';uk='Помилка в базі даних: Об''єкт авторизації ""%1"" (%2) установлений для декількох зовнішніх користувачів.'"),
				ОбъектАвторизации,
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в базе данных:"
"Для внешнего пользователя ""%1"" не задан объект авторизации.';uk='Помилка в базі даних:  Для зовнішнього користувача ""%1"" не заданий об''єкт авторизації.'"),
			ВнешнийПользователь);
	КонецЕсли;
...
Процедура ОбновитьРолиВнешнихПользователей(Знач МассивВнешнихПользователей) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При обновлении ролей внешнего пользователя ""<%1>"" роль ""<%2>"" группы внешних пользователей ""<%3>"" не найдена в метаданных.';uk=' При відновленні ролей зовнішнього користувача ""<%1>"" роль ""<%2>"" групи зовнішніх користувачів ""<%3>"" не знайдена в метаданих.'"),
			СокрЛП(Выборка.ВнешнийПользователь.Наименование),
			Выборка.Роль,
			Строка(Выборка.ГруппаВнешнихПользователей));
		ЗаписьЖурналаРегистрации(НСтр("ru='Внешние пользователи.Роль не найдена в метаданных';uk='Зовнішні користувачі.Роль не знайдена в метаданих'"),
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(Выборка.ИдентификаторПользователяИБ) И
			...
			Попытка
				...
				ВнешнийПользовательОбъект.Записать();
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru='Внешние пользователи.Ошибка при обновлении информационной базы';uk='Зовнішні користувачі.Помилка при відновленні інформаційної бази'"),
...
"%3';uk=' При створенні користувача інформаційної бази з іменем ""%1"" для зовнішнього користувача ""%2"" відбулася помилка: %3'"),
				                              Выборка.Код,
