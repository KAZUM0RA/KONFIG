...
Функция ОтключитьСеанс(НомерСеанса, СообщениеОбОшибке) Экспорт
	...
	Если ИнформационнаяБазаФайловая() Тогда
		Сообщение = НСтр("ru='Невозможно принудительно завершить сеанс в файловом режиме работы';uk='Неможливо примусово завершити сеанс у файловому режимі роботи'");
...
"Технические подробности об ошибке см. в Журнале регистрации.';uk='Невозможно принудительно завершить сеанс"
"по причине:"
"%1"
""
"Возможно не настроены параметры администрирования ИБ."
...
Функция ОтключитьСоединенияИБ(Знач ПараметрыАдминистрированияИБ = Неопределено) Экспорт
	...
	// Невозможно принудительно отсоединить сеансы в файловом режиме работы
	Если ИнформационнаяБазаФайловая() Тогда
 		СоединенияИБ.ЗаписатьНазванияСоединенийИБ(НСтр("ru='Невозможно принудительно завершить сеансы в файловом режиме работы';uk='Неможливо примусово завершити сеанси у файловому режимі роботи'"));
		Возврат Ложь; 
	КонецЕсли;
...
" если на сервере не установлена ОС Microsoft Windows';uk='Неможливо примусово завершити сеанси при роботі із клієнта через веб-сервер,  якщо на сервері не встановлена ОС Microsoft Windows'"));
		Возврат Ложь; 
	КонецЕсли;
#КонецЕсли		
	...
	Попытка
		...
		Для каждого Сеанс Из Сеансы.Сеансы Цикл
			...
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			    НСтр("ru='Завершается сеанс: %1, компьютер %2, начат %3, режим %4';uk=""Завершується сеанс: %1, комп'ютер %2, початий %3, режим %4"""),
			    Сеанс.UserName,
			    Сеанс.Host,
			    Сеанс.StartedAt,
				Сеанс.AppID);
...
Процедура ПроверитьПараметрыАдминистрированияИБ(ПараметрыАдминистрированияИБ,
	...
	Попытка
		Если ИнформационнаяБазаФайловая() Тогда
			ВызватьИсключение НСтр("ru='Невозможно получить список активных соединений в файловом режиме работы';uk=""Неможливо одержати список активних з'єднань у файловому режимі роботи""");
		КонецЕсли;
...
#Если НаКлиенте Тогда		
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда
		...
		ВызватьИсключение НСтр("ru='Невозможно получить список активных соединений при работе через веб-сервер';uk=""Неможливо одержати список активних з'єднань при роботі через веб-сервер""");
	КонецЕсли;
#КонецЕсли		
		
		Сеансы = ПолучитьАктивныеСеансыИБ(Истина, ПараметрыАдминистрированияИБ);
	Исключение
		Сообщение = НСтр("ru='Не удалось подключиться к кластеру серверов.';uk='Не вдалося підключитися до кластера серверів.'");
...
Процедура УстановитьБлокировкуРегламентныхЗаданий(Значение) Экспорт
	...
	Если ИнформационнаяБазаФайловая() Тогда
 		ВызватьИсключение НСтр("ru='Невозможно установить блокировку регламентных и фоновых заданий в файловом режиме работы';uk='Неможливо встановити блокування регламентних і фонових завдань у файловому режимі роботи'");
	КонецЕсли;
...
" если на сервере не установлена ОС Microsoft Windows';uk='Неможливо встановити блокування регламентних і фонових завдань при роботі із клієнта через веб-сервер,  якщо на сервері не встановлена ОС Microsoft Windows'");
	КонецЕсли;
#КонецЕсли		
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru='Завершение работы пользователей';uk='Завершення роботи користувачів'");
...
Функция ПодключитьсяКРабочемуПроцессу(ПараметрыСоединенияССервером1СПредприятие)
	...
	Для каждого Кластер Из Подключение.АгентСервера.GetClusters() Цикл
		...
		Если ПортРабочегоПроцесса = -1 Тогда
	 		ВызватьИсключение НСтр("ru='Нет ни одного активного рабочего процесса';uk='Немає жодного активного робочого процесу'");
		КонецЕсли;
		Прервать;
...
Функция ПодключитьсяКТекущейИБ()
	...
	Если ТекущаяБаза = Неопределено Тогда
 		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='База ""%1"" не зарегистрирована в кластере сервером';uk='База ""%1"" не зареєстрована в кластері сервером'"), Подключение.ИмяИБ);
	КонецЕсли;
			...
			Для Каждого Сеанс из СписокСеансов Цикл
				Если ВРег(Сеанс.InfoBase.Name) = ВРег(Подключение.ИмяИБ) Тогда
					Если Сеанс.SessionID = НомерСеансаДляЗавершения Тогда
						...
						Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Разорван сеанс: Пользователь %1, компьютер %2, начат %3, режим %4';uk=""Розірваний сеанс: Користувач %1, комп'ютер %2, початий %3, режим %4"""),
								Сеанс.UserName,
								Сеанс.Host,
								Сеанс.StartedAt,
								Сеанс.AppID);
