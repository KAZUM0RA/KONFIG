...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	Если НЕ ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Запуск по требованию';uk='Розсилка звітів. Запуск на вимогу'"));
	КонецЕсли;
	...
	Если НЕ Рассылка.Подготовлена
		...
		Если Рассылка.ПометкаУдаления Тогда
			Причина = Причина + Символы.ПС + НСтр("ru='Рассылка помечена на удаление';uk='Розсилка помічена на видалення'");
		КонецЕсли;
	...
	КонецЕсли;
	Если ФорматыПоУмолчанию.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru='Не установлены форматы по умолчанию.';uk='Не встановлені формати за умовчанням.'");
	КонецЕсли;
	...
	Если НЕ ПараметрыДоставки.ИспользоватьПапку
		...
		Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ru='Не выбран способ доставки.';uk='Не вибраний спосіб доставки.'")
		);
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	ЗаписьЖурнала(ПараметрыЖурнала, ,
		НСтр("ru='Инициализирована рассылка %1, автор: %2';uk='Ініціалізувала розсилка %1, автор: %2'"), , 
		"'"+ Строка(Рассылка) +"'", 
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		...
		Если СтрокаОтчет.Настройки = Неопределено Тогда
			ТекстЖурнала = ТекстЖурнала + Символы.ПС + НСтр("ru='(пользовательские настройки не заданы)';uk='(призначені для користувача налаштування не задані)'");
		КонецЕсли;
	...
	Если ДеревоОтчетов.Строки.Найти(3, "Уровень", Истина) = Неопределено Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ru='Не сохранено ни одного отчета';uk='Не збережено жодного звіту'"), ,
		);
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ru='Ошибка отправки отчетов получателю %1:';uk='Помилка відправки звітів одержувачеві  %1:'"), ИнформацияОбОшибке(), 
...
"Выполнено: %3"
"С ошибками: %4"
"Не выполнено: %5';uk='Підготовлено розсилок: %1 из %2"
"Виконано: %3"
"З помилками: %4"
"Не виконано: %5'"), 
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	ПараметрыЖурнала.ИмяСобытия = НСтр("ru='Рассылка отчетов. Запуск по расписанию';uk='Розсилка звітів. Запуск за розкладом'");
...
" - Недостаточно прав доступа к справочнику';uk='Не вдалося сформувати список одержувачів %1"
"розсилки %2 по одній з можливих причин :"
"- У одержувачів не заповнена поштова адреса %3 "
"- Не заповнений список одержувачів"
"- Вибрані порожні групи одержувачів"
"- Одержувачі помічені на видалення"
"- Недостатньо прав доступу до довідника'");
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Если ПараметрыОтчета.ЭтоВариант Тогда
		...
		Если ПараметрыОтчета.Метаданные = Неопределено Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru='Для варианта %1 отчет %2 не найден в системе';uk='Для варіанту  %1 звіт %2  не знайдений в системі'"), ,
	...
	Иначе
		...
		ЗаголовокСообщенияОбОшибке = НСтр("ru='Ошибка подключения дополнительного отчета %1:';uk='Помилка підключення додаткового звіту %1:'");
		...
		Если Результат <> Истина Тогда
			Если Результат = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					ЗаголовокСообщенияОбОшибке, НСтр("ru=""Подсистема 'Дополнительные отчеты и обработки' недоступна."";uk=""Підсистема 'Додаткові звіти і обробки' недоступна."""),
					"'"+ Строка(ПараметрыОтчета.Отчет) +"'"
				);
			КонецЕсли;
			Возврат ПараметрыОтчета.Инициализирован;
		...
		Если ТипЗнч(СведенияОВнешнейОбработке) <> Тип("Структура") Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ЗаголовокСообщенияОбОшибке, НСтр("ru='Неверно заполнены сведения о внешней обработке.';uk='Невірно заповнені відомості про зовнішню обробку.'"),
				"'"+ Строка(ПараметрыОтчета.Отчет) +"'"
			);
			Возврат ПараметрыОтчета.Инициализирован;
		КонецЕсли;
		...
		Если НЕ СведенияОВнешнейОбработке.Свойство("РассылкаОтчетов") ИЛИ СведенияОВнешнейОбработке.РассылкаОтчетов <> Истина Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ЗаголовокСообщенияОбОшибке, НСтр("ru='Не указаны сведения о внешней обработке, необходимые для рассылки.';uk='Не вказані відомості про зовнішню обробку, необхідні для розсилки.'"),
				"'"+ Строка(ПараметрыОтчета.Отчет) +"'"
			);
			Возврат ПараметрыОтчета.Инициализирован;
		КонецЕсли;
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено) Экспорт
	...
	Если ПараметрыОтчета.СКД Тогда
		...
		Если НЕ ОтчетОбъект.ПроверитьЗаполнение() Тогда
			...
			КонецЦикла;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru=""Отчет '%1' не прошел проверку заполнения:"";uk=""Звіт '%1' не пройшов перевірку заповнення:"""), СокрЛ(ТекстИсключения),
				Строка(ПараметрыОтчета.Отчет)
			);
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить Тогда
			ШаблонСообщенияОбОшибке = НСтр("ru='Ошибка отправки уведомлений по электронной почте:';uk='Помилка відправки повідомлень по електронній пошті :'");
			ВложенияПисьма = Новый Соответствие;
		Иначе
			ШаблонСообщенияОбОшибке = НСтр("ru='Ошибка отправки отчетов по электронной почте:';uk='Помилка відправки звітів по електронній пошті :'");
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если СтандартнаяОбработка = Истина Тогда
			ЗаголовокОшибки = НСтр("ru='Ошибка записи отчета %1 в формат %2:';uk='Помилка запису звіту %1 в формат %2:'");
			
			Если ПараметрыФормата.ТипФайла = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					ЗаголовокОшибки, НСтр("ru='Формат не поддерживается';uk='Формат не поддерживается'"),
					"'"+ СтрокаОтчет.Ключ +"'", 
					"'"+ ПараметрыФормата.Имя +"'"
				);
				Продолжить;
...
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		Если НЕ ЗначениеЗаполнено(ПараметрыДоставки.Папка) Тогда
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ru='Папка не заполнена, доставка в папку отключена';uk='Тека не заповнена, доставка в теку відключена'"), ,
			);
		Иначе
			ПравоДоступа = Истина;
			...
			Если НЕ ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					НСтр("ru='Недостаточно прав на изменение файлов папки %1, доставка в папку отключена';uk='Недостатньо прав на зміну файлів теки %1, доставка в теку відключена'"), ,
					"'"+ ПредставлениеПапки +"'"
				);
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если НЕ ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			Иначе
				ЗначениеПодстановки = НСтр("ru='Windows и Linux';uk='Windows та Linux'");
			КонецЕсли;
			
			ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru='Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена';uk='Мережевий каталог %1 не вибраний, доставка в мережевий каталог відключена'"), ,
				ЗначениеПодстановки
			);
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс И НЕ ЗначениеЗаполнено(ПараметрыДоставки.Сервер) Тогда
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='FTP сервер не заполнен, доставка в папку на FTP ресурс отключена';uk='FTP сервер не заповнений, доставка в теку на FTP ресурс відключена'")
		);
	КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту И НЕ ЗначениеЗаполнено(ПараметрыДоставки.УчетнаяЗапись) Тогда
		ПараметрыДоставки.ИспользоватьЭлектроннуюПочту = Ложь;
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='Учетная запись не выбрана, доставка по электронной почте отключена';uk='Обліковий запис не вибраний, доставка по електронній пошті відключена'")
		);
	КонецЕсли;
	...
	Если ПараметрыДоставки.Персонализирована Тогда
		Если НЕ ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru='Персонализированная рассылка может быть отправлена только по электронной почте';uk='Розсилка, що персоналізується, може бути відправлена тільки по електронній пошті'")
			);
			Возврат Ложь;
		КонецЕсли;
		ПараметрыДоставки.ИспользоватьПапку          = Ложь;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить 
			...
			Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ru='Использование уведомлений по электронной почте возможно только совместно с другими способами доставки';uk='Використання повідомлень по електронній пошті можливе тільки спільно з іншими способами доставки'")
			);
			Возврат Ложь;
		КонецЕсли;
...
Процедура УдалитьВременныеФайлы(ПараметрыЖурнала, ПараметрыДоставки)
	...
	Исключение
		ЗаписьЖурнала(ПараметрыЖурнала, ,
			НСтр("ru='Ошибка очистки каталога временных файлов %1:';uk='Помилка очищення каталогу тимчасових файлів %1:'"), ИнформацияОбОшибке(), 
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если НЕ ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='У пользователя %1 недостаточно прав на вывод информации';uk='У користувача %1 недостатньо прав на виведення інформації'"), ,
...
Функция ПредставлениеСпособовДоставки(ПараметрыДоставки)
	...
	Если НЕ ПараметрыДоставки.ТолькоУведомить Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ru='и';uk='та'")) 
		+ " "
	...
	Если ПараметрыДоставки.ВыполненаВПапку Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ru='и';uk='та'")) 
		+ " "
		+ НСтр("ru='доставлен в папку';uk='доставлений в теку'")
		+ " ";
	...
	Если ПараметрыДоставки.ВыполненаВСетевойКаталог Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ru='и';uk='та'")) 
		+ " "
		+ НСтр("ru='доставлен в сетевой каталог';uk='доставлений в мережевий каталог'")
		+ " ";
