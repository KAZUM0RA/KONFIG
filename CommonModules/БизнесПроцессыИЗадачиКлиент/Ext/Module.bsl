...
Функция ЗаписатьИЗакрытьВыполнить(Форма, ВыполнитьЗадачу = Ложь, ПараметрыОповещения = Неопределено) Экспорт
	...
	Если НЕ Форма.НачальныйПризнакВыполнения И ВыполнитьЗадачу Тогда
		Если НЕ Форма.Записать(Новый Структура("ВыполнитьЗадачу", Истина)) Тогда
			Возврат Ложь;
		КонецЕсли;
		ТекстОповещения = НСтр("ru='Задача выполнена';uk='Завдання виконане'");
	Иначе
		Если НЕ Форма.Записать() Тогда
			Возврат Ложь;
		КонецЕсли;
		ТекстОповещения = ?(НовыйОбъект, НСтр("ru='Задача создана';uk='Завдання створене'"), НСтр("ru='Задача изменена';uk='Завдання змінене'"));
	КонецЕсли;
...
Процедура ПеренаправитьЗадачи(МассивЗадач, ВладелецФорма) Экспорт
	...
	Если НЕ ЗадачиМогутБытьПеренаправлены И МассивЗадач.Количество() = 1 Тогда
		Предупреждение(НСтр("ru='Невозможно перенаправить уже выполненную задачу.';uk='Неможливо перенаправляти вже виконане завдання.'"));
		Возврат;
	КонецЕсли;
	...
	Результат = ОткрытьФормуМодально("Задача.ЗадачаИсполнителя.Форма.ПеренаправитьЗадачи",
		...
		?(МассивЗадач.Количество() > 1, НСтр("ru='Перенаправить задачи';uk='Перенаправляти завдання'"), 
			НСтр("ru='Перенаправить задачу';uk='Перенаправляти завдання'"))), 
		ВладелецФорма);
	...
	Если МассивЗадач.Количество() > 10 Тогда
		Состояние(НСтр("ru='Перенаправление задач';uk='Перенапрямок завдань'"),, 
			НСтр("ru='Выполняется перенаправление задач...';uk='Виконується перенапрямок завдань...'"));
	КонецЕсли;
	...
	Если МассивЗадач.Количество() > 1 Тогда
		Если ЗадачиПеренаправлены Тогда
			Состояние(НСтр("ru='Перенаправление задач';uk='Перенапрямок завдань'"),, 
				НСтр("ru='Задачи перенаправлены.';uk='Завдання переспрямовані.'"));
		Иначе
			Состояние(НСтр("ru='Перенаправление задач';uk='Перенапрямок завдань'"),,
				НСтр("ru='Не все задачи перенаправлены. Пропущены задачи, отмеченные как выполненные.';uk='Не всі завдання переспрямовані. Пропущені завдання, відзначені як виконані.'"));
		КонецЕсли;
	Иначе
		Задача = МассивПеренаправленныхЗадач[0];
		ПоказатьОповещениеПользователя(
			НСтр("ru='Задача перенаправлена';uk='Завдання переспрямоване'"),
...
Процедура СписокБизнесПроцессовПометкаУдаления(Список) Экспорт
	...
	Если ВыделенныеСтроки = Неопределено ИЛИ ВыделенныеСтроки.Количество() <= 0 Тогда
		...
		Возврат;
	КонецЕсли;	
	Результат = Вопрос(НСтр("ru='Изменить пометку удаления?';uk='Змінити позначку видалення?'"), РежимДиалогаВопрос.ДаНет);
	Если Результат <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;	
	БизнесПроцессСсылка = БизнесПроцессыИЗадачиВызовСервера.ПометитьНаУдалениеБизнесПроцессы(ВыделенныеСтроки);
	Список.Обновить();
	ПоказатьОповещениеПользователя(НСтр("ru='Пометка удаления изменена.';uk='Позначка видалення змінена.'"), 
