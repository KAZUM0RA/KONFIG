...
Процедура ОбновитьИзФайлаНаДиске(ДанныеФайла, ИдентификаторФормы) Экспорт
	...
	Иначе
		Предупреждение(НСтр("ru='Для выполнения данной операции вам нужно установить расширение работы с файлами.';uk=' Для виконання даної операції вам потрібно встановити розширення роботи з файлами.'"));
	КонецЕсли;
...
Функция Зашифровать(ДанныеФайла,
	...
	Если ДанныеФайла.Зашифрован Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Файл ""%1"" уже зашифрован.';uk='Файл ""%1"" уже зашифрований.'"), Строка(ДанныеФайла.Ссылка));
...
Функция ДобавитьЭЦПИзФайла(ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	#Иначе
		Предупреждение(НСтр("ru='В Веб-клиенте добавление ЭЦП из файла не поддерживается.';uk='В Веб-клиенте добавление ЭЦП из файла не поддерживается.'"));
		Возврат Ложь;
	#КонецЕсли
