...
Функция ПолучитьСсылкуПоШтрихкодуТабличногоДокумента(Штрихкод, Менеджеры = Неопределено) Экспорт
	...
	Если Менеджеры = Неопределено Тогда
		...
		Для Каждого ЭлементМетаданных Из Метаданные.Справочники Цикл
			МенеджерыОбъектов.Добавить(Справочники[ЭлементМетаданных.Имя]);
		КонецЦикла;
	...
	Иначе
		...
		Для Каждого ПустаяСсылка Из Менеджеры Цикл
			...
			Иначе
				ТекстИсключения = НСтр("ru='Ошибка распознавания штрихкода: тип ""%Тип%"" не поддерживается.';uk='Помилка розпізнавання штрихкода: тип ""%Тип%"" не підтримується.'");
