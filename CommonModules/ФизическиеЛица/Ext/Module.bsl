...
Процедура ЗаполнитьДокументыФизическихЛицПоКлассификаторуМВД() Экспорт
	...
	СписокДокументов.Добавить("Загранпаспорт гражданина Украины");
	...
	СписокДокументов.Добавить("Дипломатический паспорт гражданина Украины");
...
Процедура ПреобразоватьУдостоверенияЛичностиВДокументы() Экспорт
	...
	ТекстДатаВыдачи			= НСтр("ru=', выдан: %1 года';uk=', виданий: %1 року'");
	ТекстСрокДействия		= НСтр("ru=', действует до: %1 года';uk=', діє до: %1 року'");
	ТекстКодПодразделения	= НСтр("ru=', № подр. %1';uk=', № подр. %1'");
	...
	Пока Выборка.СледующийПоЗначениюПоля("Физлицо") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ПустаяСтрока(Запись.Представление) И Не Запись.ВидДокумента.Пустая() Тогда
				...
				+ ?(ЗначениеЗаполнено(Запись.КодПодразделения) И Запись.ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.Паспорт, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстКодПодразделения, Запись.КодПодразделения), "");
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаменитьВидыДокументовНаПредопределенные() Экспорт
	...
	|		КОГДА ДокументыФизическихЛиц.ВидДокумента.Наименование = ""Удостоверение личности""
	...
	|			ИЛИ ВидыДокументовФизическихЛиц.Наименование = ""Удостоверение личности"")";
	Выборка = Запрос.Выполнить().Выбрать();
