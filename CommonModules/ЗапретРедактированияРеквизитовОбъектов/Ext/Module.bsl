...
//
Процедура ПодготовитьФорму(Форма, Ссылка, СпециальнаяПолеГруппыДляКнопкиЗапрета = Неопределено)
	...
	Если Пользователи.РолиДоступны("РедактированиеРеквизитовОбъектов")
	   ...
	   И НЕ ВсеРеквизитыБезПраваРедактирования Тогда
		...
		Команда.Заголовок = НСтр("ru='Разрешить редактирование реквизитов объекта';uk=""Дозволити редагування реквізитів об'єкта""");
		...
		// Добавление кнопки
		Если СпециальнаяПолеГруппыДляКнопкиЗапрета <> Неопределено Тогда
			РодительскаяГруппа = СпециальнаяПолеГруппыДляКнопкиЗапрета;
		Иначе
