...
Процедура ОбъектНеНайден(Штрихкод, ИскатьПоВсемОбъектам = Истина) Экспорт
	...
	Если ИскатьПоВсемОбъектам Тогда
		Состояние(НСтр("ru='Выполняется поиск документа по штрихкоду во всех документах информационной базы..';uk='Виконується пошук документу по штрихкоду в усіх документах інформаційної бази.'"));
	...
	Если ВывестиСообщениеОбъектНеНайден Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Объект со штрихкодом %1 не найден';uk=""Об'єкт з штрихкодом %1  не знайдений"""), Штрихкод));
	КонецЕсли;
...
Функция ПолучитьСсылкуПоШтрихкодуТабличногоДокумента(Штрихкод, Менеджеры = Неопределено) Экспорт
	...
	Состояние(НСтр("ru='Выполняется поиск документа по штрихкоду...';uk='Виконується пошук документу по штрихкоду...'"));
