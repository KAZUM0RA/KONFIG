...
// Возвращает типы получателей в разрезах хранения и пользовательского представления этих типов
// 
// Возвращаемое значение: 
...
//       |- ОсновнойВидКИ - (СправочникСсылка.ВидыКонтактнойИнформации) вид контактной информации: e-mail, по умолчанию
//       |- ГруппаКИ - (СправочникСсылка.ВидыКонтактнойИнформации) группа вида контактной информации
//       |- ПутьФормыВыбора - (Строка) 
//
Функция ТаблицаТиповПолучателей() Экспорт
	...
	РассылкаОтчетов.ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, 
		Новый Структура("ОсновнойТип, ДополнительныйТип", 
			Тип("СправочникСсылка.Пользователи"), 
			Тип("СправочникСсылка.ГруппыПользователей")
		)
	);
	...
	Для Каждого НеиспользованныйТип Из ДоступныеТипы Цикл
		РассылкаОтчетов.ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ПустойМассив, 
			Новый Структура("ОсновнойТип", НеиспользованныйТип)
		);
	КонецЦикла;
