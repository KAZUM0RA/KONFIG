...
Процедура ПровестиРезервноеКопирование() Экспорт
	...
	Если НастройкиУдаления.ПроизводитьУдаление И КаталогХранения <> Неопределено Тогда
		...
		Исключение
			...
			НСтр("ru='Не удалось провести очистку каталога с резервными копиями.';uk='Не вдалося провести очищення каталогу з резервними копіями.'"),,Истина);
...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения) Экспорт
	...
	ИначеЕсли ВариантОповещения = "Напомнить" Тогда
		...
		ТекстПояснения = НСтр("ru='Резервное копирование не выполнялось с %1.';uk='Резервне копіювання не виконувалося з %1.'");
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Резервное копирование информационной базы';uk='Резервне копіювання інформаційної бази'");
...
Функция КлиентскиеПараметрыРезервногоКопирования() Экспорт
	#Если НЕ ВебКлиент Тогда
		...
		СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации", НСтр("ru='Резервное копирование ИБ';uk='Резервное копирование ИБ'"));
...
Процедура ПриЗавершенииРаботыСистемы(Предупреждения) Экспорт
	...
	Если ТипОтвета = Тип("Дата") Тогда 
		ТекстФлажка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Завершить работу системы после резервного копирования %1';uk='Завершити роботу системи після резервного копіювання %1'"), Ответ);
		...
		ПараметрыФормы = Новый Структура("ТипВызова, ТекущаяСтраница, ЗаголовокНадписи", 
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтрокуИзМассива(НСтр("ru='Резервное копирование будет произведено автоматически не позднее %1';uk='Резервне копіювання буде зроблено автоматично не пізніше %1'"), 
			МассивПараметров));
	...
	ИначеЕсли ТипОтвета = Тип("Булево") Тогда 
		ТекстФлажка 	= НСтр("ru='Выполнить резервное копирование';uk='Виконати резервне копіювання'");
		ПараметрыФормы 	= Новый Структура("ТипВызова, ТекущаяСтраница", 2, "СтраницаИнформацииИВыполненияРезервногоКопирования");
		ФормаОбработки 	= "Обработка.РезервноеКопированиеИБ.Форма.РезервноеКопированиеИнформационнойБазы";
