...
Функция ИнформацияПоАдресномуОбъекту(КодАдресногоОбъекта) Экспорт
	...
	Если КодАдресногоОбъекта = "AL" Тогда
		...
		Результат.Наименование        = НСтр("ru='Альтернативные названия';uk='Альтернативні назви'");
	...
	ИначеЕсли КодАдресногоОбъекта = "SO" Тогда
		...
		Результат.Наименование        = НСтр("ru='Адресные сокращения';uk='Адресні скорочення'");
		Результат.Сокращение          = "";
		Результат.Индекс              = "";
	Иначе
...
Процедура РазложитьАдресныеОбъектыПоЭлементам() Экспорт
	...
	Для Каждого СтруктураАдреса Из МассивНеактуальныхАдресов Цикл
        ...
        // Записываем запись с исправлением не актуального адреса
		МенеджерЗаписи.ТипАдресногоЭлемента = СтруктураАдреса.ТипАдресногоЭлемента;
		МенеджерЗаписи.Код = СтруктураАдреса.Код;
...
Функция ЗагрузитьАдресныеСокращения(ПутьКДаннымНаСервере) Экспорт
	...
	Если ЕстьОшибки Тогда
		СтрокаОшибки = НСтр("ru='В файле адресного классификатора %1 есть ошибки уникальности кодов';uk='У файлі адресного класифікатора %1 є помилки унікальності кодів'");
		ПараметрыВСтроке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, "socrbase.dbf");
		ЗаписьЖурналаРегистрации(НСтр("ru='Адресный классификатор.Загрузка';uk='Адресний класифікатор.Завантаження'"), 
...
"текущая версия от %3, а загружаемая от %4.';uk=""Відомості по об'єкту: %1 %2 не завантажені, тому що поточна версія від %3, а, що завантажується від %4."""),
