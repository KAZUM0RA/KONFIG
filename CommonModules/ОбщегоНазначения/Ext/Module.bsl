...
Функция ПолучитьЗначенияРеквизитов(Ссылка, ИменаРеквизитов) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный тип второго параметра ИменаИлиСтруктураРеквизитов: %1';uk='Невірний тип другого параметра ИменаИлиСтруктураРеквизитов: %1'"), 
	...
	Для Каждого ДокументСсылка Из Документы Цикл
		...
		Если ДокументОбъект.ПроверитьЗаполнение() Тогда
			...
			Исключение
				...
				ТекстСообщенияОбОшибке = НСтр("ru='Ошибка при попытке проведения документа: %1';uk='Помилка при спробі проведення документа: %1'");
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Проведение документов при печати';uk='Проведення документів при печатці'"),
...
Функция КоллекцииИдентичны(КоллекцияСтрок1, КоллекцияСтрок2, ИменаКолонок = "", ИсключаяКолонки = "", УчитыватьПоследовательностьСтрок = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИменаКолонок) Тогда
		Если ТипыОсобыхКоллекций.Найти(ТипЗнч(КоллекцияСтрок1)) <> Неопределено 
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = НСтр("ru='Для коллекции типа %1 необходимо указать имена полей, по которым производится сравнение';uk=' Для колекції типу %1 необхідно вказати імена полів, по яких проводиться порівняння'");
...
Функция УстановитьВнешнееСоединение(Параметры, СтрокаСообщенияОбОшибке = "", ОшибкаПодключенияКомпоненты = Ложь) Экспорт
	...
	Исключение
		СтрокаСообщенияОбОшибке = НСтр("ru='При установке внешнего соединения произошла ошибка: %1';uk="" При установці зовнішнього з'єднання відбулася помилка: %1""");
...
Процедура ВыполнитьБезопасно(ИмяЭкспортнойПроцедуры, Параметры = Неопределено, ОбластьДанных = Неопределено) Экспорт
	...
	Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяЭкспортнойПроцедуры (%1)';uk='Неправильний формат параметра ИмяЭкспортнойПроцедуры (%1)'"),
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Безопасное выполнение метода';uk='Безпечне виконання методу'"), УровеньЖурналаРегистрации.Ошибка, , ,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяЭкспортнойПроцедуры (%1)';uk='Неправильний формат параметра ИмяЭкспортнойПроцедуры (%1)'"),
			ИмяЭкспортнойПроцедуры);
...
Функция ВидОбъектаПоТипу(Тип) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный тип значения параметра (%1)';uk='Невірний тип значення параметра (%1)'"), Строка(Тип));
	...
	Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"""%1"".';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных().  Об''єкт метаданих не знайдений по типу: ""%1"".'"),
		...
		Если ТипОписанияОбъектаМетаданных = Тип("Строка")
		   И Метаданные.НайтиПоПолномуИмени(ОписаниеОбъектаМетаданных) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"""%1"".';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных().  Об''єкт метаданых не знайдений по повному імені: ""%1"".'"),
				ОписаниеОбъектаМетаданных);
		КонецЕсли;
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
				...
				"предопределенных элементов, например, подсистемы.';uk='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
				""
				"Для объекта метаданных ""%1"""
				"не найден идентификатор"
				"в справочнике ""Идентификаторы объектов метаданных""."
	...
	ИначеЕсли Выгрузка.Количество() > 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
				...
				"""Все функции"" -> ""Справочник. Идентификаторы объектов метаданных"" ->"
				"команда формы списка ""Обновить данные справочника"".';uk='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
				""
				"Для объекта метаданных ""%1"""
				"найдено несколько идентификаторов"
				"в справочнике ""Идентификаторы объектов метаданных""."
	...
	Если Выгрузка.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()."
				...
				""
				"Некоторые объекты метаданных могут быть добавлены в справочник только в виде"
				"предопределенных элементов, например, подсистемы.';uk='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()."
				""
				"Идентификатор ""%1"""
				"не найден в справочнике ""Идентификаторы объектов метаданных""."
				""
			...
			Если РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()."
						...
						"следует выполнить обновление вручную:"
						"""Все функции"" -> ""Справочник. Идентификаторы объектов метаданных"" ->"
						"команда формы списка ""Обновить данные справочника"".';uk='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()."
						""
						"Идентификатору ""%1"""
						"найденому в справочнике ""Идентификаторы объектов метаданных"","
						"соответствует несуществующий объект метаданных"
	...
	Если ТипЗнч(ОбъектМетаданных) <> Тип("ОбъектМетаданных") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-"
				...
				""
				"Для идентификатора ""%1"" не заполнен объект метаданных.';uk='Помилка в процедурі ОбщегоНазначенияПереопределяемый.Заповнити- ПредустановленныеИдентификаторыОбъектовМетаданных()   Для ідентифікатора ""%1"" не заповнений об''єкт метаданих.'"),
	...
	Если НЕ ЗначениеЗаполнено(Идентификатор) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-"
				...
				"""%2"".';uk='Помилка в процедурі ОбщегоНазначенияПереопределяемый.Заповнити- ПредустановленныеИдентификаторыОбъектовМетаданных()  Ошбика в рядку ідентифікатора ""%1"", заданого для об''єкта метаданих ""%2"".'"),
			ИдентификаторВСтроке,
			ПолноеИмя);
	КонецЕсли;
	...
	Если Идентификаторы.Найти(ПолноеИмя, "ПолноеИмя") <> Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-"
				...
				""
				"Для объекта метаданных ""%1"""
				"идентификатор уже задан.';uk='Помилка в процедурі ОбщегоНазначенияПереопределяемый.Заповнити- ПредустановленныеИдентификаторыОбъектовМетаданных()   Для об''єкта метаданих ""%1"" ідентифікатор уже заданий.'"),
			ПолноеИмя);
	КонецЕсли;
	...
	Если Идентификаторы.Найти(Идентификатор, "Идентификатор") <> Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-"
				...
				"для объекта метаданных ""%2"".';uk='Помилка в процедурі ОбщегоНазначенияПереопределяемый.Заповнити- ПредустановленныеИдентификаторыОбъектовМетаданных()  Ідентифікатор ""%1""  уже заданий  для об''єкта метаданих ""%2"".'"),
			ИдентификаторВСтроке,
			ПолноеИмя);
	КонецЕсли;
...
"Описание ошибки: %1';uk='Кореспондент не підтримує версii інтерфейсів підсистем. Опис помилки: %1'"
		);
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение версий интерфейса';uk='Одержання версій інтерфейсу'"), УровеньЖурналаРегистрации.Ошибка,,, СтрокаСообщения);
...
Процедура ОбновитьДанныеКэшаВерсий(Знач Идентификатор, Знач ТипДанных, Знач ПараметрыПолучения) Экспорт
	...
	Иначе
		ШаблонТекста = НСтр("ru='Не известный тип данных кэша версий: %1';uk='Не відомий тип даних кэша версій: %1'");
...
Функция ПолучитьWSDL(Знач Адрес, Знач ИмяПользователя, Знач Пароль)
	...
	Если Определения.Сервисы.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ru='Ошибка получения файла описания web-сервиса:"
			...
			"%1';uk='Помилка одержання файлу опису web-сервісу: В отриманому файлі не втримується жодного опису сервісу.   Можливо, адреса файлу опису зазначений невірно: %1'");
