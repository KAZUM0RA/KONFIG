...
Функция ВозможностьОсвободитьФайл(ОбъектСсылка, РедактируетТекущийПользователь, Редактирует, СтрокаОшибки = "") Экспорт
	...
	ИначеЕсли Редактирует.Пустая() Тогда
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		                           НСтр("ru='Нельзя освободить файл ""%1"" т.к. он никем не занят.';uk=' Не можна звільнити файл ""%1"" тому що він ніким не зайнятий.'"), Строка(ОбъектСсылка));
		Возврат Ложь;
	Иначе
		...
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		                           НСтр("ru='Нельзя освободить файл ""%1"" т.к. он занят пользователем ""%2"".';uk=' Не можна звільнити файл ""%1"" тому що він зайнятий користувачем ""%2"".'"),
		                           Строка(ОбъектСсылка),
		                           Строка(Редактирует));
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗанятьФайлПоСсылке(ОбъектСсылка, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ПоказатьОповещениеПользователя(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		    НСтр("ru='Файл ""%1"" занят для редактирования.';uk='Файл ""%1"" зайнятий для редагування.'"), Строка(ДанныеФайла.Ссылка)),
...
Процедура СоздатьДокументНаОсновеФайла(ПолноеИмяФайла, ВладелецФайла, ФормаВладелец, 
	...
	Если Не ФайловыеФункцииКлиентСервер.РасширениеФайлаРазрешеноДляЗагрузки(ЗапретЗагрузкиФайловПоРасширению, СписокЗапрещенныхРасширений, РасширениеФайла) Тогда
		ВызватьИсключение
			   СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 НСтр("ru='Загрузка файлов с расширением ""%1"" запрещена. Обратитесь к администратору системы.';uk='Завантаження файлів з розширенням ""%1"" заборонена. Звернетеся до адміністратора системи.'"),
				 РасширениеФайла);
	КонецЕсли;	
	...
	Если Файл.Размер() > МаксРазмерФайла Тогда
		...
		ВызватьИсключение
			   СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 НСтр("ru='Размер файла ""%1"" (%2 Мб) превышает максимально допустимый размер файла (%3 Мб).';uk='Розмір файлу ""%1"" (%2 Мб) перевищує максимально припустимий розмір файлу (%3 Мб).'"),
				 Файл.Имя, 
...
"Пожалуйста, подождите...';uk='Іде збереження файлу ""%1"" (%2 Мб).  Будь ласка, почекайте...'"),
		ИмяФайла,
		ФайловыеФункцииКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
	...
	Если НЕ ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, ФормаВладелец.УникальныйИдентификатор) Тогда		
		ВызватьИсключение
		  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при помещении файла в хранилище: %1';uk='Помилка при приміщенні файлу в сховище: %1'"), Файл.ПолноеИмя);
	КонецЕсли;
...
Процедура СоздатьФайл(РежимСоздания, ВладелецФайла, ФормаВладелец, НеОткрыватьКарточкуПослеСозданияИзФайла = Неопределено) Экспорт
	...
	ИначеЕсли РежимСоздания = 2 Тогда
		...
		Если РасширениеПодключено Тогда
			...
			ВыборФайла.Фильтр = НСтр("ru='Все файлы (*.*)|*.*';uk='Усі файли (*.*)|*.*'");
		...
		Иначе 
			...
			Иначе
				ВызватьИсключение
				  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				    НСтр("ru='Ошибка при помещении файла в хранилище: %1';uk='Помилка при приміщенні файлу в сховище: %1'"),ИмяФайла);
			КонецЕсли;
	...
	Иначе
		ВызватьИсключение
		  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		    НСтр("ru='Неверный режим создания файла: %1';uk='Невірний режим створення файлу: %1'"), РежимСоздания);
	КонецЕсли;
...
Функция ЗакончитьРедактированиеПоСсылкам(Знач МассивФайлов, ИдентификаторФормы) Экспорт
	...
	ПоказатьОповещениеПользователя(  
		НСтр("ru='Закончить редактирование файлов';uk='Закінчити редагування файлів'"),
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Закончено редактирование файлов (%1 из %2).';uk='Закінчене редагування файлів (%1 з %2).'"), 
...
Функция ЗакончитьРедактирование(ОбъектСсылка, ИдентификаторФормы, Знач ХранитьВерсии = Неопределено,
	...
	Если РасширениеПодключено Тогда // код для тонкого (или толстого) клиента (или веб с подключенным расширением)
		...
		Если НЕ ФайлНовойВерсии.Существует() Тогда
			...
			Если ПрименитьКоВсем = Ложь Тогда
				...
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				    НСтр("ru='Не удалось загрузить файл ""%1"" в информационную базу, т.к. он не найден на локальном компьютере';uk='Не вдалося завантажити файл ""%1"" в інформаційну базу, тому що він не знайдений на локальному комп''ютері'"),
		...
		Исключение
			...
			Предупреждение(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось вернуть файл ""%1"" в информационную базу, т.к. он заблокирован другой программой';uk='Не вдалося повернути файл ""%1"" в інформаційну базу, тому що він заблокований іншою програмою'"),
					Строка(ДанныеФайла.Ссылка)));
...
"Пожалуйста, подождите.';uk='Виконується передача файлу ""%1"" (%2 Мб)...  Будь ласка, почекайте.'"),
				    ФайловыеФункцииКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
				...
				Если ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Интерактивно, ИдентификаторФормы) Тогда
					...
					Если ПоказыватьОповещение Тогда
						...
						ПоказатьОповещениеПользователя(
							...
							ДанныеФайла.НавигационнаяСсылка,
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Файл %1 обновлен и освобожден.';uk='Файл %1 оновлений і звільнений.'"),
									Строка(ДанныеФайла.Ссылка)),
								БиблиотекаКартинок.Информация32);
					КонецЕсли;		
...
"Повторить операцию?';uk='Не вдалося повернути файл ""%1"" в інформаційну базу через: ""%2"" Повторити операцію?'"),
				...
				Если ПоместитьФайл(АдресВременногоХранилища, ПолныйПутьКФайлу, ВыбранныйПутьКФайлу, Интерактивно, ИдентификаторФормы) Тогда
					...
					Если ПоказыватьОповещение Тогда
						...
						ПоказатьОповещениеПользователя(
							...
							ДанныеФайла.НавигационнаяСсылка,
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							    НСтр("ru='Файл %1 обновлен и освобожден.';uk='Файл %1 оновлений і звільнений.'"), Строка(ДанныеФайла.Ссылка)),
							БиблиотекаКартинок.Информация32);
					КонецЕсли;	
...
"Продолжить?';uk='Скасування редагування файлів може привести до втрати Ваших змін. Продовжити?'"),
	...
	ПоказатьОповещениеПользователя(  
		...
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отменено редактирование файлов (%1 из %2).';uk='Скасоване редагування файлів (%1 з %2).'"), 
...
"Продолжить?';uk='Скасування редагування файлу ""%1"" може привести до втрати Ваших змін. Продовжити?'"), Строка(ОбъектСсылка)),
		РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ОсвободитьФайлы(ОбъектыСсылка) Экспорт
	...
	Если СписокНевозможноОсвободить.Количество() > 0 Тогда 
		Если НЕ ДиалогВопросаСоСписком(
				...
				НСтр("ru='Освободить остальные файлы?';uk='Звільнити інші файли?'"),
				НСтр("ru='При попытке освободить файлы возникли следующие ошибки:';uk=' При спробі звільнити файли виникли наступні помилки:'"),
				НСтр("ru='Освободить файлы';uk='Звільнити файли'")) Тогда
			Возврат;
		КонецЕсли;
	Иначе
		Ответ = Вопрос(
		НСтр("ru='Освобождение файлов может привести к потере Ваших изменений."
"Освободить файлы?';uk='Звільнення файлів може привести до втрати Ваших змін. Звільнити файли?'"),
		РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ПеренестиФайлыВПапку(ОбъектыСсылка, Папка) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		ПоказатьОповещениеПользователя(
			...
			ДанныеФайла.НавигационнаяСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			    НСтр("ru='Файл ""%1"" перенесен в папку %2.';uk='Файл ""%1"" перенесений у папку %2.'"), Строка(ДанныеФайла.Ссылка), Строка(Папка)),
			БиблиотекаКартинок.Информация32);
...
Процедура ОпубликоватьФайлыПоСсылкам(Знач МассивФайлов, ИдентификаторФормы) Экспорт
	...
	ПоказатьОповещениеПользователя(  
		НСтр("ru='Сохранить изменения файлов';uk='Зберегти зміни файлів'"),
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сохранены изменения файлов (%1 из %2).';uk='Збережені зміни файлів (%1 з %2).'"), 
			ДанныеФайлов.Количество(), МассивФайлов.Количество()),
		БиблиотекаКартинок.Информация32);
...
Процедура УдалитьФайл(ПолноеИмяФайла, ЗадаватьВопрос = Неопределено, ШапкаВопроса = Неопределено) Экспорт
	...
	Если ПодтверждатьПриУдаленииИзЛокальногоКэшаФайлов = Истина Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удалить файл ""%1"" из основного рабочего каталога?';uk='Вилучити файл ""%1"" з основного робочого каталогу?'"), ПолноеИмяФайла);
...
Функция ПолучитьФайлВерсииВЛокальныйКэшФайловНаЧтение(ДанныеФайла, ПолноеИмяФайла, ИдентификаторФормы = Неопределено,
	...
	Если ИмяФайлаСПутем = "" Тогда
		Предупреждение(НСтр("ru='Ошибка помещения файла в локальный кэш файлов.';uk='Помилка приміщення файлу в локальний кеш файлів.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если РазницаДат <= 1 Тогда // 1 секунда - допустимая разница (на Win95 может быть такое)
		...
		Если РазмерФайлаВБазе <> 0 И РазмерФайлаНаДиске <> РазмерФайлаВБазе Тогда
			...
			ПараметрыОткрытияФормы.Вставить("Сообщение",            		НСтр("ru='Размер файла в рабочем каталоге и на сервере отличается. Перезаписать файл в рабочем каталоге?';uk='Розмір файлу в робочому каталозі й на сервері відрізняється. Перезаписати файл у робочому каталозі?'"));
	...
	ИначеЕсли ДатаФайлаНаДиске < ДатаФайлаВБазе Тогда // В рабочем каталоге более старый
		Если ВРабочемКаталогеНаЧтение = Ложь Тогда // В рабочем каталоге на редактирование
			...
			Сообщение = НСтр("ru='Файл на локальном компьютере, отмеченный как взятый на редактирование, имеет более раннюю дату изменения, чем на сервере. Открыть файл из локального каталога или взять из информационной базы и перезаписать?';uk=""Файл на локальному комп'ютері, відзначений як узятий на редагування, має більш ранню дату зміни, чому на сервері. Відкрити файл із локального каталогу або побрати з інформаційної бази й перезаписати?""");
	...
	ИначеЕсли ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны)		
		...
		Иначе // В рабочем каталоге на чтение
			...
			Сообщение = НСтр("ru='Файл на локальном компьютере имеет более позднюю дату, возможно, он был изменен. Открыть существующий или взять с сервера и перезаписать?';uk=""Файл на локальному комп'ютері має більш пізню дату, можливо, він був змінений. Відкрити існуючий або побрати із сервера й перезаписати?""");
			ПараметрыОткрытияФормы.Вставить("Сообщение", Сообщение);
...
Функция ПолучитьФайлВерсииВЛокальныйКэшФайловНаРедактирование(ДанныеФайла, ПолноеИмяФайла, ИдентификаторФормы = Неопределено,
	...
	ИмяФайлаСПутем = ПолноеИмяФайла;
	Если ИмяФайлаСПутем = "" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка помещения файла в локальный кэш файлов.';uk='Помилка приміщення файлу в локальний кеш файлів.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли ДатаФайлаНаДиске < ДатаФайлаВБазе Тогда // В рабочем каталоге более старый
		Если ВРабочемКаталогеНаЧтение = Ложь Тогда // В рабочем каталоге на редактирование
			...
			Сообщение = НСтр("ru='Файл на локальном компьютере, отмеченный как взятый на редактирование, имеет более раннюю дату изменения, чем на сервере. Открыть файл из локального каталога или взять с сервера и перезаписать?';uk=""Файл на локальному комп'ютері, відзначений як узятий на редагування, має більш ранню дату зміни, чому на сервері. Відкрити файл із локального каталогу або побрати із сервера й перезаписати?""");
			ПараметрыОткрытияФормы.Вставить("Сообщение", Сообщение);
	...
	ИначеЕсли ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны)		
		...
		Иначе // В рабочем каталоге на чтение
			...
			Сообщение = НСтр("ru='Файл на локальном компьютере, отмеченный как взятый на редактирование, имеет более позднюю дату изменения, чем на сервере. Открыть файл из локального каталога или взять с сервера и перезаписать?';uk=""Файл на локальному комп'ютері, відзначений як узятий на редагування, має більш пізню дату зміни, чому на сервері. Відкрити файл із локального каталогу або побрати із сервера й перезаписати?""");
			ПараметрыОткрытияФормы.Вставить("Сообщение", Сообщение);
...
"%2';uk=""Довжина повного шляху до файлу (шлях робочого каталогу плюс ім'я файлу) перевищує %1 символів %2"""),
		...
		Если СтрДлина(ИмяФайла) > МаксимальнаяДлинаИмениФайла Тогда
			ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК + НСтр("ru='Измените имя файла на более короткое.';uk=""Зміните ім'я файлу на більш коротке.""");
		...
		Если СтрДлина(ДанныеФайла.РабочийКаталогВладельца) > МаксимальнаяДлинаПолногоПути - 5 Тогда
			ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК + НСтр("ru='Измените имена папок или перенесите текущую папку в другую папку.';uk='Зміните імена папок або перенесіть поточну папку в іншу папку.'");
			Предупреждение(ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;	
...
"(Содержимое рабочего каталога будет перенесено в выбранный каталог)';uk='Вибрати інший шлях до основного робочого каталогу?  (Уміст робочого каталогу буде перенесений в обраний каталог)'");
			Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
			Если Ответ = КодВозвратаДиалога.Нет Тогда
				Возврат Ложь;
			КонецЕсли;	
			...
			Заголовок = НСтр("ru='Выберите другой путь к рабочему каталогу';uk='Виберіть інший шлях до робочого каталогу'");
	...
	Если НЕ ФайлЕстьВРегистре Тогда // на Файл нет ссылки в регистре
		...
		Если ФайлНаДиске.Существует() Тогда
			...
			Если РазницаДат > 1 Тогда
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Файл ""%1"" в рабочем каталоге на локальном компьютере уже есть, но его дата изменения отличается от даты в информационной базе. Открыть файл из локального каталога или взять из информационной базы и перезаписать?';uk='Файл ""%1"" у робочому каталозі на локальному комп''ютері уже є, але його дата зміни відрізняється від дати в інформаційній базі. Відкрити файл із локального каталогу або побрати з інформаційної бази й перезаписати?'"),
					Строка(ДанныеФайла.Ссылка));
				ПараметрыОткрытияФормы.Вставить("Сообщение", Сообщение);
...
"Переименуйте его или переместите в другой каталог. ';uk='Файл ""%3"" у робочому каталозі уже є, але його розмір відрізняється від дати на сервері.   Розмір файлу в робочому каталозі: %1  Розмір файлу на сервері: %2 Перейменуйте його або перемістите в інший каталог. '"),
...
" Код файла на локальном компьютере: %3 ';uk='У робочому каталозі уже є файл ""%1"", пов''язаний з іншим файлом інформаційної бази.   Код файлу на сервері: %2  Код файлу на локальному комп''ютері: %3 '"),
							  ПолноеИмяФайла,
							  ДанныеФайла.КодФайла, 
							  ВРегистреКодФайла);			
							  
		Если ДанныеФайла.Владелец = ВРегистреПапка Тогда // одна и та же папка
			СтрокаСообщения = СтрокаСообщения + Символы.ПС + НСтр("ru='Переименуйте один из файлов в информационной базе.';uk='Перейменуйте один з файлів в інформаційній базі.'");
		Иначе
			СтрокаСообщения = СтрокаСообщения + Символы.ПС + НСтр("ru='Измените рабочий каталог одной из папок в информационной базе"
" (У двух папок не должно быть одинакового рабочего каталога).';uk='Зміните робочий каталог однієї з папок в інформаційній базі  (У двох папок не повинне бути однакового робочого каталогу).'");
		КонецЕсли;	
...
"Переименуйте его или переместите в другой каталог. ';uk='Файл ""%3"" у робочому каталозі уже є, але його дата зміни відрізняється від дати на сервері.   Дата зміни файлу в робочому каталозі: %1  Дата зміни файлу на сервері: %2 Перейменуйте його або перемістите в інший каталог. '"),
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Файл ""%1"" в рабочем каталоге на локальном компьютере уже есть, но его дата изменения отличается от даты на сервере. Открыть файл из локального каталога или взять с сервера и перезаписать?';uk='Файл ""%1"" у робочому каталозі на локальному комп''ютері уже є, але його дата зміни відрізняється від дати на сервері. Відкрити файл із локального каталогу або побрати із сервера й перезаписати?'"),
					Строка(ДанныеФайла.Ссылка));
				ПараметрыОткрытияФормы.Вставить("Сообщение", Сообщение);
...
Функция ПолучитьССервераИЗарегистрироватьВРабочемКаталоге(ДанныеФайла, ПолноеИмяФайлаВРабочемКаталоге, ДатаФайлаВБазе, 
	...
	Если ФайлВРабочемКаталоге Тогда
		...
		ИмяФайлаСПутем = ПолноеИмяФайла;
		Если ИмяФайлаСПутем = "" Тогда
			Предупреждение(НСтр("ru='Ошибка помещения файла в рабочий каталог.';uk='Помилка приміщення файлу в робочий каталог.'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если РасширениеПодключено Тогда
			...
			Если ФайлВРабочемКаталоге = Истина Тогда
				...
				Если ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны)		
					...
					Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Дата изменения файла ""%1"" на локальном компьютере более поздняя, чем в информационной базе. Возможно, он был изменен.';uk='Дата зміни файлу ""%1"" на локальному комп''ютері більш пізня, чому в інформаційній базі. Можливо, він був змінений.'"), 
						Строка(ДанныеФайла.Ссылка));
					ПараметрыОткрытияФормы.Вставить("Сообщение",  Сообщение);
		...
		#Если НЕ ВебКлиент Тогда	
			...
			Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоБазоваяВерсияКонфигурации Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данная операция не поддерживается в базовой версии.';uk='Дана операція не підтримується в базовій версії.'"));
				Возврат "";
			КонецЕсли;
		...
		Если ДанныеФайла.Зашифрован Тогда
			...
			Заголовок = НСтр("ru='Выбор режима сохранения';uk='Вибір режиму збереження'");
		...
		Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Все файлы (*.%1)|*.%1';uk='Усі файли (*.%1)|*.%1'"), Расширение, Расширение);
...
"Пожалуйста, подождите.';uk='Виконується збереження файлу ""%1"" (%2 Мб)...  Будь ласка, почекайте.'"),
				ИмяФайла, 
				ФайловыеФункцииКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
...
#Если НЕ ВебКлиент Тогда
	...
	НСтр("ru='Файл ""%1"" отсутствует в рабочем каталоге. Получить файл с сервера?';uk='Файл ""%1"" вiдсутнiй у робочому каталозі. Одержати файл із сервера?'"), ИмяФайла),
	...
	КонецЕсли;
#Иначе
	Предупреждение(НСтр("ru='В Веб-клиенте данная функция не поддерживается.';uk='У Веб-клієнтові дана функція не підтримується.'"));
#КонецЕсли
КонецПроцедуры
...
Процедура ОбработкаПеретаскиванияВЛинейныйСписок(ПараметрыПеретаскивания, ВладелецФайлаСписка, ЭтаФорма,
	...
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл") И ПараметрыПеретаскивания.Значение.ЭтоФайл() = Ложь Тогда
		...
		Предупреждение(Нстр("ru='Выберите для импорта только файлы, но не каталоги.';uk='Виберіть для імпорту тільки файли, але не каталоги.'"));
		Возврат;
...
Процедура ОбновитьИзФайлаНаДискеРеализация(ДанныеФайла, ИдентификаторФормы, ДиалогПолноеИмяФайла, 
	...
	Если НРег(РасширениеФайлаНаДиске) = НРег(РасширениеДляЗашифрованныхФайлов) Тогда
		...
		Пароль = "";
		Заголовок = НСтр("ru='Введите пароль для расшифровки';uk='Уведіть пароль для розшифрування'");
...
"Операция прервана.';uk='Файл ""%1"" на сервері має більш пізню дату зміни (%2), чому обраний файл (%3).  Операція перервана.'"), 
...
"Операция прервана.';uk='Файл ""%1"" у робочому каталозі має більш пізню дату зміни (%2), чому обраний файл (%3).  Операція перервана.'"), 
...
"Закончите редактирование перед выполнением обновления из файла на диске.';uk='Файл в основному робочому каталозі ""%1""  відкритий для редагування.  Закінчите редагування перед виконанням відновлення з файлу на диску.'"), 
								   ПолноеИмяФайла);
					Предупреждение(СтрокаОшибки);
					Возврат;
				КонецПопытки;
	...
	Если ФайлВРабочемКаталоге И ИмяИРасширениеФайлаНаДиске <> ИмяИРасширениеФайлаВБазе Тогда
		...
		Исключение
			Инфо = ИнформацияОбОшибке();
			Предупреждение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								  НСтр("ru='Описание=""%1""';uk='Опис=""%1""'"), Инфо.Описание));
...
Функция ОбновитьИзФайлаНаДиске(ДанныеФайла, ИдентификаторФормы) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Фильтр                      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файл (*.%1)|*.%1|Все файлы (*.*)|*.*';uk='Файл (*.%1)|*.%1|Всi файли (*.*)|*.*'"), ДанныеФайла.Расширение, ДанныеФайла.Расширение);
...
Функция ПоказатьЗанятыеФайлыПриЗавершенииРаботы() Экспорт
	...
	Если КоличествоЗанятыхФайлов > 0 Тогда 
		...
		ПараметрыФормы.Вставить("СообщениеВопрос", 		НСтр("ru='Завершить работу с программой?';uk='Завершити роботу із програмою?'"));
		ПараметрыФормы.Вставить("СообщениеЗаголовок", 	НСтр("ru='Следующие файлы заняты вами для редактирования:';uk='Наступні файли зайняті вами для редагування:'"));
		ПараметрыФормы.Вставить("Заголовок", 			НСтр("ru='Завершение работы';uk='Завершення роботи'"));
...
Функция Зашифровать(ФайлСсылка,
	...
	Если КоличествоВерсий > 1 Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сейчас будет выполнено шифрование всех %1 версий файла. Это может занять продолжительное время. Продолжить шифрование файла?';uk='Зараз буде виконане шифрування всіх %1 версій файлу. Це може забрати тривалий час. Продовжити шифрування файлу?'"),
			КоличествоВерсий);
		Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Если Ответ = КодВозвратаДиалога.Нет Тогда
			Возврат Ложь;
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется получение всех версий файла ""%1""';uk='Виконується одержання всіх версій файлу ""%1""'"), ФайлСсылка);
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполняется шифрование всех версий файла ""%1""';uk='Виконується шифрування всіх версій файлу ""%1""'"),
		ФайлСсылка);
...
Функция Расшифровать(ФайлСсылка, УникальныйИдентификаторФормы, ДанныеФайла,
	...
	Если КоличествоВерсий > 1 Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сейчас будет выполнена расшифровка всех %1 версий файла. Это может занять продолжительное время. Продолжить расшифровку файла?';uk='Зараз буде виконано розшифрування всіх %1 версій файлу. Це може забрати тривалий час. Продовжити розшифрування файлу?'"),
			КоличествоВерсий);
		Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Если Ответ = КодВозвратаДиалога.Нет Тогда
			Возврат Ложь;
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполняется получение всех версий файла ""%1""';uk='Виконується одержання всіх версій файлу ""%1""'"),
		ФайлСсылка);
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполняется расшифровка всех версий файла ""%1""';uk='Виконується розшифрування всіх версій файлу ""%1""'"),
		ФайлСсылка);
	Состояние(ТекстСообщения);
...
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления, ВладелецФайла, 
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Файл ""%1"" зашифрован';uk='Файл ""%1"" зашифрований'"),
		ФайлСсылка);
	Состояние(ТекстСообщения);
...
Процедура ИнформироватьОРасшифровке(ВладелецФайла, ФайлСсылка) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Файл ""%1"" расшифрован';uk='Файл ""%1"" розшифрований'"),
		ФайлСсылка);
	Состояние(ТекстСообщения);
...
Процедура ЗанестиИнформациюОПодписях(ФайлСсылка, Владелец, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если МассивДанныхДляЗанесенияВБазу.Количество() = 1 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Добавлена подпись из файла для ""%1""';uk='Доданий підпис із файлу для ""%1""'"),
			ФайлСсылка);
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Удалить его из рабочего каталога?';uk='Файл ""%1%2"" не знайдений на сервері. Вилучити його з робочого каталогу?'"),
					ИмяКаталога, Строка.Путь);
					
					
				Если ОчищатьВсе	= Ложь Тогда
					ТекстВопроса = Нстр("ru='Выполняется очистка рабочего каталога при добавлении файла.';uk='Виконується очищення рабочего каталогу при додаванні файлу.'") + Символы.ПС + Символы.ПС + ТекстВопроса;
				КонецЕсли;	
		...
		ШапкаВопроса = Нстр("ru='Выполняется очистка рабочего каталога при добавлении файла.';uk='Виконується очищення рабочего каталогу при додаванні файлу.'");
...
#Если Не ВебКлиент Тогда
				...
				Если МассивФайловВКаталоге.Количество() <> 0 Тогда
					ТекстОшибки 
						= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В выбранном рабочем каталоге (""%1"") уже есть файлы. Выберите другой каталог.';uk='В обраному робочому каталозі (""%1"") уже є файли. Виберіть інший каталог.'"),
						ИмяКаталога);
					Предупреждение(ТекстОшибки);
					Возврат Ложь;
				КонецЕсли;
...
"Повторить операцию?';uk='Не вдалося скопіювати файл ""%1"". Можливо він зайнятий іншим додатком. Повторити операцію?'"),
...
"Повторить операцию?';uk='Не вдалося вилучити файл ""%1"". Можливо він зайнятий іншим додатком. Повторити операцію?'"),
					Путь);
				Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
				Если Ответ = КодВозвратаДиалога.Нет Тогда
					Возврат Ложь;
...
Функция ПеренестиСодержимоеРабочегоКаталога(ИмяКаталогаПрежнееЗначение, ИмяКаталога) Экспорт
	...
	Если Найти(ИмяКаталога, ИмяКаталогаПрежнееЗначение) <> 0 Тогда
		ТекстСообщения =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выбранный путь к рабочему каталогу (%1) входит в старый путь (%2).';uk='Обраний шлях до робочого каталогу (%1) входить у старий шлях (%2).'"),
	...
	Если НЕ УдалитьСодержимоеКаталога(ОригинальныеФайлыИПапки) Тогда
		...
		Если НЕ СкопироватьСодержимоеКаталога(ИмяКаталога, ИмяКаталогаПрежнееЗначение, СкопированныеФайлыИПапкиВторойПроход, ОригинальныеФайлыИПапкиВторойПроход) Тогда
			...
			ТекстСообщения =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось скопировать обратно содержимое каталога ""%1"" в каталог ""%2"".';uk='Не вдалося скопіювати назад уміст каталогу ""%1"" у каталог ""%2"".'"),
				ИмяКаталога, ИмяКаталогаПрежнееЗначение);
			Предупреждение(ТекстСообщения);
			Возврат Ложь;
...
"%2';uk=""Довжина повного шляху до файлу (шлях робочого каталогу плюс ім'я файлу) перевищує %1 символів %2"""),
					...
					ТекстВопроса = ТекстСообщения + Символы.ВК + Символы.ВК 
					+ НСтр("ru='Выбрать другой путь  к основному рабочему каталогу?';uk='Вибрати інший шлях  до основного робочого каталогу?'");
					Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
					Если Ответ = КодВозвратаДиалога.Нет Тогда
						Возврат Ложь;
					КонецЕсли;	
					...
					// выбираем другой путь к рабочему каталогу
					Заголовок = НСтр("ru='Выберите другой путь к основному рабочему каталогу';uk='Виберіть інший шлях до основного робочого каталогу'");
			...
			Если ВыбранныйФайл.Существует() Тогда
						...
						Если МассивФайлов.Количество() <> 0 Тогда
							...
							Если РаботаСФайламиВызовСервера.ЕстьПапкаСТакимИменем(ИмяФайла, Владелец, ПерваяПапкаСТакимЖеИменем) Тогда
								...
								Иначе	
									...
									ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru='Папка ""%1"" уже существует. Продолжить импорт папки?';uk='Папка ""%1"" уже існує. Продовжити імпорт папки?'"),
													ИмяФайла);
				...
				Если РежимЗагрузки Тогда
					Если РаботаСФайламиВызовСервера.ЕстьФайлСТакимИменем(ИмяБезРасширения, Владелец) Тогда
						...
						Запись.Вставить("Ошибка", НСтр("ru='Файл с таким именем уже есть в информационной базе';uk='Файл із таким іменем уже є в інформаційній базі'"));
				...
				Если Не ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, ИдентификаторФормы) Тогда
					ВызватьИсключение
					  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка помещения файла в хранилище: %1';uk='Помилка приміщення файлу в сховище: %1'"), ВыбранныйФайл.ПолноеИмя);
				КонецЕсли;
...
Функция ИмпортФайловВыполнить(
	...
	Если КоличествоСуммарное = 0 Тогда
		...
		Если Не РежимЗагрузки Тогда
			Предупреждение(НСтр("ru='Нет файлов для добавления';uk='Немає файлів для додавання'"));
		КонецЕсли;
	...
	Если МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Импорт файлов завершен. Импортировано %1 файлов';uk='Імпорт файлів завершений. Імпортоване %1 файлів'"), Строка(МассивСтруктурВсехФайлов.Количество()) );
			
		Если РежимЗагрузки Тогда
			ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Загрузка файлов завершена. Загружено %1 файлов';uk='Завантаження файлів завершене. Завантажене %1 файлів'"), Строка(МассивСтруктурВсехФайлов.Количество()) );
		КонецЕсли;
...
"Пожалуйста, подождите.';uk='Іде збір інформації про каталог ""%1"".   Будь ласка, почекайте.'"), Путь ));
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = Строка(Файл) + " (" + Строка(РазмерФайлаВМб) + " " + НСтр("ru='МБ)';uk='МБ)'");
...
Функция МожноЗакрытьФормуСФайлами(ОбъектСсылка) Экспорт
	...
	Если Количество <> 0 Тогда
		...
		Результат = Вопрос(
			НСтр("ru='Один или несколько файлов заняты Вами для редактирования. Продолжить?';uk='Один або кілька файлів зайняті Вами для редагування. Продовжити?'"),
			РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Функция ВыбратьСертификатыДляШифрования(ДанныеФайла) Экспорт
    ...
    Если ОтпечатокЛичногоСертификатаДляШифрования = Неопределено ИЛИ ПустаяСтрока(ОтпечатокЛичногоСертификатаДляШифрования) Тогда
        ...
        Если ТипЗнч(СтруктураВозврата) = Тип("Структура") Тогда
            ...
            ОбновитьПовторноИспользуемыеЗначения();
        Иначе
            Предупреждение(НСтр("ru='Не выбран персональный сертификат для шифрования.';uk='Не обраний персональний сертифікат для шифрування.'"));
            Возврат Ложь;
        КонецЕсли;    
    КонецЕсли;    
...
Процедура ПриЗавершенииРаботыСистемы(Предупреждения) Экспорт
	...
	ТекстГиперСсылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Открыть список редактируемых файлов (%1)';uk='Відкрити список файлів, що редагуються (%1)'"), 
