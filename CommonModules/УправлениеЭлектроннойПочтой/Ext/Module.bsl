...
Процедура ПолучениеИОтправкаЭлектронныхПисем() Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru='Начато регламентное получение и отправка электронной почты';uk='Розпочато регламентне отримання і відправку електронної пошти'"));
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Профиль = ЭлектроннаяПочта.ИнтернетПочтовыйПрофиль(ВыборкаУчетныеЗаписи.УчетнаяЗапись);
		Попытка
...
"%2';uk='Під час підключення до облікового запису %1 сталася помилка"
...
"%2';uk='Під час відправки електронного листа %1 сталася помилка %2'"), ВыборкаПисьма.ПредставлениеПисьма, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Во время подключения к учетной записи %1 произошла ошибка"
"%2';uk='Під час підключення до облікового запису %1 сталася помилка %2'"), ВыборкаУчетныеЗаписи.УчетнаяЗапись,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Закончено регламентное получение и отправка электронной почты';uk='Закінчено регламентне отримання і відправку електронної пошти'"));
...
Процедура ЗагрузитьПочтуПользователя(ПолученоПисем, ДоступноУчетныхЗаписей, ЕстьОшибки) Экспорт
	...
	Если ДоступноУчетныхЗаписей = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет доступных для получения почты учетных записей';uk='Немає доступних для отримання пошти облікових записів'"));
		Возврат;
	КонецЕсли;
...
"%2'"), ВыборкаУчетныеЗаписи.УчетнаяЗапись,КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Пока ВыборкаПисьма.Следующий() Цикл
			...
			Исключение
				ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Во время отправки электронного письма %1 произошла ошибка"
"%2';uk='Під час відправки електронного листа %1 сталася помилка %2'"), ВыборкаПисьма.ПредставлениеПисьма,КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Пока ВыборкаУчетныеЗаписи.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Во время подключения к учетной записи %1 произошла ошибка"
"%2';uk='Під час підключення до облікового запису %1 сталася помилка %2'"), ВыборкаУчетныеЗаписи.УчетнаяЗапись,КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПолучитьПисьма(
	...
	Профиль = ЭлектроннаяПочта.ИнтернетПочтовыйПрофиль(УчетнаяЗапись);
	...
	Исключение
		...
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Во время подключения к учетной записи %1 произошла ошибка"
"%2';uk='Під час підключення до облікового запису %1 сталася помилка %2'"), УчетнаяЗапись,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"%4';uk='При отриманні листа %1 від %2, відправлене з адреси %3 виникла помилка"
"%4'"),
					Сообщение.Тема,
	...
	Если ЕстьПодписи Или ЭтоОтображаемыйФайл Тогда
		...
		Если ЕстьПодписи Тогда
			...
			Для Каждого ПодписьВложения Из МассивПодписей Цикл
				...
				ЭЦП.Комментарий = НСтр("ru='Вложение электронного письма';uk='Вкладення електронного листа'");
...
"%2';uk='Під час регламентної відправки повідомлення про прочитання електронного листа %1 сталася помилка"
...
"%2';uk='Під час відправки повідомлення про прочитання електронного листа %1 сталася помилка %2'"), Выборка.ПредставлениеПисьма,КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Взаимодействия';uk='Взаємодії'");
...
Процедура СоздатьПредопределенныеПапкиЭлектронныхПисемДляУчетнойЗаписи(УчетнаяЗапись) Экспорт
	...
	МассивИменПредопределенныхПапок.Добавить(НСтр("ru='Удаленные';uk='Видалені'"));
