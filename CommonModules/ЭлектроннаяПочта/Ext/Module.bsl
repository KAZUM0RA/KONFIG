...
Функция ЗагрузитьСообщения(Знач УчетнаяЗапись,
	...
	Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(УчетнаяЗапись, "ПротоколВходящейПочты") = "IMAP" Тогда
...
Процедура ЗаполнитьНовыеРеквизитыУчетныхЗаписей() Экспорт
	...
	|	УчетныеЗаписиЭлектроннойПочты.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА УчетныеЗаписиЭлектроннойПочты.ПользовательSMTP = """"
	|			ТОГДА УчетныеЗаписиЭлектроннойПочты.Пользователь
	|		ИНАЧЕ УчетныеЗаписиЭлектроннойПочты.ПользовательSMTP
	|	КОНЕЦ КАК ПользовательSMTP,
	|	ВЫБОР
	|		КОГДА УчетныеЗаписиЭлектроннойПочты.ПарольSMTP = """"
	|			ТОГДА УчетныеЗаписиЭлектроннойПочты.Пароль
	|		ИНАЧЕ УчетныеЗаписиЭлектроннойПочты.ПарольSMTP
	|	КОНЕЦ КАК ПарольSMTP
	|ИЗ
	|	Справочник.УчетныеЗаписиЭлектроннойПочты КАК УчетныеЗаписиЭлектроннойПочты
	|ГДЕ
	...
	Если НастройкиУчетнойЗаписи.ИспользоватьДляОтправки Тогда
		...
		Исключение
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Ошибка при отправке сообщения: %1';uk='Помилка при відправленні повідомлення: %1'"),
		...
		Исключение
			...
			СообщениеОбОшибке = СообщениеОбОшибке
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Ошибка доступа к серверу входящих сообщений: %1';uk='Помилка доступу до сервера вхідних повідомлень: %1'"),
										КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) );
		КонецПопытки;
...
Процедура ПроверитьВозможностьОтправкиТестовогоСообщения(Знач УчетнаяЗапись, Знач Пароль = Неопределено)
	...
	ПараметрыПисьма.Вставить("Тема", НСтр("ru='Тестовое сообщение 1С:Предприятие';uk='Тестове повідомлення 1С:Підприємство'"));
	ПараметрыПисьма.Вставить("Тело", НСтр("ru='Это сообщение отправлено подсистемой электронной почты Учет в ОСМД';uk='Це повідомлення відправлене підсистемою електронної пошти Облік в ОСББ'"));
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("ru='Проверка учетной записи электронной почты';uk='Перевірка облікового запису електронної пошти'");
КонецФункции
