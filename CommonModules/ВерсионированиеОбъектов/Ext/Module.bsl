...
Функция ПолучитьИнформациюПоВерсии(знач Ссылка, знач НомерВерсии) Экспорт
	...
	Если ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не удалось получить версию объекта из информационной базы. Выполнение операции прервано.';uk=""Не вдалося одержати версію об'єкта з інформаційної бази. Виконання операції перерване.""");
	КонецЕсли;
