...
Функция ПолучитьФормуВыполненияЗадачи(Знач ЗадачаСсылка) Экспорт
	...
	Если ТипЗнч(ЗадачаСсылка) <> Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный тип параметра ЗадачаСсылка (передан: %1; ожидается: %2)';uk='Неправильний тип параметра ЗадачаСсылка (переданий: %1; очікується: %2)'"),
...
Функция ПеренаправитьЗадачи(Знач МассивЗадач, Знач ИнфоОПеренаправлении, Знач ТолькоПроверка = Ложь,
	...
	Для Каждого Задача Из МассивЗадач Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи';uk='Бізнес-Процеси й завдання'"), 
...
Процедура СделатьАктивнымБизнесПроцессы(БизнесПроцессы) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции';uk='Виконання операції'"), УровеньЖурналаРегистрации.Ошибка,,, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
 		ВызватьИсключение;		
	КонецПопытки;	
...
Процедура СделатьАктивнымБизнесПроцесс(БизнесПроцесс) Экспорт
	...
	Если Не ЕстьПраваНаОстановкуБизнесПроцесса(БизнесПроцесс) Тогда 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		  НСтр("ru='Вашего уровня прав недостаточно для того, чтобы сделать активным бизнес-процесс ""%1"".';uk='Вашого рівня прав недостатньо для того, щоб зробити активним бізнес-процес ""%1"".'"),
...
Процедура ОстановитьБизнесПроцессы(БизнесПроцессы) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции';uk='Виконання операції'"), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;		
	КонецПопытки;	
...
Процедура ОстановитьБизнесПроцесс(БизнесПроцесс) Экспорт
	...
	Если Не ЕстьПраваНаОстановкуБизнесПроцесса(БизнесПроцесс) Тогда 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		  НСтр("ru='Вашего уровня прав недостаточно для остановки бизнес-процесса ""%1"".';uk='Вашого рівня прав недостатньо для зупинки бізнес-процесу ""%1"".'"),
		  Строка(БизнесПроцесс));
		ВызватьИсключение ТекстСообщения;	
	КонецЕсли;	
	...
	Если Объект.Завершен = Истина Тогда
		ВызватьИсключение НСтр("ru='Нельзя останавливать завершенные бизнес-процессы.';uk=' Не можна зупиняти завершені бізнеси-процеси.'");
	КонецЕсли;
		
	Если Объект.Стартован <> Истина Тогда
		ВызватьИсключение НСтр("ru='Нельзя останавливать не стартовавшие бизнес-процессы.';uk=' Не можна зупиняти бізнеси-процеси, що не стартували.'");
	КонецЕсли;
...
Процедура УведомитьИсполнителейОНовыхЗадачах() Экспорт
	...
	ЗаписьЖурналаРегистрации(БизнесПроцессыИЗадачиСервер.СобытиеЖурналаРегистрации(),
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Начато регламентное уведомление о новых задачах за период %1 - %2';uk='Почате регламентне повідомлення про нові завдання за період %1 - %2'"),
	...
	ЗаписьЖурналаРегистрации(БизнесПроцессыИЗадачиСервер.СобытиеЖурналаРегистрации(),
		УровеньЖурналаРегистрации.Информация, , ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Завершено регламентное уведомление о новых задачах (уведомлено исполнителей: %1)';uk='Завершене регламентне повідомлення про нові завдання (сповіщене виконавців: %1)'"),
...
Процедура ПроконтролироватьЗадачи() Экспорт
	...
	Для Каждого ПисьмоИзНабора Из НаборПисемПоАдресатам Цикл
		...
		Если ПустаяСтрока(ПисьмоИзНабора.ПочтовыйАдрес) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Уведомление не было отправлено, так как у пользователя %1 не задан адрес электронной почты.';uk='Повідомлення не було відправлено, тому що в користувача %1 не задана адреса електронної пошти.'"), 
				ПисьмоИзНабора.Получатель);
			ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о просроченных задачах';uk='Бізнес-Процеси й завдання.Повідомлення про прострочені завдання'"), 
...
" "
"%1';uk=' У Вас є не виконані в строк завдання:   %1'"), ПисьмоИзНабора.ТекстПисьма);
			ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
			
			ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У Вас имеются не выполненные в срок задачи (%1)';uk=' У Вас є не виконані в строк завдання (%1)'"),
...
" "
"%1';uk=' По створених Вами завданням пройшов крайній строк:   %1'"), ПисьмоИзНабора.ТекстПисьма);
			ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
			
			ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По Вашим задачам истек контрольный срок (%1)';uk=' По Ваших завданнях минув контрольний строк (%1)'"),
				Строка(ПисьмоИзНабора.КоличествоЗадач));
			ПараметрыПисьма.Вставить("Тема", ТекстТемыПисьма);
...
" "
"%1';uk='Пройшов крайній строк по завданнях:   %1'"), ПисьмоИзНабора.ТекстПисьма);
			ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
			
			ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Истек контрольный срок задач (%1)';uk='Минув контрольний строк завдань (%1)'"),
				Строка(ПисьмоИзНабора.КоличествоЗадач));
			ПараметрыПисьма.Вставить("Тема", ТекстТемыПисьма);
		КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при отправке уведомления о просроченных задачах: %1.';uk='Помилка при відправленні повідомлення про прострочені завдання: %1.'"),
				ОписаниеОшибки);
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Уведомление о просроченных задачах успешно отправлено на адрес %1.';uk='Повідомлення про прострочені завдання успішно відправлене на адресу %1.'"),
								Кому);
...
"%1';uk='Завдання не може бути виконана, тому що в ролі не задане жодного виконавця: %1'"), Строка(ЗадачаСсылка));
		
		ЗадачаОбъект.Наименование =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		  НСтр("ru='Назначить исполнителей: задача не может быть исполнена %1';uk='Призначити виконавців: завдання не може бути виконана %1'"), Строка(ЗадачаСсылка));
...
Функция ОтправитьУведомлениеОНовыхЗадачах(Исполнитель, ЗадачиПоИсполнителю)
	...
	Если ПустаяСтрока(ПочтовыйАдресПолучателя) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-Процеси й завдання.Повідомлення про нові завдання'"), 
			УровеньЖурналаРегистрации.Информация,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Уведомление не отправлено, так как не указан почтовый адрес у пользователя %1.';uk='Повідомлення не відправлене, тому що не зазначена поштова адреса в користувача %1.'"), Строка(Исполнитель)));
		Возврат Ложь;
	КонецЕсли;
	...
	ТемаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		  НСтр("ru='Вам направлены задачи - %1';uk='Вам спрямовані завдання - %1'"), Метаданные.КраткаяИнформация);
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-Процеси й завдання.Повідомлення про нові завдання'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			   НСтр("ru='Ошибка при отправке уведомления о новых задачах: %1';uk='Помилка при відправленні повідомлення про нові завдання: %1'"), 
			   ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		Возврат Ложь;
	КонецПопытки;
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-Процеси й завдання.Повідомлення про нові завдання'"), 
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Уведомления успешно отправлены на адрес %1.';uk='Повідомлення успішно відправлені на адресу %1.'"), ПочтовыйАдресПолучателя));
	Возврат Истина;	
...
"Крайній строк: %2'") + Символы.ПС;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Исполнитель) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru='Исполнитель: %3';uk='Виконавець: %3'") + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.РольИсполнителя) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru='Роль: %4';uk='Роль: %4'") + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ОсновнойОбъектАдресации) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru='Основной объект адресации: %5';uk=""Основний об'єкт адресації: %5""") + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ДополнительныйОбъектАдресации) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru='Доп. объект адресации: %6';uk=""Доп. об'єкт адресації: %6""") + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Автор) Тогда
		ШаблонСтроки = ШаблонСтроки + НСтр("ru='Автор: %7';uk='Автор: %7'") + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Описание) Тогда
		ШаблонСтроки = ШаблонСтроки + Символы.ПС + НСтр("ru='%8';uk='8%'") + Символы.ПС;
	КонецЕсли;
