...
Функция ЗаполнитьСтруктуруСертификатаПоОтпечатку(Отпечаток) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		Предупреждение(НСтр("ru='Сертификат не найден';uk='Сертифікат не знайдений'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьМенеджерКриптографии(Отказ = Ложь) Экспорт
	...
	Если ПустаяСтрока(ПровайдерЭЦП) Тогда
		СтрокаОшибки = НСтр("ru='Не указан провайдер ЭЦП. В настройках криптографии укажите провайдера ЭЦП, тип провайдера, алгоритмы подписи и хеширования.';uk='Не указан провайдер ЭЦП. В настройках криптографии укажите провайдера ЭЦП, тип провайдера, алгоритмы подписи и хеширования.'");
...
Процедура СохранитьПодписи(ФайлСсылка, ПолноеИмяФайла, УникальныйИдентификатор, МассивСтруктурПодписей) Экспорт
	...
	Если МассивСтруктурПодписей.Количество() <> 0 Тогда
		Текст = НСтр("ru='Каталог:';uk='Каталог:'") + Символы.ПС;
		Текст = Текст + Путь;
...
Процедура СохранитьПодпись(АдресПодписи) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Все файлы (*.%1)|*.%1';uk='Усі файли (*.%1)|*.%1'"), РасширениеДляФайловПодписи, РасширениеДляФайловПодписи);
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Подпись сохранена в файл ""%1""';uk='Підпис збережений у файл ""%1""'"),
