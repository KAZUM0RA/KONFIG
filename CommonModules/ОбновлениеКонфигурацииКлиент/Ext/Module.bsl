...
Процедура ПроверитьОбновлениеКонфигурации() Экспорт
	...
	Если НЕ НаличиеОбновления И НастройкиОбновленияКонфигурации <> Неопределено
		...
		Если НастройкиОбновленияКонфигурации.ПроверятьНаличиеОбновленияПриЗапуске = 2 Тогда
			...
			ЕстьДоступноеОбновлениеВИнтернете = Параметры.ИсточникОбновления <> -1 
	...
	Если НастройкиОбновления.КонфигурацияИзменена И НастройкиОбновления.ЕстьДоступДляПроверкиОбновления Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Обновление конфигурации';uk='Відновлення конфігурації'"),
	...
	Если ЕстьДоступноеОбновлениеВИнтернете И НастройкиОбновления.ЕстьДоступДляПроверкиОбновления Тогда
		...
		Параметры.НуженФайлОбновления = НастройкиОбновленияКонфигурации.НуженФайлОбновления;
		Параметры.ФлагАвтоПереходаНаСтраницуСОбновлением = Истина;
		ПоказатьОповещениеПользователя(НСтр("ru='Доступно обновление конфигурации';uk='Доступне відновлення конфігурації'"),
			"e1cib/app/Обработка.ОбновлениеКонфигурации",
...
Функция ПолучитьПараметрыОбновления() Экспорт
	...
	СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации"		, НСтр("ru='Обновление информационной базы';uk='Відновлення інформаційної бази'"));
...
Процедура ПроверитьНаличиеОбновленияЧерезИнтернет(ВыдаватьСообщения = Ложь) Экспорт
	...
	Если ТипЗнч(Параметры.ПараметрыФайлаПроверкиОбновления) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Предупреждение",
			НСтр("ru='Невозможно подключиться к сети Интернет для проверки обновлений.';uk='Неможливо підключитися до мережі Інтернет для перевірки відновлень.'"));
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияКонфигурации, Параметры.ПоследняяВерсияКонфигурации) >= 0 Тогда  
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
			НСтр("ru='Обновление не требуется: последняя версия конфигурации уже установлена.';uk='Відновлення не потрібно: остання версія конфігурації вже встановлена.'"));
	...
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
		НСтр("ru='Обнаружена более новая версия конфигурации в Интернете: ';uk='Виявлена більш нова версія конфігурації в Інтернеті: '") + 
...
Процедура ПроверитьОбновлениеПоРасписанию() Экспорт
	...
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),, 
		НСтр("ru='Проверка наличия обновления в интернете по расписанию.';uk='Перевірка наявності відновлення в інтернеті за розкладом.'"));
	...
	Если Параметры.ИсточникОбновления <> -1 И Параметры.ИмяСтраницы = НаименованиеСтраницыДоступногоОбновления Тогда
		...
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),, 	
			НСтр("ru='Обнаружена новая версия конфигурации: ';uk='Виявлена нова версія конфігурації: '") + 
	...
	Иначе
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),, 
			НСтр("ru='Доступных обновлений не обнаружено.';uk='Доступних відновлень не виявлене.'"));
	КонецЕсли;
	ОбновлениеКонфигурации.ЗаписатьСтруктуруНастроекПомощника(НастройкиОбновленияКонфигурации, СообщенияДляЖурналаРегистрации);
...
"%1';uk='Не вдалося створити тимчасовий каталог для перевірки наявності відновлень. %1'");
...
"%1';uk='Не вдалося перевірити наявність відновлень. %1'"), Результат.СообщениеОбОшибке);
		Если ВыдаватьСообщения Тогда
			Предупреждение(СообщениеОбОшибке);
		КонецЕсли;
		Возврат СообщениеОбОшибке;
...
"(источник: %2; приемник: %3)';uk='Помилка при копіюванні: %1 (джерело: %2; приймач: %3)'"), 
...
Функция ПолучитьПараметрыРелизовНаДискеИТС(ДискИТС, Знач ВыдаватьСообщения = Истина) Экспорт
	...
	Если ТипЗнч(ПараметрыРелизовНаДискеИТС) <> Тип("Структура") Тогда // ошибки при чтении файла
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru='Обновления на диске ИТС не найдены.';uk='Обновления на диске ИТС не найдены.'"));
		КонецЕсли;
		Возврат Неопределено;		
   	КонецЕсли;
...
Функция ПараметрыДистрибутива(Знач ИмяФайла)
	...
	Если НЕ Файл.Существует() Тогда
		Возврат НСтр("ru='Файл описания обновлений не получен';uk='Файл опису відновлень не отриманий'");
	КонецЕсли;	
	...
	Для Н = 1 По ТД.КоличествоСтрок() Цикл
		...
		Иначе
			Возврат НСтр("ru='Неверный формат сведений о наличии обновлений';uk='Невірний формат відомостей про наявність відновлень'");
		КонецЕсли;
	КонецЦикла;
	Возврат ПарКомпл;
КонецФункции
...
Функция ПрочитатьАдресаРелизовНаДискеИТС(Знач ИмяФайла, Знач ДискИТС)
	...
	Исключение
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", 
			НСтр("ru=' ПрочитатьАдресаРелизовНаДискеИТС: ';uk=' ПрочитатьАдресаРелизовНаДискеИТС: '") + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Неопределено;
	КонецПопытки;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ru='Обновление конфигурации';uk='Відновлення конфігурації'");
КонецФункции
