...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТипАдресации = 0 Тогда
		Если Исполнитель.Пустая() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не указан исполнитель задачи.';uk='Не зазначений виконавець завдання.'"),,,
				"Исполнитель",
				Отказ);
		КонецЕсли;
		Возврат;
	...
	Если Роль.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не указана роль исполнителей задачи.';uk='Не зазначена роль виконавців завдання.'"),,,
			"Роль",
			Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ИгнорироватьПредупреждения И 
		НЕ БизнесПроцессыИЗадачиСервер.ЕстьИсполнителиРоли(Роль, ОсновнойОбъектАдресации, ДополнительныйОбъектАдресации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='На указанную роль не назначено ни одного исполнителя. (Чтобы проигнорировать это предупреждение, установите флажок.)';uk='На зазначену роль не призначене жодного виконавця. (Щоб проігнорувати це попередження, установите прапорець.)'"),,,
			"Роль",
			Отказ);
