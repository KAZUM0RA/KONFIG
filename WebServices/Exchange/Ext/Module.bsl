...
"КаталогВременныхФайловСообщенийОбменаДляLinux и КаталогВременныхФайловСообщенийОбменаДляWindows.';uk='Не знайдений фрагмент %1 сесії передачі з ідентифікатором %2.  Необхідно переконатися, що у формі настроювання програми задані значення констант КаталогВременныхФайловСообщенийОбменаДляLinux і КаталогВременныхФайловСообщенийОбменаДляWindows.'"
			);
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбменДаннымиСервер.ЗарегистрироватьДанныеДляНачальнойВыгрузки",
											Параметры,
											,
											НСтр("ru='Создание обмена данными.';uk='Створення обміну даними.'")
		);
