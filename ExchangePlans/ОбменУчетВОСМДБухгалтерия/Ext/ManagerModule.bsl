...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ru='Настройки обмена для ОСМД-БП';uk='Настроювання обміну для ОСББ-БП'");
...
Функция НастройкаОтборовНаУзле() Экспорт
	...
	СтруктураНастроек.Вставить("РежимВыгрузкиОрганизаций",			Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию);
	СтруктураНастроек.Вставить("ВыгружатьПлатежныеДокументы",		Ложь);
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле) Экспорт
	...
	Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
		...
		ОграничениеОтборПоОрганизациям = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Только по организациям: %1';uk='Тільки по організаціях: %1'"),
	...
	ОграничениеОтборПоОрганизациямРежим = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Организации %1';uk='Організації %1'"),
		НРег(НастройкаОтборовНаУзле.РежимВыгрузкиОрганизаций)
	);
	
	Возврат (
		НСтр("ru='Выгружать документы и справочную информацию:';uk='Вивантажувати документи й довідкову інформацію:'")
		+ Символы.ПС
		+ ОграничениеДатаНачалаВыгрузкиДокументов
		+ Символы.ПС
		+ ОграничениеОтборПоОрганизациям
		+ Символы.ПС
		+ ОграничениеОтборПоОрганизациямРежим
	);
КонецФункции
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Создать обмен с Бухгалтерией для Украины ред. 1.2';uk='Створити обмін з Бухгалтерією для України ред. 1.2'");
