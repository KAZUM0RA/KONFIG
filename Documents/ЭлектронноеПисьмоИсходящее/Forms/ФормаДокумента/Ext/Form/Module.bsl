...
Процедура СписокПолучателейПисьмаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.СтатусПисьма <> ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Отправлено") Тогда
		...
		ИначеЕсли Элемент.Имя = "СписокПолучателейСкрытыхКопий" Тогда
			ГруппаПодбора = НСтр("ru='Скрытые';uk='Приховані'");
		КонецЕсли;
...
Процедура ОтправитьПереслатьВыполнить(Команда)
	...
	Иначе
		...
		Если Объект.ПолучателиПисьма.Количество() = 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо указать хотя бы одного получателя письма.';uk='Необхідно вказати хоч би одного одержувача листа.'"),, "Объект.СписокПолучателейПисьма");
			Возврат;
...
Процедура ПрикрепитьВходящееПисьмоОснованиеКакВложениеЕслиНеобходимо(ТекущийОбъект)
	...
	Если ТекущийОбъект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Исходящее И
		...
		НСтр("ru='Пересылаемое сообщение';uk='Повідомлення, що пересилається'"),
		"html",
		ТекущаяДатаСеанса(),
		ТекущаяДатаСеанса(),
...
Процедура СвойстваВложения(Команда)
	...
	Если ТекущиеДанные.Ссылка = Неопределено Тогда
		Результат = Вопрос(НСтр("ru='Свойства файла доступны только после его записи. Записать?';uk='Властивості файлу доступні тільки після його запису. Записати?'"),РежимДиалогаВопрос.ДаНет);
...
Функция СформироватьИсходящееОбычныйТекст(ВыборкаДанныеВходящегоПисьма, ТекущийОбъект)
	...
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru='Отправлено: ';uk='Відправлено:'") + Формат(ВыборкаДанныеВходящегоПисьма.Дата,"ДФ='dd MMMM yyyy'");
...
&НаКлиенте
Процедура ОтправитьВыполнить() Экспорт
	...
	Иначе
		...
		Если НайденныеСтроки[0].УдалятьПослеОтправки Тогда
			...
			СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Отправить';uk='Посилач '"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Отправить и сохранить';uk='Відправити і зберегти'"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Отмена';uk='Відміна'"));

			Результат = Вопрос(НСтр("ru='Для данной учетной записи не предусмотрено сохранение отправленных писем в системе."
"Продолжить?';uk='Для цього облікового запису не передбачено збереження відправлених листів в системі."
"Продовжити?'"),
			                   СписокКнопок,, КодВозвратаДиалога.Да,
			                   НСтр("ru='Отправка письма';uk='Відправка листа'"));
...
Процедура УстановитьЗаголовокКнопкиПоУмолчанию()
	...
	ИначеЕсли Объект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Исходящее Тогда
		Элементы.Отправить.Заголовок = Нстр("ru='Отправить сейчас';uk='Відправити зараз'");
	ИначеЕсли Объект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Черновик Тогда
		...
		Элементы.Отправить.Заголовок = Нстр("ru='Отправить';uk='Посилач '");
