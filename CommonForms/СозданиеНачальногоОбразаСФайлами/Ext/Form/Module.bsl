...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Узел = Менеджер.ЭтотУзел() Тогда
		...
		ВызватьИсключение НСтр("ru='Создание начального образа для данного узла невозможно.';uk='Створення початкового образа для даного вузла неможливо.'");
	...
	Если СтандартныеПодсистемыВызовСервераПереопределяемый.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая Тогда
		...
		Элементы.ПолноеИмяФайловойБазы.Заголовок = НСтр("ru='Для сервера 1С:Предприятия под управлением Microsoft Windows';uk=' Для сервера 1С:Підприємства під керуванням Microsoft Windows'"); 
		Элементы.ПутьКАрхивуСФайламиТомов.Заголовок = НСтр("ru='Для сервера 1С:Предприятия под управлением Microsoft Windows';uk=' Для сервера 1С:Підприємства під керуванням Microsoft Windows'");
...
Процедура ВидБазыПриИзменении(Элемент)
	...
	Иначе
		Элементы.ПутьКАрхивуСФайламиТомов.Заголовок = НСтр("ru='Для сервера 1С:Предприятия под управлением Microsoft Windows, (вида ""\\имя сервера\resource\files.zip"")';uk=' Для сервера 1С:Підприємства під керуванням Microsoft Windows, (виду ""\\ім''я сервера\resource\files.zip"")'");
		Элементы.ПутьКАрхивуСФайламиТомов.КнопкаВыбора = Ложь;
	КонецЕсли;	
...
Процедура СоздатьНачальныйОбраз(Команда)
	...
	Состояние(НСтр("ru='Обмен данными';uk='Обмін даними'"), ,
						 НСтр("ru='Осуществляется создание начального образа...';uk='Здійснюється створення початкового образа...'"),
						 БиблиотекаКартинок.СоздатьНачальныйОбраз);
	
	Если ВидБазы = 0 Тогда
		
		Если НЕ МожноСоздатьФайловуюБазу Тогда
			ВызватьИсключение НСтр("ru='Создание начального образа файловой информационной базы на данной платформе не поддерживается.';uk='Створення початкового образа файлової інформаційної бази на даній платформі не підтримується.'");
		КонецЕсли;
	...
	Предупреждение(НСтр("ru='Создание начального образа успешно завершено.';uk='Створення початкового образа успішно завершене.'"));
	Закрыть();
