...
"только в форме объекта-владельца.';uk=""Список приєднаних файлів можна подивитися"
"тільки у формі об'єкту-власника.""");
	КонецЕсли;
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	ИначеЕсли Элементы.ФормаСоздать.Видимость
	        И Элементы.ФормаСоздать.Доступность Тогда
...
Функция ПроверитьДействиеРазрешено(Знач ТекущееДействие = "")
	...
	Если ТекущееДействие = "ПометкаУдаления" И ТекущиеДанные.ФайлРедактируется Тогда
		...
		Если НЕ ТекущиеДанные.ФайлРедактируетТекущийПользователь Тогда
			ТекстПредупреждения = НСтр("ru='Действие недоступно, так как файл занят для редактирования.';uk='Дія недоступна, оскільки файл зайнятий для редагування.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Действие недоступно, так как файл занят для редактирования"
"другим пользователем.';uk='Дія недоступна, оскільки файл зайнятий для редагування"
"іншим користувачем.'");
		КонецЕсли;
	...
	Если ТипЗнч(Элементы.Список.ТекущаяСтрока) = ТипСправочникаСФайлами Тогда
		Возврат Истина;
	Иначе
		Предупреждение(НСтр("ru='Действие недоступно для строки группировки списка.';uk='Дія недоступна для рядка угрупування списку.'"));
		Возврат Ложь;
	КонецЕсли;
