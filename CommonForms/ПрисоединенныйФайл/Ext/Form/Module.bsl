...
"""%1""?';uk='Для виконання дії вимагається записати зміни файлу."
"Записати зміни і зняти позначку на видалення з файлу"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru='Для выполнения действия требуется записать изменения файла."
"Записать изменения и пометить на удаление файл"
"""%1""?';uk='Для виконання дії вимагається записати зміни файлу."
"Записати зміни і помітити на видалення файл"
"""%1""?'");
		КонецЕсли;
	Иначе
		Если ЭтаФорма.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru='Снять пометку на удаление с файла"
"""%1""?';uk='Зняти позначку на видалення з файлу"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru='Пометить на удаление файл"
"""%1""?';uk='Помітити на видалення файл"
"""%1""?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьЭЦП(Команда)
	...
	Ответ = Вопрос(НСтр("ru='Удалить выделенные подписи?';uk='Видалити виділені підписи?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПроверитьОднуПодпись(ДанныеСтроки, МенеджерКриптографии, ДвоичныеДанныеФайла)
	...
	Попытка
		...
		ДанныеСтроки.Статус = НСтр("ru='Верна';uk='Верна'");
...
Процедура ЗаполнитьСписокПодписей()
	...
	ТекстЗаголовка = НСтр("ru='ЭЦП';uk='ЕЦП'");
	...
	Исключение
		Если ТранзакцияАктивна Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Присоединенный файл';uk='Приєднаний файл'"),
