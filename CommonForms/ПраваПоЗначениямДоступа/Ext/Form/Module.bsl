...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = УправлениеДоступом.ЗаголовокПодчиненнойФормы(НСтр("ru='Права доступа: %1 (%2)';uk='Права доступу: %1 (%2)'"), СсылкаНаОбъект);
		...
		Если ВидыДоступа.Количество() <> 1 ИЛИ НЕ УправлениеДоступом.СвойстваВидаДоступа(ВидыДоступа[0]).ВидДоступаЧерезПраваПоЗначениямДоступа Тогда
			ВызватьИсключение(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Для значений доступа ""%1"" не найдено вида доступа через права по значениям доступа.';uk=' Для значень доступу ""%1"" не знайдене виду доступу через права за значеннями доступу.'"),
	...
	Иначе
		...
		Если Элементы.ВидДоступа.СписокВыбора.Количество() = 0 Тогда
			ВызватьИсключение(НСтр("ru='Не найдено видов доступа через права по значениям доступа.';uk='Не знайдене видів доступу через права за значеннями доступу.'"));
		КонецЕсли;
	...
	Для каждого ТекущаяСтрока Из ГруппыПрав Цикл
		...
		Если НетЗаполненногоПрава Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("ru='Необходимо заполнить не менее одного права доступа.';uk=' Необхідно заповнити не менш одного права доступу.'"),
				"ГруппыПрав",
				НомерСтроки,
				НСтр("ru='В строке %1 необходимо заполнить не менее одного права доступа.';uk='У рядку %1 необхідно заповнити не менш одного права доступу.'"));
		КонецЕсли;
...
Процедура ВидДоступаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		Предупреждение(НСтр("ru='Данные изменены. Запишите или отмените изменения.';uk='Дані змінені. Запишіть або скасуєте зміни.'"));
...
Процедура Записать(Команда)
	...
	Если НЕ Отказ И НЕ РазрешеноУправлениеПравамиПослеЗаписи Тогда
		Предупреждение(ОписаниеПредупреждения, 7, НСтр("ru='После успешной записи';uk='Після успішного запису'"));
		Закрыть();
	КонецЕсли;
...
Процедура Перечитать(Команда)
	...
	Если НЕ Модифицированность ИЛИ Вопрос(НСтр("ru='Данные изменены. Прочитать без сохранения?';uk='Дані змінені. Прочитати без збереження?'"), РежимДиалогаВопрос.ОКОтмена, 5, КодВозвратаДиалога.Отмена) = КодВозвратаДиалога.ОК Тогда
		ПрочитатьПрава();
	КонецЕсли;
...
Процедура ДобавитьРеквизитыЭлементыФормы(НовыеРеквизиты = Неопределено)
		...
		Если НовыеРеквизиты <> Неопределено Тогда
			ДобавитьРеквизит(
				...
				Новый РеквизитФормы(
					...
					"ГруппыПрав",
					НСтр("ru='Пользователь';uk='Користувач'")),
		...
		Если НовыеРеквизиты <> Неопределено Тогда
			...
			ДобавитьРеквизит(
				...
				Новый РеквизитФормы(
					...
					"ГруппыПрав",
					НСтр("ru='Наследуется от';uk='Успадковується від'")),
				НСтр("ru='<не наследуется>';uk='<не успадковується>'"));
		Иначе
			...
			Элемент.Подсказка      = НСтр("ru='Наследуется от (значения доступа)';uk='Успадковується від (значення доступу)'");
		КонецЕсли;
			...
			ДобавитьРеквизит(
				...
				Новый РеквизитФормы(
					...
					Новый ОписаниеТипов(
						...
						|Строка"),
					"ГруппыПрав",
					НСтр("ru='От группы';uk=' Від групи'")),
				Неопределено);
		Иначе
			...
			Элемент.ТолькоПросмотр = НЕ НастройкаПоЗначениюДоступа;
			Элемент.Подсказка      = НСтр("ru='Настройка прав от группы пользователей';uk='Настроювання прав від групи користувачів'");
		КонецЕсли;
	КонецЕсли;
	...
	Если НовыеРеквизиты = Неопределено Тогда
		...
		Элемент.Заголовок   = НСтр("ru='Тип прав';uk='Тип прав'");
		Элемент.Формат      = НСтр("ru='БЛ=Разрешить; БИ=Запретить';uk='БЛ=Дозволити; БИ=Заборонити'");
		Элемент.Подсказка   = НСтр("ru='Запретить/Разрешить';uk='Заборонити/Дозволити'");
	...
	Если НастройкаПоЗначениюДоступа Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если ТекущийПользователь <> Выборка.Пользователь Тогда
				...
				ГруппаПрав.НаследуетсяОт             = НСтр("ru='<не наследуется>';uk='<не успадковується>'");
...
Процедура ЗаписатьПрава(Отказ = Ложь, РазрешеноУправлениеПравамиПослеЗаписи = Ложь, ОписаниеПредупреждения = "")
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при записи прав:';uk='Помилка при записі прав:'") + Символы.ПС + ПредставлениеОшибки, , , , Отказ);
	КонецПопытки;
...
"""Права доступа: %1 (%2)"" .';uk='Це наслідуване настроювання редагується тільки у формі ""Права доступу: %1 (%2)"" .'"),
...
"""Права доступа: %1 (%2)"".';uk='Це настроювання редагується тільки у формі ""Права доступу: %1 (%2)"".'"),
				Элементы.ГруппыПрав.ТекущиеДанные["ГруппаПользователей"],
				?(ВнешнийПользовательИлиГруппа,
					НСтр("ru='Группа внешних пользователей';uk='Група зовнішніх користувачів'"),
					НСтр("ru='Группа пользователей';uk='Група користувачів'")) );
	КонецЕсли;
