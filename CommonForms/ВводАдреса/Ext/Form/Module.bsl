...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.АдресТолькоУкраинский Тогда
	...
	Если ИспользуютсяКлассификаторы И (Страна = СтранаУкраина) И Не ПустаяСтрока(Регион) Тогда
...
Процедура ИндексПриИзменении(Элемент)
	...
	// СтандартныеПодсистемы.АдресныйКлассификатор
	Если (Страна = СтранаУкраина) И ИспользуютсяКлассификаторы
...
Процедура ПолеПриИзменении(Элемент)
	...
	// СтандартныеПодсистемы.АдресныйКлассификатор
	Если ИспользуютсяКлассификаторы И (Страна = СтранаУкраина) Тогда
...
Процедура ПолеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если (Не ИспользуютсяКлассификаторы) ИЛИ (Страна <> СтранаУкраина) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ИспользуютсяКлассификаторы И (Страна = СтранаУкраина) Тогда
		...
		Если Элемент.Имя <> "Регион" Тогда
			Если ПустаяСтрока(Регион) Тогда
				ОчиститьСообщения();
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите регион.';uk='Укажіть регіон.'"), , "Регион");
				Отказ = Истина;
			Иначе
...
Процедура ОбработкаАвтоПодбораИОкончанияВвода(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если (Не ИспользуютсяКлассификаторы) ИЛИ (ПустаяСтрока(Текст)) ИЛИ (Страна <> СтранаУкраина) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьКорректностьЗаполнения(Команда)
	...
	Иначе
		...
		Предупреждение(НСтр("ru='Адрес введен корректно.';uk='Адреса введена коректно.'"));
...
Процедура СформироватьПредставление()
	...
	СтруктураАдреса = ПолучитьСтруктуруАдреса();
	СтруктураАдреса.Вставить("Страна", ?(Страна <> СтранаУкраина, Страна, Неопределено));
	СтруктураАдреса.Вставить("НаименованиеСтраны", ?(Страна <> СтранаУкраина, НаименованиеСтраны, ""));
...
Процедура ПроверитьКнопкиВыбора()
	...
	ЕстьКнопки = ИспользуютсяКлассификаторы И (Страна = СтранаУкраина);
...
Процедура ЗагрузитьРегион(МожноЗагружатьРегион = Неопределено)
	...
	Если Не ПустаяСтрока(Регион) И МожноЗагружатьРегион Тогда
		...
		ВопросПроЗагрузкуРегиона = НСтр("ru='По региону ""%Регион%"" не загружен адресный классификатор (КЛАДР).%ПереводСтроки%Загрузить?';uk=' По регіоні ""%Регіон%"" не завантажений адресний класифікатор (КЛАДР).% ПереводСтроки%Завантажити?'"); 
...
Процедура Подобрать(Команда)
	//ОткрытьФорму("Справочник.УлицыГородов.ФормаВыбора", Новый Структура("ЗакрыватьПриВыборе, МножественныйВыбор", Ложь, Истина), ЭтаФорма,,,, Новый ОписаниеОповещения("ПриЗакрытииФормыПодбора", ЭтаФорма, "Подбор"),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	ПараметрыОтбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора", Истина, Ложь);
	ОткрытьФорму("Справочник.УлицыГородов.ФормаВыбора", ПараметрыОтбора,ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыПодбора", ЭтаФорма, "ПараметрыОтбора"),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура ПриЗакрытииФормыПодбора(РезультатВыбора, Доп) Экспорт 

	Если НЕ РезультатВыбора = Неопределено Тогда
		спрУлицы = ПолучитьСправочник(РезультатВыбора);
		Индекс = спрУлицы.Индекс;
		Регион = спрУлицы.Область;
		Город = спрУлицы.Город;
		Район = спрУлицы.Район;
		НаселенныйПункт = спрУлицы.НаселенныйПункт;
		Улица = спрУлицы.Улица;
		СформироватьПредставление();
	КонецЕсли;
...
&НаСервере
Функция ПолучитьСправочник(Значение)
	ДанныеСправочника = Новый Структура;
	Спр = Справочники.УлицыГородов.НайтиПоНаименованию(Значение);
	СпрДанные = Спр.ПолучитьОбъект();
	ДанныеСправочника.Вставить("Индекс", СпрДанные.ИндексГорода);
	ДанныеСправочника.Вставить("Область", СпрДанные.Область);
	ДанныеСправочника.Вставить("Город", СпрДанные.Город);
	ДанныеСправочника.Вставить("Район", СпрДанные.Район);
	ДанныеСправочника.Вставить("НаселенныйПункт", СпрДанные.НаселенныйПункт);
	ДанныеСправочника.Вставить("Улица", СпрДанные.НазваниеУлицы);
