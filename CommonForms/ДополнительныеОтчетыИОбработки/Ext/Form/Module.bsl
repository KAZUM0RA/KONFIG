...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ДополнительныйОтчет
	 ИЛИ ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет Тогда
		Элементы.НастроитьСписокБыстрогоДоступа.Заголовок = НСтр("ru='Настроить список моих отчетов';uk='Настроїти список моїх звітів'");
	Иначе
		Элементы.НастроитьСписокБыстрогоДоступа.Заголовок = НСтр("ru='Настроить список моих обработок';uk='Настроїти список моїх обробок'");
	КонецЕсли;
...
Процедура ВыполнитьОбработкуПоПараметрам()
	...
	ИначеЕсли ДанныеОбработки.ВариантЗапуска = ДополнительныеОтчетыИОбработкиКлиент.ВариантЗапускаВызовСерверногоМетода() Тогда
		...
		Элементы.ПоясняющаяДекорация.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выполняется обработка ""%1""...';uk='Виконується обробка ""%1""...'"),
...
Процедура ВыполнитьКлиентскийМетодОбработки()
	Если ВыполняемаяОбработка.ПоказыватьОповещение Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Обработка выполняется...';uk='Обробка виконується...'"));
	КонецЕсли;
...
"Подробности см. в журнале регистрации.';uk='Не вдалося виконати обробку ""%1"". Подробиці див. у журналі реєстрації.'"),
...
Процедура ПоказатьРезультатВыполненияОбработки(Результат = Неопределено)
	// Показать оповещение
	Если ВыполняемаяОбработка.ПоказыватьОповещение Тогда
		ОповещениеЗаголовок = НСтр("ru='Обработка выполнена';uk='Обробка виконана'");
