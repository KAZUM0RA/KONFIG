...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если ДополнительныеОтчетыИОбработкиКлиент.СохранитьДанныеПроизвольнойФормы(Отказ, Модифицированность) Тогда
...
Функция ЗаполнитьДеревоОбработок(КомандыПользователя, ИмяЭлементыРеквизитаДерева)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ДОИО.Ссылка КАК Обработка,
	|	КомандыДОИО.Представление КАК Наименование,
	|	РазделыДОИО.Раздел КАК Раздел,
	|	КомандыДОИО.Идентификатор КАК Идентификатор
	|ИЗ
	|	Справочник.ДополнительныеОтчетыИОбработки КАК ДОИО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДополнительныеОтчетыИОбработки.Команды КАК КомандыДОИО
	|		ПО ДОИО.Ссылка = КомандыДОИО.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДополнительныеОтчетыИОбработки.Разделы КАК РазделыДОИО
	|		ПО ДОИО.Ссылка = РазделыДОИО.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПользовательскиеНастройкиДоступаКОбработкам КАК ДоступДОИО
	|		ПО ДОИО.Ссылка = ДоступДОИО.ДополнительныйОтчетИлиОбработка
	|			И (ДоступДОИО.ИдентификаторКоманды = КомандыДОИО.Идентификатор)
	|			И (ДоступДОИО.Пользователь = &Пользователь)
	|ГДЕ
	|	ДОИО.Вид = &ВидОбработок
	|	И НЕ ДОИО.ПометкаУдаления
	|	И ДОИО.Публикация В(&ВариантыПубликации)
	|	И ДоступДОИО.Доступно
	|ИТОГИ ПО
	|	&ИтогиПоРаздел";
	...
	Иначе
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДополнительныеОтчетыИОбработки.Разделы КАК РазделыДОИО", "");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПО ДОИО.Ссылка = РазделыДОИО.Ссылка", "");
	...
	Если НЕ КомандыПользователя Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПользовательскиеНастройкиДоступаКОбработкам КАК ДоступДОИО", "");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПО ДОИО.Ссылка = ДоступДОИО.ДополнительныйОтчетИлиОбработка", "");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И (ДоступДОИО.ИдентификаторКоманды = КомандыДОИО.Идентификатор)", "");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И (ДоступДОИО.Пользователь = &Пользователь)", "");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И ДоступДОИО.Доступно", "");
	КонецЕсли;
	...
	Если ЭтоГлобальныеОбработки Тогда
		Для Каждого СтрокаРазделы Из ДеревоКоманд.Строки Цикл
			...
			СтрокаВерхнегоУровня.Наименование = ДополнительныеОтчетыИОбработки.ПолучитьИмяРабочегоМеста(СтрокаРазделы.Раздел);
