...
Процедура ИсключитьИзГруппы(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяГруппаДоступа) Тогда
		Предупреждение(НСтр("ru='Группа доступа не выбрана.';uk='Група доступу не обрана.'"));
		Возврат;
	КонецЕсли;
...
"не полноправный администратор.';uk='Неможливо змінити групу доступу,  тому що поточний користувач не адміністратор учасників групи доступу й не повноправний адміністратор.'"));
		Возврат;
...
"так как текущий пользователь"
"не администратор участников группы доступа и"
"не полноправный администратор.';uk='Неможливо включити користувача в групу доступу,  тому що поточний користувач не адміністратор учасників групи доступу й не повноправний адміністратор.'");
		Иначе
			ОписаниеОшибки = НСтр("ru='Невозможно исключить пользователя из группы доступа,"
"так как текущий пользователь"
"не администратор участников группы доступа и"
"не полноправный администратор.';uk='Неможливо виключити користувача із групи доступу,  тому що поточний користувач не адміністратор учасників групи доступу й не повноправний адміністратор.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ Добавить И НЕ ПользовательВключенВГруппуДоступа(ТекущаяГруппаДоступа) Тогда
		ОписаниеОшибки =  НСтр("ru='Невозможно исключить пользователя из группы доступа,"
"так как он включен в нее косвенно.';uk='Неможливо виключити користувача із групи доступу,  тому що він включений у неї побічно.'");
		Возврат;
	КонецЕсли;
...
"непосредственно в группе доступа %1.';uk='Зміна состава адміністраторів виконується безпосередньо в групі доступу %1.'"),
