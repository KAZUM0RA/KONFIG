...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МоиЗаметки.Параметры.УстановитьЗначениеПараметра("Пользователь", Пользователи.ТекущийПользователь());
	МоиЗаметки.Параметры.УстановитьЗначениеПараметра("Предмет", "");
	МоиЗаметки.Параметры.УстановитьЗначениеПараметра("Пометка", Перечисления.ЦветаЗаметок.ПустаяСсылка());
	МоиЗаметки.Параметры.УстановитьЗначениеПараметра("ПоказыватьУдаленные", Ложь);
...
Процедура ПапкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
		Предупреждение(НСтр("ru='Недостаточно прав для создания папки.';uk='Недостатньо прав для створення теки.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПапкиПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru='Удалить папку ""';uk='Видалити теку ""'") + Строка(ТекущиеДанные.Значение) 
	+ НСтр("ru='"" и переместить все ее содержимое в папку ""Удаленные""';uk='"" і перемістити увесь її вміст в теку ""Видалені""'");
...
Процедура Ответить(Команда)
	...
	Если ВыборКорректен(ТекущийЭлементСписок.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			Предупреждение(НСтр("ru='Команда ""Ответить"" может быть выполнена только для входящего письма.';uk='Команда ""Відповісти"" може бути виконана тільки для листа, що входить.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура ОтветитьВсем(Команда)
	...
	Если ВыборКорректен(ТекущийЭлементСписок.Имя, Истина) Тогда
		ТекущееВзаимодействие = ТекущийЭлементСписок.ТекущиеДанные.Ссылка;
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			Предупреждение(НСтр("ru='Команда ""Ответить всем"" может быть выполнена только для входящего письма.';uk='Команда ""Відповісти усім"" може бути виконана тільки для листа, що входить.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура Переслать(Команда)
	...
	Если ВыборКорректен(ТекущийЭлементСписок.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") И 
			...
			Предупреждение(НСтр("ru='Команда ""Переслать"" может быть выполнена только для писем.';uk='Команда ""Переслати"" може бути виконана тільки для листів.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура ПредметВыполнить(Команда)
	...
	ВыбранныйТипПредмета = СписокВыбораТипаПредмета.ВыбратьЭлемент(Нстр("ru='Выберите тип предмета';uk='Виберіть тип предмета'"));
...
Процедура ДобавитьВЗакладки(Команда)
	...
	Если Лев(ИмяТекущегоЭлемента, 6) = "Список" И Не ВыборКорректен(ИмяТекущегоЭлемента) Тогда
		Предупреждение(НСтр("ru='Не выбран элемент для добавления в закладки.';uk='Не вибраний елемент для додавання в закладки.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПеренестиВПапку(Команда)
	...
	Если ИмяТекущегоЭлемента = "Папки" Тогда
		Если ТипЗнч(ПапкиТекущиеДанные.Значение) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") ИЛИ 
			...
			Предупреждение(НСтр("ru='Команда не может быть выполнена для данного объекта';uk=""Команда не може бути виконана для цього об'єкту"""));
			Возврат;
		ИначеЕсли ПапкиТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
			Предупреждение(НСтр("ru='Недостаточно прав для изменения папок.';uk='Недостатньо прав для зміни тек.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ОпределитьСтраницуСпискаДляПапки(Папка)
	...
	СписокНаименованийИсходящее.Добавить(НСтр("ru='Отправленные';uk='Відправлені'"));
	СписокНаименованийИсходящее.Добавить(НСтр("ru='Исходящие';uk='Вихідні'"));
	СписокНаименованийИсходящее.Добавить(НСтр("ru='Черновики';uk='Чернетки'"));
...
Процедура ИзменитьОтборСписок(ИмяТаблицы, ДанныеДляОбработки , ИмяСтраницыСписка);
	...
	Если ДанныеДляОбработки.Значение = Нстр("ru='Все';uk='Усе'") Тогда
	...
	ИначеЕсли ИмяТаблицы = "Свойства" Тогда
		...
		Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") И 
			ДанныеДляОбработки.Значение = Нстр("ru='Не указан';uk='Не вказаний'") Тогда
...
Функция ПолучитьВзаимодействияПоКонтактам(СтруктураКонтакты, ЗначениеРеквизитаДопОтбора = Неопределено, ИмяРеквизитаДопОтбора = "")
	...
	Если СтруктураКонтакты.Значение = НСтр("ru='Контакт не подобран';uk='Контакт не підібраний'") Тогда
...
Процедура УстановитьЗаголовокВидаПанелиНавигации()
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаРеквизитыПредмета Тогда
		...
		Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru='По ';uk='По '") + СтруктураОписание.ПредставлениеПо;
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПапки Тогда
		...
		Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru='По папкам электронных писем';uk='По теках електронних листів'");
...
Функция ДобавитьСтрокуВсе(ДанныеФормыКоллекция, НомерКартинки = 0)
	...
	КонецЕсли;
		...
		НоваяСтрока.Значение = Нстр("ru='Все';uk='Усе'");
		НоваяСтрока.Представление = Нстр("ru='Все';uk='Усе'");
...
Процедура ЗаполнитьДеревоПредметов()
	...
	Запрос.УстановитьПараметр("СтрокаПредметНеУказан", НСтр("ru='Прочие вопросы';uk='Інші питання'"));
...
Процедура ЗаполнитьДеревоКонтактов()
	...
	Для Каждого Строка ИЗ Дерево.Строки Цикл
		...
		Иначе
			СтрокаТип.Значение = НСтр("ru='Контакт не подобран';uk='Контакт не підібраний'");
			СтрокаТип.НомерКартинки = 0;
		КонецЕсли;
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Строка.Тип);
...
Процедура ЗаполнитьДеревоРеквизитовПредмета(ИмяКоманды = "")
	...
	Запрос.УстановитьПараметр("ПрочиеВопросы", НСтр("ru='Прочие вопросы';uk='Інші питання'"));
	Запрос.УстановитьПараметр("ПрочиеПредметы", НСтр("ru='Прочие предметы';uk='Інші предмети'"));
	...
	Для Каждого Строка ИЗ Дерево.Строки Цикл
		...
		Если Строка.Группировка = НСтр("ru='Прочие вопросы';uk='Інші питання'") Тогда
			СтрокаГруппировка.НомерКартинки = 0;
		ИначеЕсли Строка.Группировка = НСтр("ru='Прочие предметы';uk='Інші предмети'") Тогда
			СтрокаГруппировка.НомерКартинки = 0;
		Иначе
...
Процедура ПослеЗаполненияПанелиНавигации()
	...
	Если Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКонтакт Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Элементы.Контакты.ТекущаяСтрока = НайтиСтрокуВДанныхФормыДерево(Контакты,НСтр("ru='Все';uk='Усе'"),"Значение",Ложь);
			ТекущиеДанные = Контакты.НайтиПоИдентификатору(Элементы.Контакты.ТекущаяСтрока);
		КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПредмет Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Элементы.Предметы.ТекущаяСтрока = НайтиСтрокуВДанныхФормыДерево(Предметы,НСтр("ru='Все';uk='Усе'"),"Значение",Ложь);
			ТекущиеДанные = Предметы.НайтиПоИдентификатору(Элементы.Предметы.ТекущаяСтрока);
		КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаРеквизитыПредмета Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Элементы.РеквизитыПредмета.ТекущаяСтрока = НайтиСтрокуВДанныхФормыДерево(РеквизитыПредмета,НСтр("ru='Все';uk='Усе'"),"Значение",Ложь);
			ТекущиеДанные = РеквизитыПредмета.НайтиПоИдентификатору(Элементы.РеквизитыПредмета.ТекущаяСтрока);
		КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаСвойства Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Элементы.Свойства.ТекущаяСтрока = НайтиСтрокуВДанныхФормыДерево(Свойства,НСтр("ru='Все';uk='Усе'"),"Значение",Ложь);
			ТекущиеДанные = Свойства.НайтиПоИдентификатору(Элементы.Свойства.ТекущаяСтрока);
		КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКатегории Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Элементы.Категории.ТекущаяСтрока = НайтиСтрокуВДанныхФормыКоллекция(Категории,НСтр("ru='Все';uk='Усе'"),"Значение");
			ТекущиеДанные = Категории.НайтиПоИдентификатору(Элементы.Категории.ТекущаяСтрока);
		КонецЕсли;
	...
	Иначе
		...
		Если ИмяЭлементаФормы = "РеквизитыПредмета" Тогда
			...
			ИначеЕсли ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				НаименованиеЗакладки = НСтр("ru='Тип предмета ';uk='Тип предмета'") + Строка(ДанныеДляОбработки.Значение);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма';uk='Листи'"), НСтр("ru='Взаимодействия';uk='Взаємодії'"));
				Результат.НавигационнаяСсылкаЭлемента = ПолучитьНавигационнуюСсылку(ДанныеДляОбработки.Значение);
				
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru='Предмет не указан';uk='Предмет не вказаний'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru='Предмет не указан';uk='Предмет не вказаний'"); 
		...
		ИначеЕсли ИмяЭлементаФормы = "Предметы" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				Текст = ?(ТолькоПочта, НСтр("ru='Письма по предмету %1';uk='Листи по предмету %1'"), НСтр("ru='Взаимодействия по предмету %1';uk='Взаємодії по предмету %1'"));
				Результат.ПредставлениеЭлемента = 
			...
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru='Не указан';uk='Не вказаний'") Тогда
			...
			Иначе
				...
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма по: %1';uk='Листи  по: %1'"), НСтр("ru='Взаимодействия по: %1';uk='Взаємодії по: %1'"));
				Результат.ПредставлениеЭлемента = 
		...
		ИначеЕсли ИмяЭлементаФормы = "Свойства" Тогда
			...
			Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") И 
				ДанныеДляОбработки.Значение = Нстр("ru='Не указан';uk='Не вказаний'") Тогда
				НаименованиеЗакладки       = ТекущееСвойствоПанелиНавигации.Наименование + " " + НСтр("ru='не указан';uk='не вказаний'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма';uk='Листи'"), НСтр("ru='Взаимодействия';uk='Взаємодії'"));
			Иначе
				НаименованиеЗакладки       = ТекущееСвойствоПанелиНавигации.Наименование +
					" = " + Строка(ДанныеДляОбработки.Значение);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма с заданным свойством: %1';uk='Листи із заданою властивістю: %1'"), НСтр("ru='Взаимодействия с заданным свойством: %1';uk='Взаємодії із заданою властивістю: %1'"));
				Результат.ПредставлениеЭлемента = 
		...
		ИначеЕсли ИмяЭлементаФормы = "Категории" Тогда
			...
			НаименованиеЗакладки       = НСтр("ru='Входит в категорию ';uk='Входить в категорію'") + Строка(ДанныеДляОбработки.Значение);
		...
		ИначеЕсли ИмяЭлементаФормы = "Контакты" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				Текст = ?(ТолькоПочта, НСтр("ru='Письма по контакту %1';uk='Листи по контакту %1'"), НСтр("ru='Взаимодействия по контакту %1';uk='Взаємодії по контакту %1'"));
				Результат.ПредставлениеЭлемента = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ОбщегоНазначения.ПредметСтрокой(ДанныеДляОбработки.Значение));
				Результат.НавигационнаяСсылкаЭлемента = ПолучитьНавигационнуюСсылку(ДанныеДляОбработки.Значение);
			...
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru='Контакт не подобран';uk='Контакт не підібраний'") Тогда
				...
				НаименованиеЗакладки       = НСтр("ru='Контакт не указан';uk='Контакт не вказаний'");
			...
			Иначе
				...
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма по контактам: %1';uk='Листи по контактах: %1'"), НСтр("ru='Взаимодействия по контактам: %1';uk='Взаємодії по контактах: %1'"));
				Результат.ПредставлениеЭлемента = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, 
						ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
		...
		ИначеЕсли ИмяЭлементаФормы = "Папки" Тогда
			...
			НаименованиеЗакладки       = НСтр("ru='В папке ';uk='У теці'") + Строка(ДанныеДляОбработки.Значение);
			Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма в папке: %1';uk='Листи в теці: %1'"), НСтр("ru='Взаимодействия в папке: %1';uk='Взаємодії в теці: %1'"));
			Результат.ПредставлениеЭлемента = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, ДанныеДляОбработки.Значение);
	...
	Пока Выборка.Следующий() Цикл
		Если ПолучитьXML(КомпоновщикНастроек.ПолучитьНастройки()) =  ПолучитьXML(Выборка.КомпоновщикНастроек.Получить()) Тогда
			Результат.ТекстСообщенияОшибка = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Закладка с такими настройками уже существует : %1';uk='Закладка з такими налаштуваннями вже існує : %1'"), Выборка.Наименование);
			Возврат Результат;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьИндексППД()
	...
	Если Не ИндексППДАктуален Тогда
		...
		Результат = Вопрос(НСтр("ru='Индекс полнотекстового поиска неактуален. Обновить индекс?';uk='Індекс повнотекстового пошуку неактуальний. Оновити індекс?'"),
...
Процедура ОбновлениеИндексаПДД()
	...
	ОбновитьИндексППД();
	ИндексППДАктуален = Истина;
	Состояние(НСтр("ru='Обновление индекса полнотекстового поиска завершено...';uk='Оновлення індексу повнотекстового пошуку завершене...'"));
...
Функция НайтиВзаимодействияПолнотекстовыйПоиск(КоличествоЭлементов)
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда
		...
		Возврат НСтр("ru='Слишком много результатов, уточните запрос.';uk='Надто багато результатів, уточните запит.'");
	КонецЕсли;
...
Процедура ЗаполнитьОписаниеНайденоПП(Взаимодействие)
	...
	Иначе
		...
		Если ВзаимодействияКлиентСервер.ЯвляетсяПрисоединеннымФайломВзаимодействий(СтрокаТаблицыСОписанием.Значение) Тогда
			ТекстНайдено = НСтр("ru='Найдено в присоединенном файле - ';uk='Знайдено в приєднаному файлі -'");
		Иначе
			ТекстНайдено = НСтр("ru='Найдено в - ';uk='Знайдено в -'");
		КонецЕсли;
