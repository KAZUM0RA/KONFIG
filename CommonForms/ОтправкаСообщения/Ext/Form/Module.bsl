...
Процедура ОтправитьПисьмо(Команда)
	ОтправитьПисьмоВыполнить();
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьПисьмоВыполнить(ВызовИзФормы = ложь) Экспорт
	...
	Иначе
		ПродолжитьОтправкуПисьмаСПаролем(,ВызовИзФормы);
	КонецЕсли;
...
Функция ПолучитьТабличныйДокументПоДвоичнымДанным(Знач ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение табличного документа';uk='Одержання табличного документа'"), УровеньЖурналаРегистрации.Ошибка, , , 
...
Процедура ДобавлениеФайлаКВложениям()
	...
	Если ПодключитьРасширениеРаботыСФайлами() Тогда
		...
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(,НСтр("ru='В Веб-клиенте без установленного расширения работы с файлами добавление файлов не поддерживается.';uk='У Веб-клієнті без установленого розширення роботи з файлами додавання файлів не підтримується.'"));
	КонецЕсли;
...
&НаКлиенте
Процедура ПродолжитьОтправкуПисьмаСПаролем(Пароль = Неопределено, ВызовИзФормы = Ложь)
	
	ПараметрыПисьма = СформироватьПараметрыПисьма(Пароль);
	
	Если ПараметрыПисьма = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка формирования параметров почтового сообщения';uk='Помилка формування параметрів поштового повідомлення'"));
		Возврат;
	КонецЕсли;
	...
	Если НЕ ВызовИзФормы Тогда
		ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru='Сообщение успешно отправлено';uk='Повідомлення успішно відправлено'"));
	КонецЕсли;
