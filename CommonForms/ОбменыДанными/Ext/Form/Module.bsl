...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПлановОбмена.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Возможность настройки обмена данными не предусмотрена.';uk='Можливість настроювання обміну даними не передбачена.'");
	...
	Для Каждого Элемент ИЗ СписокПлановОбмена Цикл
		...
		Если МенеджерПланаОбмена.ИспользоватьПомощникСозданияОбменаДанными() 
			...
			Если МенеджерПланаОбмена.ПланОбменаИспользуетсяВМоделиСервиса()
				...
				Команды[ИмяКоманды].Заголовок = МенеджерПланаОбмена.ЗаголовокКомандыДляСозданияНовогоОбменаДанными() + НСтр("ru=' (в сервисе)';uk=' (у сервісі)'");
