...
Процедура ДеревоВзаимодействийПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ДеревоВзаимодействий.ВыделенныеСтроки.Количество() > 0 Тогда
		...
		Если ЕстьПомеченныеНаУдаление Тогда
			ТекстВопроса = НСтр("ru='Снять с выделенных элементов пометку на удаление?';uk='Зняти з виділених елементів позначку на видалення?'");
		Иначе
			ТекстВопроса = НСтр("ru='Пометить выделенные строки на удаление?';uk='Помітити виділені рядки на видалення?'");
		КонецЕсли;
...
Процедура ПредметВыполнить()
	...
	ВыбранныйТипПредмета = 
		СписокВыбораТипаПредмета.ВыбратьЭлемент(Нстр("ru='Выберите тип предмета';uk='Виберіть тип предмета'"));
