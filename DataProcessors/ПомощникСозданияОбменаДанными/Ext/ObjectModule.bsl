...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВебСервисВДвухБазах(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru='При создании настройки обмена во второй информационной базе возникли ошибки.';uk=' При створенні настроювання обміну в другій інформаційній базі виникли помилки.'");
		КонецЕсли;
...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВнешнееСоединение(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			Сообщение = НСтр("ru='При создании настройки обмена во второй информационной базе возникли ошибки: %1';uk=' При створенні настроювання обміну в другій інформаційній базі виникли помилки: %1'");
...
"Возможно, информационная база не является подчиненным узлом в РИБ.';uk='Главный узел для текущей информационной базы не определен."
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если СтруктураНастроек.Свойство("ИмяПланаОбмена")
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru='Файл содержит настройки обмена для другой информационной базы.';uk='Файл містить настроювання обміну для іншої інформаційної бази.'");
	...
	Если Не Отказ Тогда
		...
		Если ВерсияФорматаФайлаНастроекОбменаДанными = "1.0" Тогда
			...
			ЭтотОбъект.НаименованиеЭтойБазы = НСтр("ru='Эта информационная база';uk='Ця інформаційна база'");
...
"Настройку обмена требуется начать заново.';uk='На першому етапі настроювання обміну був неправильно зазначений префікс другої інформаційної бази. Настроювання обміну потрібно почати заново.'");
		//
		ОбменДаннымиСервер.СообщитьОбОшибке(СтрокаСообщенияОбОшибке(), Отказ);
