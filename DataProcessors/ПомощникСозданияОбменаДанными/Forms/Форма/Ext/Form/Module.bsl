...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны("ДобавлениеИзменениеОбменовДанными") Тогда
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru='Нет прав на добавление/изменение обменов данными.';uk='Немає прав на додавання/зміна обмінів даними.'"), Отказ);
		Возврат;
	КонецЕсли;
	...
	Если Не Параметры.Свойство("ИмяПланаОбмена", Объект.ИмяПланаОбмена) Тогда
		...
		НСтр("ru='Работа помощника поддерживается только при вызове из командного интерфейса. Работа помощника завершена.';uk='Робота помічника підтримується тільки при виклику з командного інтерфейсу. Робота помічника завершена.'"), Отказ);
		
	ИначеЕсли ПустаяСтрока(Объект.ИмяПланаОбмена) Тогда
		
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru='Не задано имя плана обмена. Работа помощника завершена.';uk=""Не задане ім'я плану обміну. Робота помічника завершена."""), Отказ);
	...
	Заголовок = Метаданные.ПланыОбмена[Объект.ИмяПланаОбмена].Синоним + " " + НСтр("ru='(создание)';uk='(створення)'");
	...
	ИначеЕсли ИспользоватьТранспортСообщенийОбменаFILE Тогда
...
"Работа помощника не может быть завершена.';uk='Виконується створення обміну даними. Робота помічника не може бути завершена.'")
		);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	НСтрока = НСтр("ru='Отменить настройку обмена и выйти из помощника?';uk='Скасувати настроювання обміну й вийти з помічника?'");
...
Процедура ИмяФайлаНастроекОбменаДаннымиДляЗагрузкиПриИзменении(Элемент)
	...
	Если    Не Файл.Существует()
		...
		Предупреждение(НСтр("ru='Укажите корректное имя файла настроек.';uk=""Укажіть коректне ім'я файлу настроювань."""));
...
Процедура СохранитьФайлНастроекОбменаДанными(Команда)
	...
	Если Отказ Тогда
		...
		Предупреждение(НСтр("ru='При сохранении файла настроек обмена данными возникли ошибки.';uk=' При збереженні файлу настроювань обміну даними виникли помилки.'"));
	...
	Иначе
		...
		#Иначе
			...
			Диалог.Заголовок      = НСтр("ru='Укажите имя файла настроек обмена данными';uk=""Укажіть ім'я файлу настроювань обміну даними""");
...
Процедура ПроверитьПодключениеCOM(Команда)
	...
	Если Отказ Тогда
		...
		Предупреждение(НСтр("ru='При установке подключения возникла ошибка(см. Журнал регистрации).';uk=' При установці підключення виникла помилка(див. Журнал реєстрації).'"));
...
Процедура КакОпределитьПараметрыПодключенияКВебСервису(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Как определить параметры подключения ко второй информационной базе';uk=' Як визначити параметри підключення до другої інформаційної бази'"));
...
Процедура КакОпределитьПараметрыПодключенияКСервису(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Как определить параметры подключения к приложению, расположенному в сервисе';uk=' Як визначити параметри підключення до додатка, розташованого в сервісі'"));
...
Процедура КакОпределитьПрефиксВторойИнформационнойБазы(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Как определить префикс второй информационной базы';uk=' Як визначити префікс другої інформаційної бази'"));
...
Процедура КакСформироватьФайлСНастройкамиОбменаДанными(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Как сформировать файл с настройками обмена данными';uk=' Як сформувати файл із настроюваннями обміну даними'"));
...
"Перейти в журнал регистрации?';uk=' При створенні обміну даними виникли помилки. Перейти в журнал реєстрації?'"
		);
...
"%1';uk='Параметри підключення до додатка в сервісі: %1'");
...
Процедура ИнициализацияОбменаДаннымиНаКлиенте(Отказ)
	...
	Состояние(НСтр("ru='Выполняется выгрузка данных...';uk='Виконується вивантаження даних...'"));
	...
	Состояние(НСтр("ru='Выгрузка данных завершена';uk='Вивантаження даних завершене'"));
	
	Если Отказ Тогда
		
		Предупреждение(НСтр("ru='Во время выгрузки данных возникли ошибки(см. журнал регистрации).';uk=' Під час вивантаження даних виникли помилки(див. журнал реєстрації).'"));
...
Функция ПолучитьОписаниеНастроекТранспортаОбмена()
	...
	Если ПустаяСтрока(Результат) Тогда
		...
		Результат = НСтр("ru='Параметры транспорта не заданы.';uk='Параметри транспорту не задані.'");
...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	СтрокаСообщения = ?(Отказ,  НСтр("ru='При установке подключения возникли ошибки(см. Журнал регистрации).';uk=' При установці підключення виникли помилки(див. Журнал реєстрації).'"),
								НСтр("ru='Подключение успешно установлено.';uk='Підключення успішне встановлене.'"));
	//
...
Процедура ПроверитьПодключениеWSНаСервере(Отказ, РасширеннаяПроверка, ПредлагатьПерейтиВЖурналРегистрации)
	...
	Если РасширеннаяПроверка Тогда
		...
		Если Не ПараметрыПриемника.ПланОбменаСуществует Тогда
			...
			Сообщение = НСтр("ru='Вторая информационная база не предназначена для обмена с текущей информационной базой.';uk='Друга інформаційна база не призначена для обміну з поточної інформаційною базою.'");
...
Процедура ВыбратьФайлНастроекОбмена(Интерактивно)
	...
	Если ПоместитьФайл(АдресВременногоХранилища, ИмяФайлаПоУмолчанию, ВыбранноеИмяФайла, Интерактивно, УникальныйИдентификатор) Тогда
		...
		Если Отказ Тогда
			Предупреждение(НСтр("ru='Указан неправильный файл настроек обмена данными. Укажите корректный файл.';uk='Зазначений неправильний файл настроювань обміну даними. Укажіть коректний файл.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеРеквизитовВФорме(Отказ, ИмяФормыДляПроверки, ПараметрыФормы, ИмяРеквизитаФормы)
	...
	Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки.';uk="" Необхідно задати обов'язкові настроювання."""),,, ИмяРеквизитаФормы, Отказ);
...
Процедура ЗавершениеПервогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Если Не Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		НСтрока = НСтр("ru='Сохраните файл с настройками для информационной базы-приемника';uk='Збережете файл із настроюваннями для інформаційної бази-приймача'");
...
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Состояние(НСтр("ru='Выполняется создание настройки обмена данными';uk='Виконується створення настроювання обміну даними'"));
	
	НастроитьНовыйОбменДаннымиНаСервере(Отказ, НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле);
	
	Если Отказ Тогда
		
		Предупреждение(НСтр("ru='При создании настройки обмена данными возникли ошибки.';uk=' При створенні настроювання обміну даними виникли помилки.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВключитьИспользованиеЖурналаРегистрации(Знач СобытиеЖурналаРегистрации, Предупреждение)
	...
	Исключение
		Предупреждение = НСтр("ru='Не удалось включить использование журнала регистрации: %1';uk='Не вдалося включити використання журналу реєстрації: %1'");
...
Процедура ОткрытьФормуСопоставления()
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		Предупреждение(НСтр("ru='Для этих данных нельзя выполнить сопоставление.';uk=' Для цих даних не можна виконати зіставлення.'"));
		Возврат;
	КонецЕсли;
...
Функция НадписьДалееFILE()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для задания настроек обмена через локальный или сетевой каталог.';uk='Натисніть кнопку ""Далі"" для завдання настроювань обміну через локальний або мережний каталог.'");
...
Функция НадписьДалееFTP()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для задания настроек обмена через FTP-ресурс.';uk='Натисніть кнопку ""Далі"" для завдання настроювань обміну через Ftp-Ресурс.'");
...
Функция НадписьДалееEMAIL()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для задания настроек обмена по почте.';uk='Натисніть кнопку ""Далі"" для завдання настроювань обміну поштою.'");
...
Функция НадписьДалееНастройки()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для настройки дополнительных параметров обмена данными.';uk='Натисніть кнопку ""Далі"" для настроювання додаткових параметрів обміну даними.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование этой информационной базы.';uk='Укажіть найменування цієї інформаційної бази.'");
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование второй информационной базы.';uk='Укажіть найменування другої інформаційної бази.'");
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	Если Не Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Если ПустаяСтрока(Объект.ИмяФайлаНастроекОбменаДаннымиДляЗагрузки) Тогда
			...
			НСтрока = НСтр("ru='Выберите файл с настройками обмена данными';uk='Виберіть файл із настроюваннями обміну даними'");
...
"Следует использовать хотя бы один вид обмена данными.';uk='Не зазначені настроювання ні для одного виду обміну даними.  Слід використовувати хоча б один вид обміну даними.'");
		//
...
"Префикс второй информационной базы: %5';uk='%1%2%3Префікс цієї інформаційної бази: %4 Префікс другої інформаційної бази: %5'");
...
"========================================================"
"%5%6Префикс информационной базы: %7';uk='%1"
"Настройки обмена данными для этой информационной базы:"
"========================================================"
"%2%3Префикс информационной базы: %4"
""
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі створення настроювання обміну даними.  Для вирішення проблем скористайтеся журналом реєстрації.'")
		);
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі аналізу даних.  Для вирішення проблем скористайтеся журналом реєстрації.'")
		);
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхАвтоматическоеСопоставление_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		Предупреждение(НСтр("ru='Возникли ошибки на этапе анализа данных.';uk='Виникли помилки на етапі аналізу даних.'"));
...
"Продолжить?';uk='Не всі дані зіставлені. Наявність незіставлених даних може привести до появи однакових елементів довідників (дублів). Продовжити?'"
		);
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі синхронізації довідників.  Для вирішення проблем скористайтеся журналом реєстрації.'")
		);
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі збереження настроювань.  Для вирішення проблем скористайтеся журналом реєстрації.'")
		);
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі синхронізації даних.  Для вирішення проблем скористайтеся журналом реєстрації.'")
		);
...
"Измените значение префикса или используйте существующий обмен.';uk='Значення префікса другої інформаційної бази не унікально. У системі вже існує обмін даними для інформаційної бази із зазначеним префіксом. Зміните значення префікса або використовуйте існуючий обмін.'");
		//
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Процедура СтраницаМастераОжиданиеПроверкиПодключенияВнешнегоСоединения_ОбработкаДлительнойОперацииНаСервере(Отказ)
	...
	Если НРег(СтрокаСоединенияИнформационнойБазы()) = НРег(ВнешнееСоединение.СтрокаСоединенияИнформационнойБазы()) Тогда
		...
		Сообщение = НСтр("ru='Настройки подключения указывают на текущую информационную базу.';uk='Настроювання підключення вказують на поточну інформаційну базу.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
...
Процедура ПроверитьНаличиеСуществующегоОбменаДаннымиСВторойБазой(Отказ)
	...
	Если Не ПустаяСтрока(КодУзла)
		...
		Сообщение = НСтр("ru='Обмен данными уже настроен в системе. Используйте существующий обмен.';uk='Обмін даними вже настроєний у системі. Використовуйте існуючий обмін.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
