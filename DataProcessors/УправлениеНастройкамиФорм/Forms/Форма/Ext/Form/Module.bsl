...
Процедура СкопироватьВыполнить()
	...
	Если Элементы.ОтфильтрованныеФормы.ВыделенныеСтроки.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Для копирования нужно выбрать настройки, которые требуется скопировать.';uk=' Для копіювання потрібно вибрати настроювання, які потрібно скопіювати.'"));
		Возврат;
	КонецЕсли;
	...
	Если СписокПользователей.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Невозможно скопировать настройки, так как в системе не заведен ни один пользователь.';uk='Неможливо скопіювати настроювання, тому що в системі не заведений жоден користувач.'"));
		Возврат;
	КонецЕсли;
	...
	Если СписокПользователей.ОтметитьЭлементы(НСтр("ru='Отметьте пользователей, которым нужно скопировать настройки';uk='Відзначте користувачів, яким потрібно скопіювати настроювання'")) Тогда
		...
		Если ПользователиПриемник.Количество() = 0 Тогда
			Предупреждение(НСтр("ru='Для копирования нужно отметить пользователей, которым требуется скопировать настройки.';uk=' Для копіювання потрібно відзначити користувачів, яким потрібно скопіювати настроювання.'"));
			Возврат;
		КонецЕсли;
...
"Вы действительно хотите скопировать настройки выбранным пользователям?';uk='Після копіювання настроювань користувачеві, форма в користувача буде відкриватися з настроюваннями, які йому скопiются.   При цьому його власні настроювання будуть загублені.  Ви дійсно прагнете скопіювати настроювання обраним користувачам?'");
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Настройки скопированы';uk='Настроювання скопійовані'"));
	КонецЕсли;
...
Процедура УдалитьВыполнить()
	...
	Если Элементы.ОтфильтрованныеФормы.ВыделенныеСтроки.Количество() = 0 Тогда
		...
		Предупреждение(НСтр("ru='Для удаления нужно выбрать настройки, которые требуется удалить.';uk=' Для видалення потрібно вибрати настроювання, які потрібно вилучити.'"));
		Возврат;
		
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru='После удаления настроек форма будет открываться с настройками по умолчанию. "
"Вы действительно хотите удалить настройки?';uk='Після видалення настроювань форма буде відкриватися з настроюваннями за замовчуванням.  Ви дійсно прагнете вилучити настроювання?'");
	Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
	Если Ответ = КодВозвратаДиалога.Нет Тогда
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Настройки удалены';uk='Настроювання вилучені'"));
