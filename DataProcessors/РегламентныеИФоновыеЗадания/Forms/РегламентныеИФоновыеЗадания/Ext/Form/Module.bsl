...
"выполняется только администраторами.';uk='Недостатньо правий доступу.  Робота з регламентними й фоновими завданнями виконується тільки адміністраторами.'");
	КонецЕсли;
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки.Количество() > 1 Тогда
		Предупреждение(НСтр("ru='Выберите одно регламентное задание.';uk='Виберіть одне регламентне завдання.'"));
		
	ИначеЕсли Элемент.ТекущиеДанные.Предопределенное Тогда
		Предупреждение(НСтр("ru='Невозможно удалить предопределенное регламентное задание.';uk='Неможливо вилучити визначене регламентне завдання.'") );
	Иначе
		Ответ = Вопрос(НСтр("ru='Удалить регламентное задание?';uk='Вилучити регламентне завдання?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		Предупреждение( НСтр("ru='Выберите регламентное задание.';uk='Виберіть регламентне завдання.'") );
	Иначе
			...
			Если ИнформационнаяБазаФайловая Тогда
				...
				Если ВыполнитьРегламентноеЗаданиеВручнуюНаСервере(
					...
					ПоказатьОповещениеПользователя(
						НСтр("ru='Выполнена процедура регламентного задания';uk='Виконана процедура регламентного завдання'"),
						,
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1."
"Процедура выполнялась с %2 по %3';uk='%1. Процедура виконувалася з %2 по %3'"),
					...
					Если ПроцедураУжеВыполняется Тогда
						...
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Процедура регламентного задания ""%1"" уже выполняется в сеансе %2, открытом %3.';uk='Процедура регламентного завдання ""%1"" уже виконується в сеансі %2, відкритому %3.'"),
								ТекущиеДанные.Наименование,
								Строка(НомерСеанса),
								Строка(НачалоСеанса)));
					Иначе
			...
			Иначе
				...
				Если ВыполнитьРегламентноеЗаданиеВручнуюНаСервере(
					...
					ПоказатьОповещениеПользователя(
						НСтр("ru='Запущена процедура регламентного задания';uk='Запущена процедура регламентного завдання'"),
						,
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1."
"Процедура запущена в фоновом задании %2';uk='%1. Процедура запущена у фоновому завданні %2'"),
							ТекущиеДанные.Наименование,
							Строка(МоментЗапуска)),
						БиблиотекаКартинок.ВыполнитьРегламентноеЗаданиеВручную);
				...
				Иначе
					Если ПроцедураУжеВыполняется Тогда
						...
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Процедура регламентного задания ""%1"" уже выполняется в фоновом задании ""%2"", начатом %3.';uk='Процедура регламентного завдання ""%1"" уже виконується у фоновому завданні ""%2"", початому %3.'"),
								ТекущиеДанные.Наименование,
								ПредставлениеФоновогоЗадания,
								Строка(МоментЗапуска)));
					Иначе
...
Процедура ЗапуститьОтдельныйСеанс(Команда)
	...
	Состояние(НСтр("ru='Запуск отдельного сеанса для выполнения регламентных заданий.';uk='Запуск окремого сеансу для виконання регламентних завдань.'"),
	          ,
	          НСтр("ru='Пожалуйста, подождите...';uk=' Будь ласка, почекайте...'"));
...
Процедура ОтменитьФоновоеЗаданиеВыполнить(Команда)
	...
	Если Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные = Неопределено Тогда
		Предупреждение( НСтр("ru='Выберите фоновое задание.';uk='Виберіть фонове завдання.'") );
...
"возможно потребуется обновить данные вручную.';uk='Завдання скасоване, але стан скасування буде установлене сервером тільки через секунди, можливо буде потрібно обновити дані вручну.'"));
	КонецЕсли;
...
Процедура ЗаполнитьНастройкиФормы(Знач Настройки)
	...
	ЭтаФорма["ОтбиратьПоРегламентномуЗаданию"] = Ложь;	
...
Процедура СообщитьОбОкончанииРучногоВыполненияРегламентногоЗадания()
	...
	Для каждого Оповещение Из ОповещенияОбОкончанииВыполнения Цикл
		...
		ПоказатьОповещениеПользователя(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1."
"Процедура завершена в фоновом задании %2';uk='%1. Процедура завершена у фоновому завданні %2'"),
	...
	Для каждого Задание ИЗ ТекущаяТаблица Цикл
		...
		Если ЗначениеЗаполнено(Обновляемое.ИдентификаторРегламентногоЗадания) Тогда
			...
			Обновляемое.НаименованиеРегламентногоЗадания
				= ?(Строки.Количество() = 0, НСтр("ru='<не найдено>';uk='<не знайдене>'"), Строки[0].Наименование);
		Иначе
