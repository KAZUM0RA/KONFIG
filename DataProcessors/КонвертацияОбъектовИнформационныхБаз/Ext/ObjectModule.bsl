...
Функция КлючСообщенияЖурналаРегистрации()
	...
	Если ТипЗнч(КлючСообщенияЖурналаРегистрации) <> Тип("Строка")
		...
		КлючСообщенияЖурналаРегистрации = НСтр("ru='Обмен данными';uk='Обмін даними'");
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	СтрокаСообщения = НСтр("ru='Начало процесса обмена данными для узла: %1';uk='Початок процесу обміну даними для вузла: %1'");
	...
	// запись в журнале регистрации
	СтрокаСообщения = НСтр("ru='%1, %2; Обработано %3 объектов';uk=""%1, %2; Оброблене %3 об'єктів""");
...
Процедура ИнициализироватьВедениеПротоколаОбмена()
	...
	Исключение
		ФайлПротоколаДанных = Неопределено;
		СтрокаСообщения = НСтр("ru='Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';uk='Помилка при спробі запису у файл протоколу даних: %1. Опис помилки: %2'");
...
"Если подходящего значения приемника нет, то указать пустое значение.';uk='У правилі конвертації значень (ПКЗ) необхідно зіставити значення Джерела значенню Приймача. Якщо підходящого значення приймача ні, те вказати порожнє значення.'");
				//
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
    ...
    Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ОписаниеОшибки(), Правило.Имя, Объект, НСтр("ru='ПередВыгрузкойОбъектаВыборки (глобальный)';uk='ПередВыгрузкойОбъектаВыборки (глобальний)'"));
		КонецПопытки;
...
Процедура ВызватьСобытияПослеВыгрузкиОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
    ...
    Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ОписаниеОшибки(), Правило.Имя, Объект, НСтр("ru='ПослеВыгрузкиОбъектаВыборки (глобальный)';uk='ПослеВыгрузкиОбъектаВыборки (глобальний)'"));
		КонецПопытки;
	КонецЕсли;
...
Функция ПолучитьСтруктуруРеквизитовРегистрации(ТаблицаПКС)
	...
	Для Каждого ПКС ИЗ МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Загрузка правил конвертации';uk='Обмін даними.Завантаження правил конвертації'"),
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаРеквизитовРегистрации Цикл
		...
		Иначе // реквизиты табличной части
			...
			Если ОбъектМД.ТабличныеЧасти.Найти(СтрокаТаблицы.ИмяТабличнойЧасти) = Неопределено Тогда
				СтрокаСообщения = НСтр("ru='Неправильно указана табличная часть ""%1"" объекта ""%2"". Табличная часть не существует.';uk='Неправильно зазначена таблична частина ""%1"" об''єкта ""%2"". Таблична частина не існує.'");
			...
			Для Каждого Реквизит ИЗ СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если НЕ (ОбъектМД.ТабличныеЧасти[СтрокаТаблицы.ИмяТабличнойЧасти].Реквизиты.Найти(Реквизит.Ключ) <> Неопределено
					...
					СтрокаСообщения = НСтр("ru='Неправильно указаны реквизиты табличной части ""%1"" объекта ""%2"". Реквизит ""%3"" не существует.';uk='Неправильно зазначені реквізити табличної частини ""%1"" об''єкта ""%2"". Реквізит ""%3"" не існує.'");
...
"Конфигурация приемник: %3';uk='Правила обміну Дата створення:         %1 Конфігурація джерело: %2 Конфігурація приймач: %3'");
					...
					Если СтрокаТаблицы = Неопределено Тогда
						...
						Для Каждого Колонка Из ТаблицаДанныхСообщенияОбмена.Колонки Цикл
							...
							Если Колонка.ТипЗначения.СодержитТип(ТипСтрока) Тогда
								...
								СтрокаТаблицы[Колонка.Имя] = НСтр("ru='удаление объекта';uk=""видалення об'єкта""");
	...
	Для Каждого СтрокаТаблицы ИЗ ДокументыДляОтложенногоПроведения() Цикл
		...
		Попытка
			...
			Иначе
				...
				ОписаниеОшибки = НСтр("ru='Ошибка заполнения реквизитов документа';uk='Помилка заповнення реквізитів документа'");
		...
		Если Не СтрокаТаблицы.ДокументПроведенУспешно Тогда
			...
			СтрокаСообщения = НСтр("ru='Ошибка при отложенном проведении документа: %1. Описание ошибки: %2';uk='Помилка при відкладеному проведенні документа: %1. Опис помилки: %2'");
...
Процедура ВыгрузитьОбщиеДанныеУзлов(Знач НомерОтправленного)
	...
	ТабличныеЧастиОбщегоУзла = ОбменДаннымиПовтИсп.ТабличныеЧастиОбъекта(МетаданныеОбщегоУзла);
...
Процедура ИнициализацияСообщений()
	...
	СообщенияОбОшибках.Вставить(11, НСтр("ru='Ссылка на несуществующий объект метаданных в правилах обмена';uk=""Посилання на неіснуючий об'єкт метаданих у правилах обміну"""));
	СообщенияОбОшибках.Вставить(12, НСтр("ru='Не указано имя файла с правилами обмена (Имя файла правил)';uk=""Не зазначене ім'я файлу із правилами обміну (Ім'я файлу правил)"""));
	...
	СообщенияОбОшибках.Вставить(16, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';uk=""Помилка одержання значення властивості підлеглого об'єкта ( по імені властивості джерела)"""));
	СообщенияОбОшибках.Вставить(17, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';uk=""Помилка одержання значення властивості підлеглого об'єкта ( по імені властивості приймача)"""));
		
	СообщенияОбОшибках.Вставить(19, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';uk='Помилка в оброблювачі події ПередЗагрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(20, НСтр("ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';uk='Помилка в оброблювачі події ПриЗагрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(21, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';uk='Помилка в оброблювачі події ПослеЗагрузкиОбъекта'"));
	СообщенияОбОшибках.Вставить(22, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';uk='Помилка в оброблювачі події ПередЗагрузкойДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(23, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';uk='Помилка в оброблювачі події ПослеЗагрузкиДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(24, НСтр("ru='Ошибка при удалении объекта';uk=""Помилка при видаленні об'єкта"""));
	...
	СообщенияОбОшибках.Вставить(27, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';uk='Помилка в оброблювачі події ПередОбработкойПравилаОчистки'"));
	СообщенияОбОшибках.Вставить(28, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';uk='Помилка в оброблювачі події ПослеОбработкиПравилаОчистки'"));
	СообщенияОбОшибках.Вставить(29, НСтр("ru='Ошибка в обработчике события ПередУдалениемОбъекта';uk='Помилка в оброблювачі події ПередУдалениемОбъекта'"));
	
	СообщенияОбОшибках.Вставить(31, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';uk='Помилка в оброблювачі події ПередОбработкойПравилаВыгрузки'"));
	СообщенияОбОшибках.Вставить(32, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';uk='Помилка в оброблювачі події ПослеОбработкиПравилаВыгрузки'"));
	СообщенияОбОшибках.Вставить(33, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в оброблювачі події ПередВыгрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(34, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';uk='Помилка в оброблювачі події ПослеВыгрузкиОбъекта'"));
			
	СообщенияОбОшибках.Вставить(39, НСтр("ru='Ошибка при выполнении алгоритма, содержащегося в файле обмена';uk='Помилка при виконанні алгоритму, що втримується у файлі обміну'"));
			
	СообщенияОбОшибках.Вставить(41, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в оброблювачі події ПередВыгрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(42, НСтр("ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';uk='Помилка в оброблювачі події ПриВыгрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(43, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';uk='Помилка в оброблювачі події ПослеВыгрузкиОбъекта'"));
	...
	СообщенияОбОшибках.Вставить(48, НСтр("ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';uk='Помилка в оброблювачі події ПередОбработкойВыгрузки групи властивостей'"));
	СообщенияОбОшибках.Вставить(49, НСтр("ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';uk='Помилка в оброблювачі події ПослеОбработкиВыгрузки групи властивостей'"));
	СообщенияОбОшибках.Вставить(50, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';uk=""Помилка в оброблювачі події ПередВыгрузкой (об'єкта колекції)"""));
	СообщенияОбОшибках.Вставить(51, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';uk=""Помилка в оброблювачі події ПриВыгрузке (об'єкта колекції)"""));
	СообщенияОбОшибках.Вставить(52, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';uk=""Помилка в оброблювачі події ПослеВыгрузки (об'єкта колекції)"""));
	СообщенияОбОшибках.Вставить(53, НСтр("ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередЗагрузкойОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(54, НСтр("ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПослеЗагрузкиОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(55, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';uk='Помилка в оброблювачі події ПередВыгрузкой (властивості)'"));
	СообщенияОбОшибках.Вставить(56, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (свойства)';uk='Помилка в оброблювачі події ПриВыгрузке (властивості)'"));
	СообщенияОбОшибках.Вставить(57, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';uk='Помилка в оброблювачі події ПослеВыгрузки (властивості)'"));
	
	СообщенияОбОшибках.Вставить(62, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';uk='Помилка в оброблювачі події ПередВыгрузкойДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(63, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';uk='Помилка в оброблювачі події ПослеВыгрузкиДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(64, НСтр("ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередКонвертациейОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(65, НСтр("ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередВыгрузкойОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(66, НСтр("ru='Ошибка получения коллекции подчиненных объектов из входящих данных';uk=""Помилка одержання колекції підлеглих об'єктів із вхідних даних"""));
	СообщенияОбОшибках.Вставить(67, НСтр("ru='Ошибка получения свойства подчиненного объекта из входящих данных';uk=""Помилка одержання властивості підлеглого об'єкта із вхідних даних"""));
	...
	СообщенияОбОшибках.Вставить(69, НСтр("ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПослеВыгрузкиОбъекта (конвертація)'"));
	...
	СообщенияОбОшибках.Вставить(73, НСтр("ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';uk='Помилка в оброблювачі події ПоследовательностьПолейПоиска'"));
	...
	СообщенияОбОшибках.Вставить(75, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)';uk='Помилка в оброблювачі події ПослеЗагрузкиПравилОбмена (конвертація)'"));
	СообщенияОбОшибках.Вставить(76, НСтр("ru='Ошибка в обработчике события ПередОтправкойИнформацииОбУдалении (конвертация)';uk='Помилка в оброблювачі події ПередОтправкойИнформацииОбУдалении (конвертація)'"));
	СообщенияОбОшибках.Вставить(77, НСтр("ru='Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)';uk='Помилка в оброблювачі події ПриПолученииИнформацииОбУдалении (конвертація)'"));
	...
	СообщенияОбОшибках.Вставить(79, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';uk='Помилка в оброблювачі події ПослеВыгрузкиОбъектаВФайл'"));
	
	СообщенияОбОшибках.Вставить(80, НСтр("ru='Ошибка установки свойства предопределенного элемента."
"Нельзя помечать на удаление предопределенный элемент. Пометка на удаление для объекта не установлена.';uk=""Помилка установки властивості визначеного елемента.  Не можна позначати на видалення визначений елемент. Позначка на видалення для об'єкта не встановлена."""));
	//
	СообщенияОбОшибках.Вставить(81, НСтр("ru='Возникла коллизия изменений объектов."
"Объект этой информационной базы был заменен версией объекта из второй информационной базы.';uk=""Виникла колізія змін об'єктів. Об'єкт цієї інформаційної бази був замінений версією об'єкта із другої інформаційної бази."""));
	//
	СообщенияОбОшибках.Вставить(82, НСтр("ru='Возникла коллизия изменений объектов."
"Объект из второй информационной базы не принят. Объект этой информационной базы не изменен.';uk=""Виникла колізія змін об'єктів. Об'єкт із другої інформаційної бази не прийнятий. Об'єкт цієї інформаційної бази не змінений."""));
	//
	...
	СообщенияОбОшибках.Вставить(175, НСтр("ru='Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)';uk='Помилка в оброблювачі події ПередПолучениемИзмененныхОбъектов (конвертація)'"));
	СообщенияОбОшибках.Вставить(176, НСтр("ru='Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';uk='Помилка в оброблювачі події ПослеПолученияИнформацииОбУзлахОбмена (конвертація)'"));
