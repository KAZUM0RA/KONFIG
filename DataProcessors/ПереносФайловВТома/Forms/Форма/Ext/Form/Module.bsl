...
Процедура ВыполнитьПереносФайловВТома(Команда)
	...
	Если ФайловыеФункции.ПолучитьТипХраненияФайлов() <> ТипХраненияВТомах Тогда
		Предупреждение(НСтр("ru='Не установлен тип хранения файлов ""В томах на диске""';uk='Не встановлений тип зберігання файлів ""У томах на диску""'"));
		Возврат;
	КонецЕсли;	
	
	Если Не ФайловыеФункции.ЕстьТомаХраненияФайлов() Тогда
		Предупреждение(НСтр("ru='Нет ни одного тома для размещения файлов';uk='Немає жодного тому для розміщення файлів'"));
		Возврат;
	КонецЕсли;	
	
	Если ЧислоВерсийВБазе = 0 Тогда
		Предупреждение(НСтр("ru='Нет ни одного файла в информационной базе';uk='Немає жодного файлу в інформаційній базі'"));
		Возврат;
	КонецЕсли;	
	
	Результат = Вопрос(
	  НСтр("ru='Выполнить перенос файлов в информационной базе в тома хранения файлов?"
" Эта операция может занять продолжительное время.';uk='Виконати перенос файлів в інформаційній базі в томи зберігання файлів?  Ця операція може забрати тривалий час.'"),
	...
	Состояние(НСтр("ru='Выполняется получение списка файлов...';uk='Виконується одержання списку файлів...'"));
	...
	Для Каждого ВерсияСтруктура Из МассивВерсий Цикл
		...
		Состояние(НСтр("ru='Выполняется перенос файла в том...';uk='Виконується перенос файлу в тому...'"), Прогресс, ВерсияСтруктура.Текст, БиблиотекаКартинок.УстановитьВремя);
	...
	Если ЧислоПеренесенных <> 0 Тогда
		СтрокаСообщения
			...
			НСтр("ru='Завершен перенос файлов в тома. Перенесено файлов: %1';uk='Завершений перенос файлів у томи. Перенесене файлів: %1'"),
			ЧислоПеренесенных);
	...
	Если МассивФайловСОшибками.Количество() <> 0 Тогда
		...
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Количество ошибок при переносе: %1';uk='Кількість помилок при переносі: %1'"),
			МассивФайловСОшибками.Количество());
			
		Если ОбработкаПрервана Тогда
			Пояснение = НСтр("ru='Не удалось перенести ни одного файла из пакета. Перенос прерван.';uk='Не вдалося перенести жодного файлу з пакета. Перенос перерваний.'");
		КонецЕсли;	
