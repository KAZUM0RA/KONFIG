...
Процедура ЗавершитьСеанс(Команда)
	...
	Если НомерСеансаДляЗавершения = НомерСеансаИнформационнойБазы Тогда
		Предупреждение(НСтр("ru='Невозможно завершить текущий сеанс. Для выхода из программы можно закрыть главное окно программы.';uk='Неможливо завершити поточний сеанс. Для виходу із програми можна закрити головне вікно програми.'"));
		Возврат;
	КонецЕсли;
	...
	Если Отключено Тогда
		Предупреждение(НСтр("ru='Сеанс завершен.';uk='Сеанс завершений.'"));
	КонецЕсли;
...
Процедура ОткрытьЖурналРегистрации()
	...
	Если Элементы.СписокПользователей.ВыделенныеСтроки.Количество() > 1 Тогда
		Предупреждение(НСтр("ru='Для просмотра журнала регистрации выберите в списке только одного пользователя.';uk=' Для перегляду журналу реєстрації виберіть у списку тільки одного користувача.'"));
		Возврат;
	КонецЕсли;
		
	ТекущиеДанные = Элементы.СписокПользователей.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Предупреждение(НСтр("ru='Невозможно открыть журнал регистрации для выбранного пользователя.';uk='Неможливо відкрити журнал реєстрації для обраного користувача.'"));
		Возврат;
	КонецЕсли;
