...
Процедура ПриОткрытии(Отказ)
	...
	Если НекорректныйВызовОбработки Тогда
		Отказ = Истина;
		Предупреждение(НСтр("ru='Самостоятельное использование формы не предусмотрено.';uk='Самостійне використання форми не передбачене.'"));
		Возврат;
	КонецЕсли;
...
Процедура Изменить(Команда)
	...
	Если Элементы.ФормаИзменить.Заголовок = НСтр("ru='Изменить';uk='Змінити'") Тогда
	
		Если Объект.Операции.НайтиСтроки(Новый Структура("Изменять", Истина)).Количество() = 0 Тогда
			
			ТекстВопроса = НСтр("ru='Не задано ни одного изменения. Выполнить перезапись данных без изменений?';uk='Не задане жодного зміни. Виконати перезапис даних без змін?'");
			Результат = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , 
				НСтр("ru='Изменение объектов';uk=""Зміна об'єктів"""));
...
Процедура ИсторияОпераций(Команда)
	...
	Если ИсторияОперацийСписок.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Для данного типа объектов нет истории ранее выполненных групповых изменений.';uk="" Для даного типу об'єктів немає історії раніше виконаних групових змін."""));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьОперацииИзменения(МассивОпераций)
	...
	Если ЕстьЗаблокированные Тогда
		Предупреждение(НСтр("ru='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';uk='Деякі реквізити заблоковані для зміни, зміни не встановлені.'"));
	КонецЕсли;
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов()
	...
	Результат = Вопрос(ТекстВопроса, Кнопки, , КодВозвратаДиалога.Да, НСтр("ru='Реквизит заблокирован';uk='Реквізит заблокований'"));
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Иначе
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ru='Изменить';uk='Змінити'");
	КонецЕсли;
...
Процедура ИзменитьОбъекты()
	...
	Если КоличествоОбъектовДляОбработки = 0 Тогда
		...
		Предупреждение(НСтр("ru='Не указаны объекты для изменения';uk=""Не зазначені об'єкти для зміни"""));
		Возврат;
	КонецЕсли;
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("ru='Групповое изменение объектов';uk=""Групова зміна об'єктів"""), ,НСтр("ru='Пожалуйста подождите, обработка может занять некоторое время...';uk=' Будь ласка почекайте, обробка може зайняти якийсь час...'"));
	...
	Иначе
		...
		Состояние(НСтр("ru='Обрабатываются объекты...';uk=""Обробляються об'єкти..."""), 0, НСтр("ru='Групповое изменение объектов';uk=""Групова зміна об'єктів"""));
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения)
	...
	Пока Истина Цикл
		...
		Если Объект.ИзменятьВТранзакции Тогда
			ТекстПредупреждения = НСтр("ru='При изменении объектов обнаружены ошибки - изменения отменены.';uk="" При зміні об'єктів виявлені помилки - зміни скасовані.""");
...
"';uk='При изменении объекта (группы объектов) возникли ошибки."
"Прервать изменение объектов и перейти к просмотру ошибок?"
"'");
		Кнопки = Новый СписокЗначений;
		...
		Результат = Вопрос(ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, НСтр("ru='Ошибки при изменении объектов';uk=""Помилки при зміні об'єктів"""));
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("ru='Обрабатываются объекты...';uk=""Обробляються об'єкти..."""), ТекущийПроцент, НСтр("ru='Групповое изменение объектов';uk=""Групова зміна об'єктів"""));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если Объект.ИзменятьВТранзакции
	   И ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
		ПоказатьОповещениеПользователя(НСтр("ru='Групповое изменение объектов';uk=""Групова зміна об'єктів"""), , НСтр("ru='Обработка объектов завершена...';uk=""Обробка об'єктів завершена..."""));
	КонецЕсли;
	...
	Если ТекущееСостояниеИзменения.КоличествоОшибок > 0 Тогда
		...
		Элементы.ПодсказкаПослеИзмененияЕстьОшибки.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось изменить объекты. Измененно объектов %1 из %2.';uk=""Не вдалося змінити об'єкти. Змiнено об'єктів %1 з %2."""),
...
"Изменено объектов: %1.';uk=""Зміна об'єктів успішно завершена. Змінене об'єктів: %1."""),
...
Функция ИзменитьНаСервере(ОбъектыДляОбработки, знач ОстанавливатьИзменениеПриОшибке)
	...
	Иначе
		НаименованиеЗадания = НСтр("ru='Групповое изменение объектов';uk=""Групова зміна об'єктів""");
...
Функция ТекстПодсказкиСтраницыДействий(КоличествоОбъектов)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='"
"Нажмите кнопку ""Далее"", чтобы перейти к просмотру изменяемых объектов (%1)';uk=' Натисніть кнопку ""Далі"", щоб перейти до перегляду змінюваних об''єктів (%1)'"),
				КоличествоОбъектов);
