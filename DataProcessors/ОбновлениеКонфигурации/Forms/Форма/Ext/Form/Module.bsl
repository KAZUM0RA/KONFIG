...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаписьЖурналаРегистрации(ОбновлениеКонфигурации.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,,
		НСтр("ru='Открытие помощника обновления конфигурации...';uk='Відкриття помічника відновлення конфігурації...'"));
...
#Если ВебКлиент Тогда
	Отказ = Истина;
	ТекстСообщения = НСтр("ru='Обновление конфигурации информационной базы недоступно в веб-клиенте.';uk='Відновлення конфігурації інформаційної бази недоступно у веб-клієнтові.'");
	...
	Иначе
		Если НЕ ОбновлениеКонфигурацииКлиентПереопределяемый.ГотовностьКОбновлениюКонфигурации(Истина) Тогда
			...
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
				"Информация", НСтр("ru='Конфигурация не может быть обновлена. Завершение проверки обновления.';uk='Конфігурація не може бути оновлена. Завершення перевірки відновлення.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли; 
...
Процедура ПолеФайлОбновленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Диалог.ПроверятьСуществованиеФайла = Истина;
	Диалог.Фильтр		= НСтр("ru='Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации(*.cfu)|*.cfu';uk='Усі файли поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файли поставки конфігурації (*.cf)|*.cf|Файли поставки відновлення конфігурації(*.cfu)|*.cfu'");
	Диалог.Заголовок	= НСтр("ru='Выбор поставки обновления конфигурации';uk='Вибір поставки відновлення конфігурації'");
...
Процедура НадписьИнформацияОбОбновленииНажатие(Элемент)
	...
	Иначе
		Предупреждение(НСтр("ru='Информация об обновлении отсутствует.';uk=' Інформація об відновленні відсутня.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура НадписьПорядокОбновленияНажатие(Элемент)
	...
	Иначе
		Предупреждение(НСтр("ru='Описание порядка обновления отсутствует.';uk='Опис порядку відновлення вiдсутнiй.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура НадписьПереченьРелизовНаДискахИТСНажатие(Элемент)
	Если ПустаяСтрока(ПереченьРелизовНаДискахИТС) Тогда
		Предупреждение(НСтр("ru='Информация о перечне релизов не обнаружена.';uk='Інформація про перелік релізів не виявлена.'"));
	КонецЕсли;
...
Функция ПроверитьИУстановитьПарольАдминистратора(СообщенияДляЖурналаРегистрации)
	...
	Если ТекущийПользователь.АутентификацияОС И НЕ ТекущийПользователь.АутентификацияСтандартная Тогда
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось установить пароль администратора: ';uk='Не вдалося встановити пароль адміністратора: '");
	...
	Пока ТекущаяВерсияОт <> ВерсияДо Цикл
		...
		Если ТекущаяВерсияОт = НоваяТекущаяВерсияОт И НоваяТекущаяВерсияОт <> ВерсияДо Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недоступно обновление на версию %1 с текущей версии %2';uk='Недоступне відновлення на версію %1 з поточної версії %2'"), ВерсияДо, ВерсияОт);
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуСпискаОбновлений(Знач АдресФайлаЗагрузки, ТаблицаОбновлений = Неопределено)
	...
	СообщениеОбОшибке = НСтр("ru='Ошибка при чтении файла списка обновлений ';uk='Помилка при читанні файлу списку відновлень '") + ПолныйПутьФайлаЗагрузки;
	...
	Если ПараметрыРаботыКлиента.НастройкиОбновления.ЕстьДоступДляОбновления 
		...
		Иначе
			...
			Исключение
				...
				ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,, 
					НСтр("ru='Ошибка при восстановлении настроек обновления:';uk='Помилка при відновленні настроювань відновлення:'") + " " +
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено) 
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ДоступноеОбновление Тогда
		...
		Если СтруктураДоступногоОбновления <> Неопределено Тогда
			...
			Элементы.НадписьДоступноеОбновление.Заголовок		= НСтр("ru='Версия ';uk='Версія '") + СтруктураДоступногоОбновления.Версия;
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ОбновленияНеОбнаружено Тогда
		...
		Элементы.НадписьВерсияДляОбновления.Заголовок			= ?(ТипЗнч(СтруктураДоступногоОбновления) = Тип("Структура"), НСтр("ru='Доступная версия для обновления - ';uk='Доступна версія для відновлення - '") + СтруктураДоступногоОбновления.Версия, "");
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ВерсияКонфигурации,
			...
			Элементы.ЗаголовокОбновления.Заголовок					= НСтр("ru='Обновление не требуется';uk='Відновлення не потрібно'");
		Иначе 
			...
			Элементы.ЗаголовокОбновления.Заголовок					= НСтр("ru='Обновления конфигурации не обнаружено';uk='Відновлення конфігурації не виявлене'");
		КонецЕсли;
	...
	КнопкаДалее.Заголовок = ? (ФункцияКнопкиДалее, НСтр("ru='&Далее >';uk='&Далі >'"), НСтр("ru='&Готово';uk='&Готове'"));
...
Функция ТекстНадписиНеобходимыеДискиИТС()
	...
	Для каждого ТекДоступноеОбновление Из Объект.ДоступныеОбновления Цикл
		Если НЕ ПустаяСтрока(ТекДоступноеОбновление.НомерДискаИТС) И НЕ ПустаяСтрока(ТекДоступноеОбновление.Версия) И
			...
			Результат = Результат + ?(ПустаяСтрока(Результат), "", Символы.ПС) + ТекДоступноеОбновление.НомерДискаИТС + НСтр("ru=' - Версия ';uk=' - Версія '") + ТекДоступноеОбновление.Версия;
		КонецЕсли;
	КонецЦикла;
...
Функция ВосстановитьРезультатыПредыдущегоЗапуска(Отказ = Ложь)
	...
	Если ИмяСтраницы = Страницы.ДоступноеОбновление.Имя Тогда
		...
		Если ПроверитьФайлыОбновленияПолучены() Тогда
			...
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
				"Информация",  
				НСтр("ru='Обнаружено, что файлы обновления конфигурации уже были получены и сохранены локально.';uk='Виявлене, що файли відновлення конфігурації вже були отримані й збережені локально.'"));
...
Функция ОбработатьСтраницуПодключениеКСайту(ВыдаватьСообщения = Истина)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодПользователяСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru='Укажите код пользователя сервера обновлений.';uk='Укажіть код користувача сервера відновлень.'"));
		КонецЕсли;	
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПарольСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru='Укажите пароль пользователя сервера обновлений.';uk='Укажіть пароль користувача сервера відновлень.'"));
		КонецЕсли;
...
Функция ОбработатьСтраницуВыборРежимаОбновления(ВыдаватьСообщения = Истина, ФлагЗавершенияРаботы = Ложь)
	...
	Если Объект.РежимОбновления = 0 Тогда   // Обновить сейчас
		...
		Если ИнформационнаяБазаФайловая И НЕ РежимИмитацииКлиентСервернойИБ() Тогда
			...
			Если НаличиеСоединений Тогда
				Предупреждение(НСтр("ru='Невозможно продолжить обновление конфигурации, так как не завершены все соединения с информационной базой.';uk=""Неможливо продовжити відновлення конфігурації, тому що не завершені всі з'єднання з інформаційною базою."""));
	...
	ИначеЕсли Объект.РежимОбновления = 2 Тогда  // Запланировать обновление
		...
		Если НЕ ЗапланироватьОбновлениеКонфигурации() Тогда
			Предупреждение(НСтр("ru='Невозможно запланировать обновление конфигурации. Сведения об ошибке сохранены в журнал регистрации.';uk='Неможливо запланувати відновлення конфігурації. Відомості про помилку збережені в журнал реєстрації.'"));
			Возврат ТекущаяСтраница.Имя;
		КонецЕсли;
	...
	Сообщение = НСтр("ru='Выбран режим обновления: ';uk='Обраний режим відновлення: '");
	Если Объект.РежимОбновления = 0 Тогда   // Обновить сейчас
...
Процедура ОбработатьНажатиеКнопкиДалее(ФлагЗавершенияРаботы = Ложь) 
	...
	Если НЕ СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().НастройкиОбновления.ЕстьДоступДляОбновления
		...
		Предупреждение(НСтр("ru='Недостаточно прав для выполнения обновления конфигурации.';uk='Недостатньо правий для виконання відновлення конфігурації.'"));
	КонецЕсли;
...
Функция ОбработатьСтраницуДоступноеОбновление(ВыдаватьСообщения = Истина)
	...
	Если ПроверитьФайлыОбновленияПолучены() Тогда
		ИмяСобытияЖурнала  = ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
			"Информация", НСтр("ru='Обнаружено, что файлы обновления конфигурации уже были получены и сохранены локально.';uk='Виявлене, що файли відновлення конфігурації вже були отримані й збережені локально.'"));
...
Функция ОбработатьСтраницуФайлОбновления(ВыдаватьСообщения = Истина)
	...
	Если Объект.НуженФайлОбновления = 1 Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ИмяФайлаОбновления) Тогда
			Если ВыдаватьСообщения Тогда
				Предупреждение(НСтр("ru='Укажите файл поставки обновления конфигурации.';uk='Укажіть файл поставки відновлення конфігурації.'"));
			КонецЕсли;
		...
		Если НЕ Файл.Существует() ИЛИ НЕ Файл.ЭтоФайл() Тогда
			Если ВыдаватьСообщения Тогда
				Предупреждение(НСтр("ru='Файл поставки обновления конфигурации не найден.';uk='Файл поставки відновлення конфігурації не знайдений.'"));
			КонецЕсли;
			ТекущийЭлемент = Элементы.ПолеФайлОбновления;
			Возврат Страницы.ФайлОбновления.Имя;
		КонецЕсли;
...
Функция ОбработатьСтраницуПараметрыАвторизации(ВыдаватьСообщения = Истина)
	...
	Если НЕ ПустаяСтрока(ТекстОшибки) И ВыдаватьСообщения Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru='Не удалось установить пароль администратора: ';uk='Не вдалося встановити пароль адміністратора: '") + КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецЕсли;
		Возврат Страницы.ПараметрыАвторизации.Имя;
	КонецЕсли; 
...
Функция НайтиДискИТС(ПутиДискаИТС = "")
	...
	Исключение
		ИмяСобытияЖурнала = ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
			"Ошибка", НСтр("ru='Ошибка при поиске диска ИТС: ';uk='Ошибка при поиске диска ИТС: '") + ОписаниеОшибки());
		Возврат "";	
	КонецПопытки;
...
Функция ПроверитьДоступКИБ()
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru=' Проверка подключения к ИБ по адресу: %1, с помощью %2';uk=' Проверка подключения к ИБ по адресу: %1, с помощью %2'"),
...
Функция ПроверитьДоступККластеру(ПараметрыПодключения)
	...
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
		Результат = НСтр("ru='Ошибка при проверке соединения с кластером серверов. ';uk=""Помилка при перевірці з'єднання із кластером серверів. """);
		ИмяСобытияЖурнала = ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
...
Функция СоздатьЗадачуПланировщика(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Попытка
		...
		Если КодОшибки <> 0 Тогда	// Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
				"Ошибка", НСтр("ru='Ошибка при создании задачи планировщика: ';uk='Помилка при створенні завдання планувальника: '") + КодОшибки);
			Возврат 0;
		КонецЕсли;		
		ТекстСообщения = НСтр("ru='Задача планировщика успешно запланирована (команда: %1; дата: %2; код задачи: %3).';uk='Завдання планувальника успішно заплановане (команда: %1; дата: %2; код завдання: %3).'");
	...
	Исключение
		...
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
			"Ошибка", НСтр("ru='Ошибка при создании задачи планировщика: ';uk='Помилка при створенні завдання планувальника: '") + ОписаниеОшибки());
		Возврат 0;	
	КонецПопытки;
...
Функция ПроверитьДопустимуюДатуОбновленияНаСервере(ДатаВремя)
	...
	Если ДатаВремя < Сейчас Тогда
		Возврат НСтр("ru='Обновление конфигурации может быть запланировано только на будущую дату и время.';uk='Відновлення конфігурації може бути заплановане тільки на майбутню дату й час.'");
	КонецЕсли;
	Если ДатаВремя > ДобавитьМесяц(Сейчас, 1) Тогда
		Возврат НСтр("ru='Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';uk='Відновлення конфігурації може бути заплановане не пізніше, чим через місяць щодо поточної дати.'");
	КонецЕсли;
...
Функция УдалитьЗадачуПланировщика(КодЗадачи)
	...
	Попытка
		...
		Если НЕ Результат Тогда	// Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
				"Ошибка", НСтр("ru='Ошибка при удалении задачи планировщика: ';uk='Помилка при видаленні завдання планувальника: '") + КодОшибки);
			Возврат Результат;
		КонецЕсли;		
		ТекстСообщения = НСтр("ru='Задача планировщика успешно удалена (код задачи: %1).';uk='Завдання планувальника успішно вилучене (код завдання: %1).'");
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
	...
	Исключение
		...
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
			"Ошибка", НСтр("ru='Ошибка при удалении задачи планировщика: ';uk='Помилка при видаленні завдання планувальника: '") + ОписаниеОшибки());
		Возврат Ложь;	
	КонецПопытки;
...
Функция ВводитьПарольАдминистратора()
	...
	Возврат ЕстьАктивныеПользователи(СообщенияДляЖурналаРегистрации) И
		(ДанныеПоПользователю.АутентификацияОС И НЕ ДанныеПоПользователю.АутентификацияСтандартная);
КонецФункции
...
Функция ЗапрашиватьПарольАдминистратора()
	ДанныеПоПользователю	= СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ИнформацияОПользователе;
	Возврат ЕстьАктивныеПользователи(СообщенияДляЖурналаРегистрации) И 
		((ДанныеПоПользователю.ПарольУстановлен И ДанныеПоПользователю.АутентификацияСтандартная) ИЛИ
		(ДанныеПоПользователю.АутентификацияОС И НЕ ДанныеПоПользователю.АутентификацияСтандартная));
КонецФункции
...
"%2';uk='Помилка при видаленні тимчасового файлу %1 %2'");
		ИмяСобытияЖурнала =	ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,
...
Процедура РаспаковатьРекламу() 
	...
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Информация", 
		НСтр("ru='Выполняется распаковка файлов рекламы...';uk='Виконується розпакування файлів реклами...'"));
	...
	Если ФайлСуществуетНаКлиенте(ПараметрыОбновления.ПутьКФайлуАрхиваРекламы) Тогда
		Попытка 
			...
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
				"Информация", НСтр("ru='Файлы рекламы успешно обработаны.';uk='Файли реклами успішно оброблені.'"));
		Исключение
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
				"Ошибка", НСтр("ru='Ошибка при распаковке файлов рекламы: ';uk='Помилка при розпакуванні файлів реклами: '") + ОписаниеОшибки());
		КонецПопытки;
...
Функция РаспаковатьДистрибутивОбновления()
	...
	ИмяСобытияЖурнала = ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
		"Информация", НСтр("ru='Выполняется распаковка дистрибутива обновления...';uk='Виконується розпакування дистрибутива відновлення...'"));
	...
	Для Каждого Обновление Из Объект.ДоступныеОбновления Цикл
		...
		Попытка 
			...
			Если НЕ ФайлСуществуетНаКлиенте(ИмяФайлаОбновления) Тогда
				ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
					"Ошибка",  НСтр("ru='Дистрибутив обновления не содержит 1cv8.cfu: ';uk='Дистрибутив відновлення не містить 1cv8.cfu: '") + Обновление.ПутьКЛокальномуФайлу);
				Возврат Ложь;
			КонецЕсли;
			Обновление.ПутьКЛокальномуФайлуОбновления = ИмяФайлаОбновления;
			
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,
				"Информация",  НСтр("ru='Файлы дистрибутива обновления успешно распакованы: ';uk='Файли дистрибутива відновлення успішно розпаковані: '") + ИмяФайлаОбновления);
		Исключение
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
				"Ошибка",  НСтр("ru='Ошибка при распаковке дистрибутива обновления: ';uk='Помилка при розпакуванні дистрибутива відновлення: '") + ОписаниеОшибки());
			Возврат Ложь;
		КонецПопытки;
...
Функция ПолучитьФайлСпискаШаблонов(Знач ВыдаватьСообщения = Истина)
	...
	Если Результат.Статус <> Истина Тогда
		...
		Исключение
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(),
				"Ошибка", 
				НСтр("ru='Ошибка при удалении временного файла ';uk='Помилка при видаленні тимчасового файлу '") + ПутьКФайлуСпискаШаблонов + Символы.ПС +
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	...
	Исключение
		ТекстОшибки	= НСтр("ru='Ошибка при распаковке файла со списком доступных обновлений: ';uk='Помилка при розпакуванні файлу зі списком доступних відновлень: '");
...
Функция ПолучитьОбновление(ВыдаватьСообщения = Истина)
	...
	ИначеЕсли Объект.ИсточникОбновления = 1 Тогда
		Сообщение = Сообщение + НСтр("ru='Получение файлов с диска ИТС...';uk='Получение файлов с диска ИТС...'");
	Иначе
		Сообщение = Сообщение + НСтр("ru='Получение файла обновления из указанного источника...';uk='Одержання файлу відновлення із зазначеного джерела...'");
	КонецЕсли;
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
		"Информация", Сообщение);
		...
		Если Файл.Значение <> Неопределено И Файл.Значение.Получен <> Истина Тогда	// может быть также Неопределено
			Если Объект.ИсточникОбновления = 0 Тогда
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru=' Получаем файл %1 в %2';uk=' Одержуємо файл %1 в %2'"),
				...
				Если Результат.Статус <> Истина Тогда
					...
					Исключение
						ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
							"Ошибка", НСтр("ru='Ошибка при удалении временного файла ';uk='Помилка при видаленні тимчасового файлу '") +
			...
			Иначе
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru=' Получаем файл %1 в %2';uk=' Одержуємо файл %1 в %2'"), Файл.Значение.Адрес, Файл.Значение.ЛокальныйПуть);
				Элементы.НадписьПрогресс.Заголовок = Сообщение;
				ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), , Сообщение);
	...
	Если ФайлыПолученыУспешно Тогда
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
			"Информация", НСтр("ru='Файлы обновления успешно получены.';uk='Файли відновлення успішно отримані.'"));
		Если Объект.ИсточникОбновления = 1 Тогда
		...
		КонецЕсли;
	Иначе
		Сообщение = НСтр("ru='Не удалось получить обновление конфигурации.';uk='Не вдалося одержати відновлення конфігурації.'");
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
			"Ошибка", Сообщение);
...
Функция ПолучитьФайлыОбновленияЧерезИнтернет(ВыдаватьСообщения)
	...
	Если ДоступноеОбновление.ИмяСтраницы = Страницы.ДоступноеОбновление.Имя Тогда
		...
		Если Объект.ДоступныеОбновления.Количество() = 0 Тогда
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
				"Информация", НСтр("ru='Невозможно продолжить обновление: нет доступных обновлений.';uk='Неможливо продовжити відновлення: немає доступних відновлень.'"));
			Возврат Страницы.ОбновленияНеОбнаружено.Имя;
		КонецЕсли;
...
Функция ПроверитьОбновлениеИнтернет(ВыдаватьСообщения = Истина) 
	Страницы		= Элементы.СтраницыПомощника.ПодчиненныеЭлементы;
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
		"Информация", НСтр("ru='Проверка обновления в Интернете...';uk='Перевірка відновлення в Інтернеті...'"));
...
Функция ПроверитьОбновлениеИТС(ВыдаватьСообщения = Истина)
	Страницы					= Элементы.СтраницыПомощника.ПодчиненныеЭлементы;
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
		"Информация",  НСтр("ru='Проверка обновления на диске ИТС...';uk='Проверка обновления на диске ИТС...'"));
	ДоступноеОбновление			= Неопределено;
	...
	Если Объект.ИсточникДискаИТС <> 1 Тогда
		...
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(),
			"Информация",  НСтр("ru='Обнаружен диск ИТС: ';uk='Обнаружен диск ИТС: '") + ДискИТС);
	Иначе
		Если ПустаяСтрока(ДискИТС) Тогда
			Предупреждение(НСтр("ru='Укажите местоположение диска ИТС.';uk='Укажите местоположение диска ИТС.'"));
		...
		Если НЕ ПроверкаСуществования.Существует() ИЛИ НЕ ПроверкаСуществования.ЭтоКаталог() Тогда
			Предупреждение(НСтр("ru='Указанный каталог не существует.';uk='Зазначений каталог не існує.'"));
			Возврат Страницы.МестоположениеДискаИТС.Имя;
		КонецЕсли;
...
"%1';uk='Диск ИТС не найден по указанным путям:"
"%1'"), ПутиДискаИТС);
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
...
"%1';uk='На диске ИТС не найден каталог обновления."
"%1'"), ТекущийДискИТС);
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
			"Предупреждение", СтрокаСообщения);
		Элементы.НадписьОбновлениеНеОбнаружено.Заголовок = СтрокаСообщения;
...
"%1';uk='На диске ИТС не найдена информация о последнем обновлении."
"%1'"), ТекущийДискИТС);
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
			"Предупреждение",  СтрокаСообщения);
		Элементы.НадписьОбновлениеНеОбнаружено.Заголовок = СтрокаСообщения;
...
"%1';uk='Немає доступних відновлень. %1'"), ТекущийДискИТС);
		ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
			"Предупреждение", СтрокаСообщения);
		Элементы.НадписьОбновлениеНеОбнаружено.Заголовок = СтрокаСообщения;
		Возврат Страницы.ОбновленияНеОбнаружено.Имя;
	...
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(),
		"Информация", 
		НСтр("ru='Обнаружена более новая версия конфигурации на диске ИТС: ';uk='Обнаружена более новая версия конфигурации на диске ИТС: '") + ПоследняяВерсияКонфигурации + ".");
	
	Если НЕ ПолучитьФайлСпискаШаблоновИТС(ВыдаватьСообщения) Тогда
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='На диске ИТС не найден список доступных обновлений."
"%1';uk='На диске ИТС не найден список доступных обновлений."
"%1'"), ТекущийДискИТС);
...
Функция ПроверитьОбновлениеФайл(ВыдаватьСообщения = Истина)
	...
	Если Объект.НуженФайлОбновления = 1 Тогда
		...
		Если НЕ Файл.Существует() ИЛИ НЕ Файл.ЭтоФайл() Тогда
			...
			ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
				"Информация", НСтр("ru='Невозможно продолжить обновление: файл поставки обновления конфигурации не найден.';uk='Неможливо продовжити відновлення: файл поставки відновлення конфігурації не знайдений.'"));
...
Функция ТекстНадписиРезервнойКопииИБ()
	Результат = НСтр("ru='Не создавать резервную копию ИБ и не выполнять откат при нештатной ситуации';uk='Не создавать резервную копию ИБ и не выполнять откат при нештатной ситуации'");
	Если Объект.СоздаватьРезервнуюКопию = 1 Тогда
		Результат = НСтр("ru='Создавать временную резервную копию ИБ и выполнять откат при нештатной ситуации';uk='Створювати тимчасову резервну копію ІБ і виконувати відкіт за виникнення позаштатної ситуації'");
	ИначеЕсли Объект.СоздаватьРезервнуюКопию = 2 Тогда
		Если Объект.ВосстанавливатьИнформационнуюБазу Тогда
			Результат = НСтр("ru='Создавать резервную копию ИБ и выполнять откат при нештатной ситуации';uk='Створювати резервну копію ІБ і виконувати відкіт за виникнення позаштатної ситуації'");
		Иначе
			Результат = НСтр("ru='Создавать резервную копию ИБ и не выполнять откат при нештатной ситуации';uk='Створювати  резервну копію ІБ і не виконувати відкіт за виникнення позаштатної ситуації'");
		КонецЕсли;
	КонецЕсли; 
	Возврат Результат;
КонецФункции
...
Функция ТекстНадписиПроверкиОбновления()
	Результат = НСтр("ru='Отключена';uk='Відключена'");
	Если Объект.ПроверятьНаличиеОбновленияПриЗапуске = 2 Тогда
		Результат = НСтр("ru='При каждом запуске программы';uk=' При кожному запуску програми'");
	ИначеЕсли Объект.ПроверятьНаличиеОбновленияПриЗапуске = 1 Тогда     
		Результат = НСтр("ru='По расписанию: ';uk=' За розкладом: '") + Строка(Объект.РасписаниеПроверкиНаличияОбновления);
	КонецЕсли; 
	Возврат Результат;
КонецФункции
...
Процедура ВыполнитьОбновлениеКонфигурации()
	...
	ОбщегоНазначенияКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
		"Информация",  
		НСтр("ru='Выполняется процедура обновления конфигурации: ';uk='Виконується процедура відновлення конфігурації: '") + ИмяГлавногоФайлаСкрипта);
