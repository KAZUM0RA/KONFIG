...
Процедура КомандаОК(Команда)
	...
	Если НастройкиИзменены Тогда
		...
		Если ПериодПовтораВТечениеДня > 0 И ПериодПовтораВТечениеДня < 60 * 5 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Интервал проверки не может быть задан чаще, чем один раз 5 минут.';uk='Інтервал перевірки не може бути заданий частіше, чим один раз 5 хвилин.'"));
			Возврат;
		КонецЕсли;
...
Функция ТекстНадписиОткрытьРасписание(Форма)
	...
	Возврат ?(Не ПустаяСтрока(СтроковоеПредставлениеРасписания),
		СтроковоеПредставлениеРасписания, НСтр("ru='Открыть расписание';uk='Відкрити розклад'"));
