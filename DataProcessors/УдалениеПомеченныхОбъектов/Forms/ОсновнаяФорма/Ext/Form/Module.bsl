...
Процедура ВыполнитьДалее()
	...
	Если ТекущаяСтраница = Элементы.ВыборРежимаУдаления Тогда
		Состояние(НСтр("ru='Выполняется поиск помеченных на удаление объектов';uk=""Виконується пошук позначених на видалення об'єктів"""));
...
Процедура ВыполнитьУдаление()
	...
	Если РежимУдаления = "Полный" Тогда
		Состояние(НСтр("ru='Выполняется поиск и удаление помеченных объектов';uk=""Виконується пошук і видалення позначених об'єктів"""));
	Иначе
		Состояние(НСтр("ru='Выполняется удаление выбранных объектов';uk=""Виконується видалення обраних об'єктів"""));
	КонецЕсли;
...
Процедура ОбновитьСодержание(Результат, СообщениеОбОшибке, ТипыУдаленныхОбъектов)
	...
	Если КоличествоНеУдаленныхОбъектов = 0 Тогда
		Если УдаленныхОбъектов = 0 Тогда
			Текст = НСтр("ru='Не помечено на удаление ни одного объекта. Удаление объектов не выполнялось';uk=""Не позначене на видалення жодного об'єкта. Видалення об'єктів не виконувалося""");
			ОбновитьДеревоПомеченных = Ложь;
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			             НСтр("ru='Удаление помеченных объектов успешно завершено."
"Удалено объектов: %1.';uk=""Видалення позначених об'єктів успішно завершене. Вилучене об'єктів: %1."""),
			             УдаленныхОбъектов);
		КонецЕсли;
		Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.ВыборРежимаУдаления;			 
...
Процедура ЗаполнитьСтрокуРезультатов(КоличествоУдаляемых)
	...
	Если УдаленныхОбъектов = 0 Тогда
		СтрокаРезультатов = НСтр("ru='Не удален ни один из объектов, так как в информационной базе существуют ссылки на удаляемые объекты';uk=""Не вилучений жоден з об'єктів, тому що в інформаційній базі існують посилання на об'єкти, що віддаляються""");
	Иначе
		СтрокаРезультатов = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Удаление помеченных объектов завершено."
"Удалено объектов: %1.';uk=""Видалення позначених об'єктів завершене. Вилучене об'єктів: %1."""),
...
"Нажмите ОК для просмотра списка оставшихся (не удаленных) объектов.';uk=""Не вилучене об'єктів: %1. Об'єкти не вилучені для збереження цілісності інформаційної бази, тому що на них ще є посилання. Натисніть ОК для перегляду списку, що залишилися (не вилучених) об'єктів."""),
