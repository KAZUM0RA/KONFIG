...
#Если ВебКлиент Тогда
	Отказ = Истина;
	Предупреждение(НСтр("ru='В Веб-клиенте извлечение текстов не поддерживается.';uk='У Веб-клієнтові добування текстів не підтримується.'"));
	Возврат;
#КонецЕсли
...
Процедура ЗаписьЖурналаРегистрацииСервер(ТекстСообщения)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Извлечение текста файла';uk='Добування тексту файлу'"), УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
...
Процедура ОбновлениеОбратногоОтсчета()
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='До начала извлечения текстов осталось %1 сек';uk=' До початку добування текстів залишилося %1 сек'"), 
							Осталось);
...
Процедура ИзвлечениеТекстовКлиент(РазмерПорции = Неопределено)
	...
	Состояние(НСтр("ru='Начато извлечение текста';uk='Почате добування тексту'"));
	...
	Попытка
		...
		Если МассивФайлов.Количество() = 0 Тогда
			Состояние(НСтр("ru='Нет файлов для извлечения текста';uk='Немає файлів для добування тексту'"));
			Возврат;
		КонецЕсли;
		...
		Для Индекс = 0 По МассивФайлов.Количество() - 1 Цикл
			...
			Попытка
				...
				Состояние(НСтр("ru='Идет извлечение текста файла';uk='Іде добування тексту файлу'"), Прогресс, ИмяСРасширением);
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Во время извлечения текста из файла ""%1"" произошла неизвестная ошибка.';uk=' Під час добування тексту з файлу ""%1"" відбулася невідома помилка.'"), 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								 НСтр("ru='Извлечение текста завершено. Обработано файлов: %1';uk='Добування тексту завершене. Оброблене файлів: %1'"), 
...
Процедура ИзвлечьВсе(Команда)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Завершено извлечение текста из всех файлов с неизвлеченным текстом. Обработано файлов: %1.';uk='Завершене добування тексту із усіх файлів з невитягнутим текстом. Оброблене файлів: %1.'"),
