...
Процедура УстановитьТекстПоясняющейНадписи(Знач ЗначениеПереключателя)
	...
	Если ЗначениеПереключателя  = 1 Тогда
		ТекстНадписи = НСтр("ru='Нажмите ""Далее"" для перехода к настройке расписания автоматического резервного копирования.';uk='Натисніть ""Далі"" для перехід до настроювання розкладу автоматичного резервного копіювання.'");
	ИначеЕсли ЗначениеПереключателя = 2 Тогда
		ТекстНадписи = НСтр("ru='Более подробно о выполнении резервного копирования можно узнать из документации к используемой СУБД или специализированным средствам резервного копирования. Нажмите ""Готово"", чтобы закрыть помощник.';uk=' Більш докладно про виконання резервного копіювання можна довідатися з документації до використовуваної СУБД або спеціалізованим засобам резервного копіювання. Натисніть ""Готове"", щоб закрити помічник.'");
	ИначеЕсли ЗначениеПереключателя = 3 Тогда
		ТекстНадписи = НСтр("ru='Нажмите ""Готово"", чтобы отложить оповещение на выбранный срок.';uk='Натисніть ""Готове"", щоб відкласти оповіщення на обраний строк.'");
	КонецЕсли;
...
Процедура ПриЗавершенииРаботыПомощника(РезультатРаботыПомощника)
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Работа помощника завершена.';uk='Робота помічника завершена.'"), "e1cib/app/Обработка.НастройкаРезервногоКопированияИБ", РезультатРаботыПомощника);
	ЭтаФорма.Закрыть();
...
Процедура ОбработатьНачальнуюНастройкуПомощника()
	...
	ИначеЕсли КодПереключателя = 2 Тогда // Копирование сторонними средствами.
		ОповещениеПользователю(НСтр("ru='Работа помощника завершена. С этого момента оповещения о резервном копировании поступать не будут.';uk='Робота помічника завершена. Із цього моменту оповіщення про резервне копіювання надходити не будуть.'"));
	...
	ИначеЕсли КодПереключателя = 3 Тогда // Отложенное копирование.
		...
		ОповещениеПользователю(НСтр("ru='Был изменен период оповещений о резервном копировании.';uk='Був змінений період оповіщень про резервне копіювання.'"));
	КонецЕсли;
	ПриОткрытииНовойСтраницы(НоваяСтраницаПомощника);
...
Процедура ПриПереходеНаАвтоматическийРежимКопирования()
	...
	Если ПроверитьЗаполнение() Тогда
		...
		КонецЕсли;	
		ПриЗавершенииРаботыПомощника(НСтр("ru='Настройка расписания резервного копирования прошла успешно.';uk='Настроювання розкладу резервного копіювання пройшло успішно.'"));
