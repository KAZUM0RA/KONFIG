...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("ИмяФайлаСообщенияОбмена") Тогда
		...
		НСтрока = НСтр("ru='Форма обработки не может быть открыта интерактивно.';uk='Форма обробки не може бути відкрита интерактивно.'");
	...
	СписокВыбора.Добавить("ВсеОбъекты",                          НСтр("ru='Все данные';uk='Усі дані'"));
	СписокВыбора.Добавить("СопоставленныеОбъектыНеутвержденные", НСтр("ru='Изменения';uk='Зміни'"));
	СписокВыбора.Добавить("СопоставленныеОбъекты",               НСтр("ru='Сопоставленные данные';uk='Зіставлені дані'"));
	СписокВыбора.Добавить("НесопоставленныеОбъекты",             НСтр("ru='Несопоставленные данные';uk='Незіставлені дані'"));
	СписокВыбора.Добавить("НесопоставленныеОбъектыПриемника",    НСтр("ru='Несопоставленные данные этой базы';uk='Незіставлені дані цієї бази'"));
	СписокВыбора.Добавить("НесопоставленныеОбъектыИсточника",    НСтр("ru='Несопоставленные данные второй базы';uk='Незіставлені дані другої бази'"));
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ТаблицаНеутвержденныхСвязей.Количество() > 0 Тогда
		...
		Если ВыдаватьПредупреждениеПриЗакрытииФормы = Истина Тогда
			...
			НСтрока = НСтр("ru='Данные были изменены. Сохранить изменения?';uk='Дані були змінені. Зберегти зміни?'");
		...
		Если Отказ Тогда
			...
			Предупреждение(НСтр("ru='Возникли ошибки.';uk='Виникли помилки.'"));
...
Процедура Обновить(Команда)
	...
	Состояние(НСтр("ru='Выполняется сопоставление объектов. Пожалуйста, подождите...';uk=""Виконується зіставлення об'єктів. Будь ласка, почекайте..."""));
	...
	Если Отказ Тогда
		...
		Предупреждение(НСтр("ru='При выполнении операции сопоставления возникли ошибки.';uk=' При виконанні операції зіставлення виникли помилки.'"));
...
Процедура ВыполнитьАвтоматическоеСопоставление(Команда)
	...
	Состояние(НСтр("ru='Выполняется автоматическое сопоставление. Пожалуйста, подождите...';uk='Виконується автоматичне зіставлення. Будь ласка, почекайте...'"));
	...
	Если Отказ Тогда
		...
		Предупреждение(НСтр("ru='Во время выполнения автоматического сопоставления возникли ошибки.';uk=' Під час виконання автоматичного зіставлення виникли помилки.'"));
...
Процедура ВыполнитьЗагрузкуДанных(Команда)
	...
	НСтрока = НСтр("ru='Выполнить загрузку данных в информационную базу?';uk='Виконати завантаження даних в інформаційну базу?'");
	...
	Если Объект.ДанныеУспешноЗагружены Тогда
		...
		НСтрока = НСтр("ru='Данные уже были успешно загружены. Выполнить загрузку данных повторно?';uk='Дані вже були успішно завантажені. Виконати завантаження даних повторно?'");
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("ru='Уменьшите количество полей (можно выбирать не более %1 полей)';uk='Зменшите кількість полів (можна вибирати не більш %1 полів)'");
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (ТекущиеДанные.СтатусСопоставления = -1
		...
		//
		Предупреждение(НСтр("ru='Объекты уже сопоставлены';uk=""Об'єкти вже зіставлені"""), 2);
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Если Результат.Количество() = 0 Тогда
		...
		Результат.Добавить(НСтр("ru='<не задан>';uk='<не заданий>'"));
