...
//
Процедура ЗаполнитьТаблицуЗапросов(ТаблицаЗапросов, ДопДокументы, ДопПоляШапки, УчитыватьТЧВДопДокументах = Ложь,ТолькоАктуальные = Ложь) Экспорт
	...
	Для Каждого ЭлементСостава Из Метаданные.КритерииОтбора.ДокументыПоЛицевомуСчету.Состав Цикл
	...
	Для Каждого СтрокаКорня Из ДеревоДокументов.Строки Цикл
		...
		Если Не ПустаяСтрока(ТекстУсловийПоДокументу) Тогда
			...
			Если СтрокаКорня.Документ.Имя = "ВводНачальныхОстатков" Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "NULL  КАК ВидОперации,", "ПРЕДСТАВЛЕНИЕССЫЛКИ(ТаблицаШапки.РазделУчета) КАК ВидОперации,");	
			КонецЕсли;
