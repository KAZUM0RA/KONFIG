...
#Если ВебКлиент Тогда
	Предупреждение(НСтр("ru='Данная операция недоступна в веб-клиенте.';uk='Дана операція недоступна у веб-клієнтові.'"));
	Возврат;
#КонецЕсли
	ОчиститьСообщения();
	...
	Если ТекЭлемент = Неопределено ИЛИ
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо выбрать точку действия или вложенный бизнес-процесс карты маршрута.';uk=' Необхідно вибрати крапку дії або вкладений бізнес-процес карти маршруту.'"),,
			"КартаМаршрута");
		Возврат;
	КонецЕсли;

	ЗаголовокФормы = НСтр("ru='Задачи по точке маршрута бизнес-процесса';uk='Завдання по крапці маршруту бізнес-процесу'");
