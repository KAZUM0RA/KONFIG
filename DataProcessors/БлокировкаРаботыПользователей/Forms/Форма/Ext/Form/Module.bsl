...
"%1';uk='Є активні сеанси, які не можуть бути завершені примусово. Блокування не встановлене. %1'"), 
				НазванияСоединенийИБ);
...
"%1';uk='Є активні сеанси, які не можуть бути завершені примусово ( тому що сервер працює під керуванням ОС, відмінної від Microsoft Windows). Блокування не встановлене. %1'"), 
				НазванияСоединенийИБ);
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;	
...
"Подробности см. в Журнале регистрации.';uk='Не вдалося завершити роботу всіх активних користувачів. Подробиці див. у Журналі реєстрації.'"), 30);
			Закрыть();
		КонецЕсли;
...
"• Нет, чтобы продолжить без корректировки.';uk='Сейчас будет завершена работа всех активных пользователей."
"Указано слишком близкое время начала действия блокировки, к которому пользователи могут не успеть сохранить все свои данные и завершить работу."
"Рекомендуется установить время начала на 5 минут отн'");
...
"• Нет, чтобы продолжить без корректировки.'");
		...
		Иначе	
			ТекстВопроса = НСтр("ru='Сейчас будет завершена работа всех активных пользователей."
"Продолжить?';uk='Зараз буде завершена робота всіх активних користувачів. Продовжити?'");
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Блокировка работы пользователей';uk='Блокування роботи користувачів'"),
		...
		?(Объект.ВременноЗапретитьРаботуПользователей, НСтр("ru='Блокировка установлена.';uk='Блокування встановлене.'"), НСтр("ru='Блокировка снята.';uk='Блокування зняте.'")), 
...
Функция УстановитьСнятьБлокировку()
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Завершение работы пользователей';uk='Завершення роботи користувачів'"), 
...
Процедура ОбновитьСтраницуНастройки()
	...
	Элементы.ЗапретитьРаботуРегламентныхЗаданий.Заголовок = ?(Объект.ВременноЗапретитьРаботуПользователей,
		НСтр("ru='Оставить блокировку работы регламентных заданий';uk='Залишити блокування роботи регламентних завдань'"), НСтр("ru='Также запретить работу регламентных заданий';uk='Також заборонити роботу регламентних завдань'"));
...
"Работа пользователей завершается. Осталось активных сеансов: %1';uk=' Будь ласка, почекайте... Робота користувачів завершується. Залишилося активних сеансів: %1'"),
