...
Процедура Искать(Направление)
	...
	Попытка
		Если ПустаяСтрока(СтрокаПоиска) Тогда
			Предупреждение(НСтр("ru='Введите, что нужно найти.';uk='Уведіть, що потрібно знайти.'"));
			Возврат;
		КонецЕсли;
		...
		НСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется поиск ""%1""...';uk='Виконується пошук ""%1""...'"), СтрокаПоиска);
		Состояние(НСтрока);
		...
		Иначе
			ПоказаныРезультатыСПо = НСтр("ru='Не найдено';uk='Не знайдене'");
			Элементы.Далее.Доступность = Ложь;
			Элементы.Назад.Доступность = Ложь;
		КонецЕсли;
		
		Если Направление = 0 И Результат.ТекущаяПозиция = 0 И Результат.СлишкомМногоРезультатов Тогда
			Предупреждение(НСтр("ru='Слишком много результатов, уточните запрос.';uk='Занадто багато результатів, уточните запит.'"));
		КонецЕсли;
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Попытка
			...
			Если ДатаАктуальностиИндекса <> '00010101000000' Тогда
				Элементы.СостояниеИндекса.Подсказка = НСтр("ru='Последнее обновление индекса: ';uk='Останнє відновлення індексу: '") + Строка(ДатаАктуальностиИндекса);
			КонецЕсли;
			...
			Если Не ИндексАктуален Тогда
				СостояниеИндекса = НСтр("ru='Требуется обновление индекса';uk=' Потрібне відновлення індексу'");
			КонецЕсли;
		Исключение
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		ПоказаныРезультатыСПо = НСтр("ru='Введите, что нужно найти.';uk='Уведіть, що потрібно знайти.'");
		ТекущаяПозиция = 0;
