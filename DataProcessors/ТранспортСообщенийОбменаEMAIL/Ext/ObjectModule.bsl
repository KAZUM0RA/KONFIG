...
Перем мТемаСообщения;		// шаблон темы сообщения
Перем мТелоСообщенияПростое;	// текст тела сообщения с вложением - файлом XML
Перем мТелоСообщенияСжатое;		// текст тела сообщения с вложением - сжатым файлом
Перем мТелоСообщенияПакетное;	// текст тела сообщения с вложением - сжатый файл, в котором с набор файлов
...
Функция ФайлСообщенияОбменаСуществует() Экспорт
	...
	Для Каждого ПочтовоеСообщение Из НаборСообщений Цикл
		...
		Если ВРег(СокрЛП(ПочтовоеСообщение.Тема)) = ВРег(СокрЛП(мТемаСообщения)) Тогда
...
Процедура Инициализация() Экспорт
	...
	мТемаСообщения = "Exchange message (%1)"; // строка не подлежит локализации
	мТемаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(мТемаСообщения, ШаблонИмениФайлаСообщения);
	
	мТелоСообщенияПростое	= НСтр("ru='Сообщение обмена данными';uk='Повідомлення обміну даними'");
	мТелоСообщенияСжатое	= НСтр("ru='Сжатое сообщение обмена данными';uk='Стисле повідомлення обміну даними'");
	мТелоСообщенияПакетное	= НСтр("ru='Пакетное сообщение обмена данными';uk='Пакетне повідомлення обміну даними'");
...
Функция ПолучитьСообщениеОбмена()
	...
	Если ТаблицаСообщенийОбмена.Количество() = 0 Тогда
		...
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, мТемаСообщения);
...
Процедура ИнициализацияСообщенийОшибок()
	...
	// Общие коды ошибок
	мСообщенияОшибок.Вставить(001, НСтр("ru='Не обнаружены сообщения обмена.';uk='Не виявлені повідомлення обміну.'"));
	...
	мСообщенияОшибок.Вставить(006, НСтр("ru='Сообщение обмена не отправлено: превышен допустимый размер сообщения.';uk='Повідомлення обміну не відправлене: перевищений припустимий розмір повідомлення.'"));
	
	// Коды ошибок, зависящие от вида транспорта
	мСообщенияОшибок.Вставить(101, НСтр("ru='Ошибка инициализации: не указана учетная запись электронной почты транспорта сообщений обмена.';uk='Помилка ініціалізації: не зазначений обліковий запис електронної пошти транспорту повідомлень обміну.'"));
	мСообщенияОшибок.Вставить(102, НСтр("ru='Ошибка при отправке сообщения электронной почты.';uk='Помилка при відправленні повідомлення електронної пошти.'"));
	мСообщенияОшибок.Вставить(103, НСтр("ru='Ошибка при получении заголовков сообщений с сервера электронной почты.';uk='Помилка при одержанні заголовків повідомлень із сервера електронної пошти.'"));
	мСообщенияОшибок.Вставить(104, НСтр("ru='Не обнаружены сообщения обмена на почтовом сервере.';uk='Не виявлені повідомлення обміну на поштовому сервері.'"));
	мСообщенияОшибок.Вставить(105, НСтр("ru='Ошибка при получении сообщения с сервера электронной почты.';uk='Помилка при одержанні повідомлення із сервера електронної пошти.'"));
	мСообщенияОшибок.Вставить(106, НСтр("ru='Ошибка при записи файла сообщения обмена на диск.';uk='Помилка при записі файлу повідомлення обміну на диск.'"));
	мСообщенияОшибок.Вставить(107, НСтр("ru='Проверка параметров учетной записи завершилась с ошибками.';uk='Перевірка параметрів обліковому запису завершилася з помилками.'"));
	...
	мСообщенияОшибок.Вставить(109, НСтр("ru='Ошибка: в почтовом сообщении не найден файл с сообщением.';uk='Помилка: у поштовому повідомленні не знайдений файл із повідомленням.'"));
...
Функция ОтправитьСообщениеПоЭлектроннойПочте(Тело, ИмяФайлаИсходящегоСообщения, ПутьКФайлу)
	...
	ПараметрыСообщения.Вставить("Тема", мТемаСообщения);
