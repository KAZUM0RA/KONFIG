...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.НадписьПояснениеКИнтерактивномуКопированию.Заголовок = НСтр("ru='Нажмите ""Далее"" для выполнения резервного копирования прямо сейчас.';uk='Натисніть ""Далі"" для виконання резервного копіювання прямо зараз.'");
	КонецЕсли;
...
Процедура ПереключательИнтерактивногоРезервногоКопированияПриИзменении(Элемент)
	...
	ИначеЕсли ПараметрПереключения = 2 Тогда
		...
		Элементы.НадписьПояснениеКИнтерактивномуКопированию.Заголовок = НСтр("ru='Нажмите ""Готово"", чтобы закрыть помощник. Резервное копирование будет проведено через указанный период.';uk='Натисніть ""Готове"", щоб закрити помічник. Резервне копіювання буде проведено через зазначений період.'");
	ИначеЕсли ПараметрПереключения = 3  Тогда
		...
		Элементы.НадписьПояснениеКИнтерактивномуКопированию.Заголовок = НСтр("ru='Нажмите ""Готово"", чтобы закрыть помощник. Резервное копирование начнется перед закрытием программы.';uk='Натисніть ""Готове"", щоб закрити помічник. Резервне копіювання почнеться перед закриттям програми.'");
	КонецЕсли;	
...
"%1';uk='Є активні сеанси, які не можуть бути завершені примусово. %1'"), 
						НазванияСоединенийИБ);
	...
	ИначеЕсли НоваяСтраница = ПодчиненныеСтраницы.ДополнительныеНастройки Тогда
		...
		Если Объект.КодТипаИнтерактивногоРезервногоКопирования = 1 Тогда 
			Элементы.ПояснениеКДалее.Заголовок = НСтр("ru='Нажмите ""Далее"" для выполнения резервного копирования';uk='Натисніть ""Далі"" для виконання резервного копіювання'");
			УстановитьЗаголовокКнопкиДалее(Истина);
		Иначе
			Элементы.ПояснениеКДалее.Заголовок = НСтр("ru='Нажмите ""Готово"", чтобы завершить настройку резервного копирования';uk='Натисніть ""Готове"", щоб завершити настроювання резервного копіювання'");
			УстановитьЗаголовокКнопкиДалее(Ложь);
		КонецЕсли;	
	КонецЕсли;	
...
Функция ПроверитьЗаполнениеРеквизитов(ИнформацияОбОшибке)
	...
	Если ПустаяСтрока(Объект.КаталогСРезервнымиКопиями) Тогда
		ИнформацияОбОшибке = Нстр("ru='Не выбран каталог для резервной копии.';uk='Не обраний каталог для резервної копії.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НайтиФайлы(Объект.КаталогСРезервнымиКопиями).Количество() = 0 Тогда
		ИнформацияОбОшибке = Нстр("ru='Указан несуществующий каталог.';uk='Зазначений неіснуючий каталог.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = Нстр("ru='Нет доступа к каталогу с резервными копиями.';uk='Немає доступу до каталогу з резервними копіями.'");
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОбработатьВыборРежимаИнтерактивногоКопирования()
	...
	ИначеЕсли КодПереключателя = 2 Тогда
		...
		ОповещениеПользователю(НСтр("ru='Резервное копирование будет проведено через указанный срок.';uk='Резервне копіювання буде проведено через зазначений строк.'"));
		
	ИначеЕсли КодПереключателя = 3 Тогда
		
		ОповещатьОРезервномКопированииПриЗавершенииСеанса = Истина;
		ОповещениеПользователю(НСтр("ru='Резервное копирование будет проведено при завершении работы программы.';uk='Резервне копіювання буде проведено при завершенні роботи програми.'"));
...
Процедура УстановитьЗаголовокКнопкиДалее(ПараметрЗаголовка)
	...
	Иначе
		Элементы.Далее.Заголовок = НСтр("ru='Готово';uk='Готове'");
	КонецЕсли;
...
Функция ОбработкаЗакрытияФормы()
	...
	Если ТекущаяСтраница = Элементы.СтраницыПомощника.ПодчиненныеЭлементы.СтраницаИнформацииИВыполненияРезервногоКопирования Тогда
		...
		ТекстВопроса = НСтр("ru='Прервать подготовку к резервному копированию данных?';uk='Перервати підготовку до резервного копіювання даних?'");
		Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, ,КодВозвратаДиалога.Нет, НСтр("ru='Отмена резервного копирования.';uk='Скасування резервного копіювання.'"));
